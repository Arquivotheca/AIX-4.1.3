static char sccsid[] = "@(#)69	1.1  src/bos/diag/tu/gga/fastcpytu.c, tu_gla, bos41J, 9515A_all 4/6/95 09:26:55";
/*
...
int fast_copy_tu (void)
{
  ...
  ULONG xres, yres, *source_addr, *target_addr;
  ULONG x, y, data;
  BOX     box;
  ...
  if ((rc = clear_screen ()) == SUCCESS)
  {
    ...
    num_scan_lines = (ULONG) (box.yend - box.ystart + 1);   /* lines to cpy */

    /* calculate the from/to addresses to copy (using NATIVE addressing)      */
    source_addr =                                /* addr of scr to copy from  */
       W9100ADDR((ULONG) box.xstart, (ULONG) (box.yend + 1));
    target_addr =                                /* addr of scr to copy to    */
       W9100ADDR((ULONG) box.xstart, (ULONG) box.ystart);
      ...
      if (rc == SUCCESS)
      {
        ...
        for (y = box.ystart; (rc == SUCCESS) && (y <= box.yend); y++)
          ...
          {           /* increment x by BYTES_IN_WORD because we R/W 4 pixels */
            data = W9100_NATIVE_READ(x, y);      /* get the data from adapter */
            if (data != BLACK)                   /* error if data != BLACK (0)*/
            {                                    /*because bottom of scr = BLK*/
              rc = VRAM_FAST_COPY_ERR;
              set_mem_info ((ULONG) W9100ADDR(x, y), BLACK, data, rc,
    ...
    if (rc == SUCCESS)
      while (!end_tu (get_dply_time ()));        /* wait if keytouch on      */
  } /* endif */
