static char sccsid[] = "@(#)36  1.3  src/bos/diag/tu/ethstw/tu00E.c, tu_ethi_stw, bos411, 9428A410j 10/5/92 14:56:27";
/******************************************************************************
...

/*****************************************************************************
Function(s) Test Unit 00E - Riser External Loopback Test (Twisted Pair)
			    with Fairness.	
Module Name :  tu00E.c
...
*****************************************************************************/
int tu00E (int fdes, TUTYPE *tucb_ptr) 
   {
	uchar rd_data;
	...
	/**********************************
        ...
        if (rc = pos_rd(tucb_ptr->mdd_fd, POS2, &rd_data))
          {
          PRINT((tucb_ptr->msg_file,"tu00E: Unable to read POS2\n"));
          return (POS2_RD_ERR);
          }
        ...
        if (rc = pos_wr(tucb_ptr->mdd_fd, POS2, &rd_data))
          {
          PRINT((tucb_ptr->msg_file,"tu00E: Unable to write to POS2\n"));
          return (POS2_WR_ERR);
          }
	...
	if (io_rd(tucb_ptr->mdd_fd, IO6, &rd_data))
	  {
          PRINT ((tucb_ptr->msg_file,"tu00E: Unable to read I/O reg 6\n"));
          return (IO6_RD_ERR);
          }
	...
	if (rd_data != TP_CARD)
	  {
          PRINT((tucb_ptr->msg_file,"tu00E: Riser card is not Twisted Pair\n"));
