static char sccsid[] = "@(#)37  1.3  src/bos/diag/tu/ethstw/tu00F.c, tu_ethi_stw, bos411, 9428A410j 10/5/92 14:57:33";
/******************************************************************************
...

/*****************************************************************************
Function(s) Test Unit 015 - Riser External Loopback Test (Twisted Pair)
			    with Parity and Fairness enable.
Module Name :  tu00F.c
...
*****************************************************************************/
int tu00F (int fdes, TUTYPE *tucb_ptr) 
   {
	uchar rd_data;
	...
	/**********************************
        ...
        if (rc = pos_rd(tucb_ptr->mdd_fd, POS2, &rd_data))
          {
          PRINT((tucb_ptr->msg_file,"tu00F: Unable to read POS2\n"));
          return (POS2_RD_ERR);
          }
        ...
        if (rc = pos_wr(tucb_ptr->mdd_fd, POS2, &rd_data))
          {
          PRINT((tucb_ptr->msg_file,"tu00F: Unable to write to POS2\n"));
          return (POS2_WR_ERR);
          }
	...
	if (io_rd(tucb_ptr->mdd_fd, IO6, &rd_data))
          {
          PRINT ((tucb_ptr->msg_file,"tu00F: Unable to read I/O reg 6\n"));
          return (IO6_RD_ERR);
          }
	...
	if (rd_data != TP_CARD)
          {
	  PRINT((tucb_ptr->msg_file,"tu00F: Riser card is not Twisted Pair\n"));
