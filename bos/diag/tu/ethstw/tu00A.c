static char sccsid[] = "@(#)32  1.3  src/bos/diag/tu/ethstw/tu00A.c, tu_ethi_stw, bos411, 9428A410j 10/5/92 14:54:49";
/*****************************************************************************
...

/*****************************************************************************
Function(s) Test Unit 00A - External Loopback Test (BNC/DIX) with Parity
...
External Wrap Test (BNC or DIX) with Parity
*****************************************************************************/
int tu00A (int fdes, TUTYPE *tucb_ptr) 
   {
	uchar rd_data;
	...
	/**********************************
        ...
        if (rc = pos_rd(tucb_ptr->mdd_fd, POS2, &rd_data))
          {
          PRINT((tucb_ptr->msg_file,"tu00A: Unable to read POS2\n"));
          return (POS2_RD_ERR);
          }
        ...
        if (rc = pos_wr(tucb_ptr->mdd_fd, POS2, &rd_data))
          {
          PRINT((tucb_ptr->msg_file,"tu00A: Unable to write to POS2\n"));
          return (POS2_WR_ERR);
          }
	...
	/***********************
        ...
        if (io_rd(tucb_ptr->mdd_fd, IO6, &rd_data))
          {
	  PRINT ((tucb_ptr->msg_file,"tu00A: Unable to read I/O reg 6\n"));
          return (IO6_RD_ERR);
          }
	...
	if ((rd_data != TK_GDFUSE) && (rd_data != TN_CARD))
          ...
          if(rd_data == TK_BDFUSE)
            {
            PRINT((tucb_ptr->msg_file,"tu00A: Thick card has a blown fuse.\n"));
            return (BADFUSE);
            }

          PRINT((tucb_ptr->msg_file,"tu00A: Riser card is not THICK/THIN\n"));
