static char sccsid[] = "@(#)93	1.3  src/bos/diag/tu/sky/a000.c, tu_sky, bos411, 9428A410j 6/1/94 10:14:51";
/*
...
{ word ret_code;
  ...
  do
  {
    ...
    {
      writebyte(0x00, INDX_ADDR);
      ret_code = testreg(DATAB_ADDR,0x0E,MEMCONF,"Memory Configuration Reg");
      if (ret_code > ERR_LEVEL) break;
      X_PLUS_2 = TRUE;
...
{ word ret_code;
  ...
  { writebyte(0x10,INDX_ADDR);
    ...
    if (X_PLUS_2)
    {
      ...
      ret_code = testreg(DATAB_ADDR ,0xDB,HORZSBRD,"Start Horz. Border Reg");
      if (ret_code > ERR_LEVEL) break;
      writebyte(0x18,INDX_ADDR);
      ret_code = testreg(DATAB_ADDR,0xAC,HORZSYNS,"Horz. Sync Start Reg");
      if (ret_code > ERR_LEVEL) break;
      writebyte(0x1A,INDX_ADDR);
      ret_code = testreg(DATAB_ADDR,0x93,HORZSYE1,"Horz. Sync End 1 Reg");
      if (ret_code > ERR_LEVEL) break;
...
{ word ret_code;
  ...
  do
  {
    ...
    if (X_PLUS_2)
    {
      ...
      ret_code = testreg(DATAB_ADDR ,0x04 ,VERTSYE,"Vert. Sync End 1 Reg ");
      if (ret_code > ERR_LEVEL) break;
    }
    else
    {
...
{ word ret_code;
  ...
  do
  {
    ...
    ret_code = tstreg(DATAB_ADDR ,tinfo.misc.dispmod1,0x63,DISPMOD1,"Display Mode 1 Reg");
    if (ret_code > ERR_LEVEL) break;
