static char sccsid[] = "@(#)66	1.2  src/bos/diag/tu/swmono/e030.c, tu_swmono, bos411, 9428A410j 10/29/93 14:19:23";
/*
...
{ 
  ...
  do
  {
    ...
    rc = make_map(MAPC, skyvbase+0x0A0000, mapht=64-1, mapwth=480-1, BPP4);FUSE;
    ...
    /* 128 * 512  Color Bars */
    rc = make_map(MAPC, skyvbase+0x0A0000, mapht=512, mapwth=160-1, BPP4);FUSE;
    for (dec=0;dec<8; dec++)
    ...
    rc = make_map(MAPC, skyvbase+0x0A0000, mapht=127, mapwth=160-1, BPP4);FUSE;
    for (dec=0;dec<8; dec++)
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=95,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=42,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=32,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=16,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=8,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=4,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=12,BPP4);FUSE;
    rc = clrscr(MAPC, black); FUSE;
    rc = s2dpxblt(MAPC,0x00,0x00,mapwth,mapht,MAPA,948,895);FUSE;
    strcpy(tinfo.info.err_buff,"E0300000:");

    rc = make_map(MAPC,skyvbase+0x0A0000,mapht=127,mapwth=2,BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
    ...
    rc = make_map(MAPC, skyvbase+0x0A0000, mapht=127, mapwth=295, BPP4);FUSE;
    rc = clrscr(MAPC, black); FUSE;
    rc = s2dpxblt(MAPC,0x00,0x00,mapwth,mapht,MAPA,984,895);FUSE;

    /* Lower Left Corner Single Pixels */
    rc = make_map(MAPC, skyvbase+0x0A0000, mapht=0, mapwth=0, BPP4);FUSE;
    rc = clrscr(MAPC, white); FUSE;
