static char sccsid[] = "@(#)61	1.2  src/bos/diag/tu/swmono/c0f1.c, tu_swmono, bos411, 9428A410j 10/29/93 14:19:11";
/*
...
do
{
  ...
  do /* C030 */
  {
    ...
    rc = make_map(MAPB, skyvbase+0x0A0000, 0xFF, 0xFF, BPP4); FUSE;
    rc = clrscr(MAPB, red); FUSE;

    rc = bres_h304560v(MAPB, white); FUSE;
    rc = bresbox(MAPB, white); FUSE;

    rc = make_map(MAPC, skyvbase+0x0A0000+0x08000, 0xA0, 0xA0, BPP4);FUSE;
  ...
  { 
    ...
    rc = make_map(MAPC, skyvbase+0x0A0000+0x08000, ht, wth, BPP1);FUSE;
  ...
  {
    col = 0;
    rc = make_map(MAPB, skyvbase+0x0A0000, 0x1FF, 0x1FF, BPP4);FUSE;
    ...
    for (msz=0x1FF, xoff=yoff=0; msz > 2; yoff=xoff=(0x200-(msz/=1.3))/2)
    {
      ...
      writebyte(BPP1, MEM_ACCESS_MODE);
      writeword(COL_CMP_OFF, COLOR_CMP_COND);
      rc = make_map(MASK, skyvbase+0x0A0000+0x20000, ht, wth, BPP1);FUSE;
      writebyte(BPP1, MEM_ACCESS_MODE);
  ...
  { 
    ...
    writebyte(BPP4, MEM_ACCESS_MODE);
    rc = make_map(MAPA, skyvbase+0x0A0000, 0x1FF, 0x1FF, BPP4);FUSE;
    ...
    rc = make_map(MASK, skyvbase+0x0A0000+0x20000, ht, wth, BPP1);FUSE;
    rc = make_map(MAPC, skyvbase+0x0A0000+0x20000, ht, wth, BPP1);FUSE;
    ...
    */
    for (msz=0x1FF, xoff=yoff=0; msz > 2; msz/=2.0)
    {
      ...
      writebyte(BPP1, MEM_ACCESS_MODE);
      writeword(COL_CMP_OFF, COLOR_CMP_COND);
      rc = make_map(MAPB, skyvbase+0x0A0000+0x20000+0x40000/4,ht,wth,BPP1);FUSE;
  ...
  rc = make_map(MAPA, skyvbase, 1024-1, 1280-1, BPP4); FUSE;
  rc = set_ret(0xC0F10000,0xCD1B, rc, MAPA);
