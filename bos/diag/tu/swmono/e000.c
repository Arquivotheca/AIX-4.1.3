static char sccsid[] = "@(#)62	1.2  src/bos/diag/tu/swmono/e000.c, tu_swmono, bos411, 9428A410j 10/29/93 14:19:15";
/*
...
e00c()
{
  ...
  {
    initcop();
    rc = make_map(MAPA, skyvbase, 341-1, 1280-1, BPP4); FUSE;
    rc = clrscr(MAPA, red=0x00); FUSE;
    rc = make_map(MAPB, skyvbase+1280/2*341,  341-1, 1280-1, BPP4); FUSE;
    rc = clrscr(MAPB, green=0x08); FUSE;
    rc = make_map(MAPC, skyvbase+1280/2*341*2,  342-1, 1280-1, BPP4); FUSE;
    rc = clrscr(MAPC, blue=0x0F); FUSE;
    rc = make_map(MAPA, skyvbase, 1024-1, 1280-1, BPP4); FUSE;
  } while (FALSE);  /* do once allowing for break on err */
                      /* and red=0x00o if we were interrupted   */
...
{ word data, rc = GOOD_OP;
  ...
  { 
    initcop();
    rc = make_map(MAPA, skyvbase, 1023, 1279, BPP4); FUSE;
    clrscr(MAPA, bcol);
    ...
    rc=cursor_up(0x01,0xE0,0x00,0x5F); FUSE;
...
{ word data, rc = GOOD_OP;
  ...
  { 
    ...
    rc=cursor_up(0x02,0x00,0x00,0x5F); FUSE;
...
{ 
  ...
  make_map(MAPA, skyvbase, 1024-1, 1280-1, BPP4);
  clrscr(MAPA, black);
  return(0);
}
/***************************************************************************
