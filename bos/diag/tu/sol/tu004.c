static char sccsid[] = "@(#)35	1.1  src/bos/diag/tu/sol/tu004.c, tu_sol, bos411, 9428A410j 4/23/91 11:43:54";
/*
 * COMPONENT_NAME:  SOL Test Unit
 *
 * FUNCTIONS: tu_004  (Optics OPTICAL wrap test (Activate, SCR, OLS, RHR, CRC))
 *
 * ORIGINS: 27
 *
...
TUTYPE *tucb_ptr;
{
  ...
  diag_struct.diag_mode = SOL_NORMAL;
  ...
  result = ioctl(fdes,SOL_SYNC_OTP,&diag_struct);
  if (result != 0) {                      /* if error */
          if (errno == EIO) {
                return(SOL_SYNC_OTP_FAILED);
          } else {
               tucb_ptr -> diag.sla_error = errno;
               return(SOL_PROGRAMMING_ERROR);
          } /* endif */
