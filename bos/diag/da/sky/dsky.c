static char sccsid[] = "@(#)41	1.9  src/bos/diag/da/sky/dsky.c, dasky, bos411, 9428A410j 5/26/94 09:45:51";
/*************************************************************************
 *
 * COMPONENT_NAME: DASKY  -  diagnostic application to test Color Graphics Adapter
 *
...
{
	{"", FRUB1, 0x874, 0x110, DSKY_RGB,  /* 100% BASE ADAPTER FAILED */
		{
		    {100, "", "", 0, DA_NAME, NONEXEMPT},
		},
	},

	{"", FRUB1, 0x874, 0x120, DSKY_RGB,
		{
...
int             i;		/* loop counter */
int		menunum = 0x874000;
int		l_menunum = 0x874010;
int		colornum = 0x874020;
int		colorselfnum = 0x874030;
int		tu_exit_flg = 0;
int		disp_attached = 0;
...
interact_tu_seq()
{
	    ...
	    for (i=0; i<3; ++i) {
	        ...
	        switch (i) 
		  {
	        ...
	        /* Put up screen telling that a color will be displayed */
              ...
              rc = diag_display(0x874040, catd, NULL, DIAG_IO,
	...
	rc = diag_display(0x874123, catd, NULL, DIAG_IO,
...
int color;
{
	...
	{
		first = FALSE;
		sprintf(string,t_uinfo[1].text, diag_cat_gets(catd,DSKY_MSGS,color));
	...
	else
	{
		...
		menutypes->cur_index = 1;
		menutypes->multi_select = 'y';
		sprintf(string,t_uinfo[3].text, diag_cat_gets(catd,DSKY_MSGS,color));
