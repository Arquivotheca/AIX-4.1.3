static char sccsid[] = "@(#)88 1.1 src/bos/usr/fvcioem/ddt/tok/ddt_com.c, fvcioem, bos411, 9428A410j 4/26/94 13:53:58";
...
int         dumpinit;      /* dump init flag */
int         dumpstart;     /* dump start flag */
   ...
   char  *devpath;
{
   ...
   while (!mode) {
       ...
       i = prompt("\t Read/Write (R), or Diagnostic mode (D)",
               "%c", "R");
#else
       i = prompt("\t Read/Write (R), or Diag (D), or Wrap (W) mode",
               "%c", "R");
#endif
       if ( i < 0 ) return(-1);
       mode = i;
   ...
   if (fildes < 0) {       /* if error, */
       ...
       if (kern){ /* set kid mbuf threshold and fastwrite address */
      IOCTL( fildes, KID_MBUF_THRESH, atoi(profile.mbuf_thresh));
      IOCTL(fildes,KID_SET_FASTWRT,NULL);
       }
       printf("\n\t -- Open succeeded --");
   }
    exit:
   ...
   int   errcode)
{
   ...
   fprintf(stderr, "\n\tERROR: ");
   if (errcode <= sys_nerr)
       fprintf(stderr, "%d --- %s", errcode, sys_errlist[errcode]);
   else
   ...
   char     *filepath)
{
   ...
   print_buffer( buffer, length, 0, fp, TRUE);  /* save buffer */
   fprintf(fp, "\n");         /* end line */
