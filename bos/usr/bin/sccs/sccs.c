static char sccsid[] = "@(#)46 1.22 src/bos/usr/bin/sccs/sccs.c, cmdsccs, bos41B, 9504A 12/9/94 10:00:02";
/*
...
static struct sccsprog SccsProg[] =
{
	...
	"sccshelp",	PROG,	NO_SDOT,		PROGPATH(sccshelp),
...
# ifndef SCCSDIR
	...
	if (p != NULL && p[0] != '\0')
	{
		...
		else
		{
			...
			if (pw == NULL)
			{
				usrerr(MSGSTR(NOUSER, "User %s does not exist.\n\
\tChange the PROJECTDIR environment variable to an existing user name.\n"), p); /*MSG*/
			...
			if (access(buf, 0) < 0)
			{
				...
				if (access(buf, 0) < 0)
				{
					usrerr(MSGSTR(NOSOURCE,"Project %s has no /source or /src subdirectory.\n\
\tCreate a /source or /src subdirectory in the project home directory.\n"), p); /*MSG*/
					exit(EX_USAGE);
				}
			}
			SccsDir = buf;
...
# endif /* V6 */
	...
	*/
	...
	{
		fprintf(stderr, 
				MSGSTR(SCCSUSAGE, "Usage: %s [-d Path] [-p Path] [-r] Command [CommandFlag...] [File...]\n\
\tRuns SCCS programs as an interface between SCCS and the user.\n"), 
	...
	{
		while ((c = getopt(argc, argv, "rd:p:")) != EOF)
		{
			switch (c)
			{
...
# endif
			  ...
			  default:
				fprintf(stderr,MSGSTR(SCCSUSAGE, "Usage: %s [-d Path] [-p Path] [-r] Command [CommandFlag...] [File...]\n"), MyName);
				exit(1);
			}
		}
		if (SccsPath[0] == '\0')
...
static command(argv, forkflag, arg0)
	char **argv;
	bool forkflag;
	char *arg0;
{
	...
	*/
	...
	switch (cmd->sccsoper)
	{
	  ...
	  case FIX:		/* fix a delta */
		...
		{
			usrerr(MSGSTR(NEEDR4FIX, "Specify the -r flag with the fix command\n")); /*MSG*/
			rval = EX_USAGE;
			break;
		}
...
# endif
		...
		syserr(MSGSTR(CANNOTEXEC, "Cannot run %s.\n\
...
static callprog(progpath, flags, argv, forkflag)
	char *progpath;
	short flags;
	char **argv;
	bool forkflag;
...
# endif
		...
		if (i < 0)
		{
			syserr(MSGSTR(CANNOTFORK, "Cannot create another process at this time.\n\
\tTry again later or\n\
\tuse local problem reporting procedures.\n")); /*MSG*/
			exit(EX_OSERR);
		}
...
\tuse local problem reporting procedures.\n")); /*MSG*/
	...
	execv(progpath, argv);
	syserr(MSGSTR(CANTEXEC, "Cannot execute %s.\n\
\tCheck path name and permissions or\n\
	...
	char *name;
{
	...
	*/
	...
	else
	{
		...
		if (p == NULL)
		{
			perror(MSGSTR(NOMEM, "sccs: There is not enough memory available now.\n\
\tTry again later or\n\
\tuse local problem reporting procedures.\n")); /*MSG*/
			exit(EX_OSERR);
		}
	...
	char **argv;
{
	...
	*/
	...
	for (ap = argv; *++ap != NULL; )
	{
		...
		{
			if (subdir != NULL)
				usrerr(MSGSTR(TOOMANYARGS, "Specify only one directory.\n")); /*MSG*/
			else
				subdir = *ap;
		}
	}
	...
	*/
	...
	if (dirfd == NULL)
	{
		usrerr(MSGSTR(CANTOPEN, "Cannot open directory %s.\n\
\tCheck path name and permissions.\n"), buf); /*MSG*/
	...
	*/
		...
		*/
		...
		if (pfp != NULL)
		{
			...
			while ((pf = getpfent(pfp)) != NULL)
			{
				...
				printf(MSGSTR(BEDIT, "%s is being edited: "), basefile); /*MSG*/
				putpfent(pf, stdout);
			}
			fclose(pfp);
		}
		...
		/* remove l.file if it exists because it can be recreated from s.file */
		gstrcpy(bufend, "/l.", sizeof(buf));
		basefile = bufend + 3;
		gstrcpy(basefile, &dir->d_name[2], sizeof(buf));
		unlink(bufend+1);
	}
	...
	if (!gotedit && mode == INFOC)
	{
		...
		if (!nobranch) {
			if (usernm == NULL)
				printf(MSGSTR(NOEDIT, "Nothing is being edited.\n"));
			else
				printf(MSGSTR(NOEDITBY, "Nothing is being edited by %s.\n"), usernm);
		} else {
			if (usernm == NULL)
				printf(MSGSTR(NOEDITTRNK, "No trunk of the SCCS file tree is being edited.\n"));
			else
				printf(MSGSTR(NOEDITTRNKBY, 
				"No trunk of the SCCS file tree is being edited by %s.\n"), usernm);
		}
		printf("\n");
	}
	if (mode == CHECKC)
	...
	char *fn;
{
	...
	*++q = 'p';
	...
	if (pfp == NULL)
	{
		printf(MSGSTR(NOBEDIT, "File %s is not being edited.\n\
	...
	*/
	...
	if (delete) {
		...
		if (errno == 0)
			...
			if (access(".", 2) < 0) {
	bad:
				printf(MSGSTR(CANTREMOVE, "Cannot remove %s.\n\
\tCheck permissions.\n"), cp); /*MSG*/
				fclose(tfp);
				fclose(pfp);
				unlink(tfn);
	...
	if (others)
	{
		...
		{
			syserr(MSGSTR(CANTREOPEN, "Cannot open %s again.\n\
\tUse local problem reporting procedures.\n"), tfn); /*MSG*/
			exit(EX_OSERR);
		}
...
\tuse local problem reporting procedures.\n"), tfn); /*MSG*/
	...
	if (delete)
	{
		 ...
		 */
		if (unlink(cp) >= 0)
			printf(MSGSTR(REMOVED, "Temporary file %s is removed.\n"), cp); /*MSG*/
		return (TRUE);
	}
	else
	{
		printf(MSGSTR(NOEDITBYYOU, 
				"Only the user who is editing the file can unedit the file.\n"), fn); /*MSG*/
		return (FALSE);
	}
}
	...
	char *gfile;
{
	...
	if (pipe(pipev) < 0)
	{
		syserr(MSGSTR(DODIFFPIPE, "dodiff: The pipe system call failed.\n\
\tUse local problem reporting procedures.\n")); /*MSG*/
		exit(EX_OSERR);
	}
	if ((pid = fork()) < 0)
	{
		syserr(MSGSTR(DODIFFFORK,"dodiff: Cannot create another process at this time.\n\
\tTry again later or\n\
\tuse local problem reporting procedures.\n")); /*MSG*/
		exit(EX_OSERR);
	}
...
# ifdef UIDUSER
	...
	if (pw == NULL)
	{
		syserr(MSGSTR(UNKNOWNUID, " %d is not an existing user number.\n\
...
static gstrbotch(str1, str2)
	char	*str1, *str2;
{
	usrerr(MSGSTR(FNAMSTOOBIG,
			"The following file name or file names are too large\n\
\tfor the buffer:\n\
\t%1$s %2$s\n"), str1, str2); /*MSG*/
}
