static char sccsid[] = "@(#)37	1.23  src/bos/usr/bin/newform/newform.c, cmdfiles, bos411, 9428A410j 4/12/94 16:33:47";
/*
...
#include "newform_msg.h"
#define MSGSTR(id,ds) catgets(catd, MS_NEWFORM, id, ds)
static nl_catd catd;
#include	<stdio.h>
#include	<stdlib.h>
#include	<locale.h>
...
int	tabtbl[500] = {		/* Table containing tab stops		*/
	...
	*spectbl[40] = {	/* Table of pointers into tabtbl	*/
	...
	&tabtbl[0],		/* -s  specification (default)		*/
...
char	*pachar,		/* Prepend/append character		*/
	...
	*wlast  = &work[3*NCOLS], /* Pointer to end of work area	*/
	siline[NCOLS+1],	/* First standard input line		*/
	*savchr,     /* Holds 1 or 2 byte char stripped from line start */
...
void	coptionfail(), truncfail(), inputnfail();
/*
 * NAME: newform [-s] [-itabspec] [-otabspec] [-bn] [-en] [-pn] [-an]
 *       [-f] [-cchar] [-ln] [files]
 *                                                                    
 * FUNCTION: Changes the format of a text
 *           -a[num]        adds num characters to the end of a line
 *           -b[num]        truncates num characters from the beginning
 *                          of a line
 *           -c[char]       changes the prefix/add character to char
 *           -e[num]        truncates num characters from the end of
 *                          a line
 *           -f             writes the tab specification format line to
 *                          standard out before any other line is wrote
 *           -i[tabspec]    replaces all tabs with tabspec spaces
 *           -l[num]        sets the effective line length
 *           -o[tabspec]    replaces tabspec spaces with a tab
 *           -p[num]        prefixes num char's to the beginning of a line
 *           -s             removes leading characters from the beginning 
 *                          of a line and saves them and put them at the end
 *                          of the line unless another flag conflicts
 *
...
char	**argv;			/* Array of pointers to arguments	*/
{
	...
	catd = catopen(MF_NEWFORM, NL_CAT_LOCALE);

	if ((savchr = (char *) malloc(sizeof(char) * mb_cur_max * 9))== NULL)
	{
		perror ("newform");
		exit (2);
	}
		...
		while (--argc) {
			...
			if (*scan != '-') {
				if ((fp = fopen(scan,"r")) == NULL) {
					fprintf(stderr,
					MSGSTR(M_NOPEN, "newform: can't open %s\n"),scan);
					exit(1);
					}
				process(fp);
				fclose(fp);
 ...
 * NAME: options
 *                                                                    
 * FUNCTION: get options, set flags and check usage
 */  
options(argc, argv)		/* Process command line options		*/
int	argc;		        /* Count of arguments			*/
char	**argv;		        /* Array of pointers to arguments	*/
{
	int	i,n;		/* Temporary number holder		*/
	char    *pad;		/* Pad char tmp pointer			*/
	...
	while (--argc > 0) {
		...
		if (*scan++ == '-') {
			...
			case 'c':
				flp->option = 'c';
				if (*scan) {
					if ((n=mblen(scan, mb_cur_max))<1)
						coptionfail();
					if ((pad = (char *)malloc(sizeof(char)*n+1))==NULL) {
						perror("newform");
						exit(2);
						}
					for(i=0; i<n; i++)
			...
			case 'i':
				...
				break;
			case 'o':
				/* 1st, check for -o-0 'n ignore if found */
 				if(*scan=='-' && *(scan+1)=='0' && *(scan+2)=='\0')
					break;
				flp->option = 'o';
				strcpy(format,scan);
				okludge++;
	...
	return;
usageerr:
	fprintf(stderr,MSGSTR(M_USAGE, "usage: newform  [-f] [-s] [-a[NUMBER]] [-b[NUMBER]] [-c[char]]\n\t\t[-e[NUMBER]] [-i[tabspec]] [-l[NUMBER]] [-o[tabspec]]\n\t\t[-p[NUMBER]] [file ...]\n"));
	exit(1);
}
 ...
 * NAME: inputn
 *                                                                    
 * FUNCTION: convert string number into an integer
 */  
int inputn(scan)		/* Read a command option number		*/
char	*scan;		        /* Pointer to string of digits		*/
{
	int	n;		/* Number				*/
	...
	while ((c = *scan++) >= '0' && c <= '9') {
		n = n * 10 + c - '0';
		if (n > LINE_MAX) 		/* can't exceed LINE_MAX */
			inputnfail();
	}
	return(n);
}
 ...
 * NAME: inputnfail
 *                                                                    
 * FUNCTION: number specified with -pealb was larger than LINE_MAX.
 */  
void 
inputnfail()
{
	fprintf(stderr, MSGSTR(M_TOOLARGE, "newform: specify a number smaller than %d\n"),LINE_MAX);
	exit(1);
}
 ...
 * NAME: coptionfail
 *                                                                    
 * FUNCTION: char specified with -c option is invalid print error 
 *           message
 */  
void 
coptionfail()
{
	fprintf(stderr, MSGSTR(M_CWIDE, "newform: character format not supported in -c option\n"));
	exit(1);
}
 ...
 * NAME: truncfail
 *                                                                    
 * FUNCTION: truncate failed print error message
 */  
void
truncfail()
{
	fprintf(stderr, MSGSTR(M_TWIDE, "newform: cannot split a multibyte display character\n"));
	exit(1);
}
 ...
 * NAME: center
 *                                                                    
 * FUNCTION: center the text in the work area
 */  
center()			/* Center the text in the work area.	*/
{
	...
	if (plast - pfirst > MAXLINE) {
		fprintf(stderr,MSGSTR(M_2LONG, "newform: line length must be under %d bytes\n"),MAXLINE);
		exit(1);
		}
 ...
 * NAME: cnvtspec
 *                                                                    
 * FUNCTION: Convert tab secification to tab positions
 */  
int cnvtspec(p)		/* Convert tab specification to tab positions.	*/
char	*p;		      /* Pointer to spec string.		*/
{
	int	state,		/* DFA state				*/
	...
	if (lock == 1) {
		fprintf(stderr,MSGSTR(M_INDIR, "newform: tab specification error\n"));
		exit(1);
		}
	lock = 1;
	if (spectype == 12) {
	...
	if (spectype == 13) {
		if ((fp = fopen(filep,"r")) == NULL) {
			fprintf(stderr,MSGSTR(M_NOPEN, "newform: can't open %s\n"), filep);
			exit(1);
			}
		readline(fp,work);
		fclose(fp);
 ...
 * NAME: readspec
 *                                                                    
 * FUNCTION: Read tabspec from a file
 */  
int readspec(p)			/* Read a tabspec from a file		*/
char	*p;		        /* Pointer to buffer to process		*/
{
 ...
 * NAME: type
 *                                                                    
 * FUNCTION: Determine type of a charcter
 */  
 ...
 * NAME: process
 *                                                                    
 * FUNCTION: Process one line of input
 */  
 ...
 * NAME: append
 *                                                                    
 * FUNCTION: append characters to end of line
 */  
append(n)			/* Append characters to end of line.	*/
int	n;			/* Number of characters to append.	*/
{
	int  chlen;
	char *chtmp, 		/* walking pointer	           	*/
	     *chptr;		/* possible truncation point.      	*/
	int  padsize;		/* # of bytes in padchar (for mb case) 	*/
	int  i;

	if (pachar && (mb_cur_max>1))
		padsize = mblen(pachar, mb_cur_max);
	else
		padsize = 1;
	chtmp = pfirst;
	for (i=0; *chtmp && chtmp<=plast; i++) {	/* find str. length */
		if ((chlen=mblen(chtmp, mb_cur_max)) < 1)
			truncfail();
		chtmp += chlen;
		}
	i--;		/* don't count the newline: Defect 41996 */

	if (i < effll) {
		n = n ? n : effll - i;
		if (plast + (n-1) * padsize >= wlast) 
			center();
		while (n--) {
			if (padsize>1) {
				for(i=0; i<padsize; i++)
 ...
 * NAME: prepend
 *                                                                    
 * FUNCTION: prepend characters to line
 */  
 ...
 * NAME: begtrunc
 *                                                                    
 * FUNCTION: truncate characters from beginning of line
 */  
begtrunc(n)		    /* Truncate characters from beginning of line. */
int	n;			        /* Number of chars to truncate.    */
{
	int i;
	int chlen;
	char *chtmp, 			/* walking pointer	           */
	...
	chptr = plast;			/* be sure this is initialized	   */
	chtmp = pfirst;
	for (i=0; *chtmp && chtmp<plast; ) {	/* find string length */
		if ((chlen=mblen(chtmp, mb_cur_max)) < 1)
			truncfail();
		chtmp += chlen;
		if (++i==n)
	...
	if (i>effll) {
		if (!n) {
			n = i - effll;
			chptr = pfirst;
			for (i=0; i<n && chptr <= plast; i++)
				chptr += mblen(chptr, mb_cur_max);
			}
		pfirst = chptr;	
		if (pfirst >= plast)
			*(pfirst = plast = &work[NCOLS]) = '\n';
...
int	n;			/* Number of characters to truncate.	*/
{
	...
	i--;		/* don't count the newline: Defect 40620 */

	if (i>effll) {
		n = n? i-n : effll;
		chtmp = pfirst;

		for(i=0; i<n && chtmp<=plast; i++) 
			chtmp += mblen(chtmp, mb_cur_max);
		plast = chtmp;
 ...
 * NAME: inputtabs
 *                                                                    
 * FUNCTION: expand according to input tab specifications
 */  
...
int	p;			/* Pointer to tab specification.	*/
{
	...
	int	n;		/* Temp # of bytes in MB char		*/
	...
	while (pfirst <= plast) {
		if (logcol >= *tabs) tabs++;
		if ((n=mblen(pfirst, mb_cur_max))>1) {   /* if mb char */
			logcol += (len = mbswidth(pfirst,n)) == -1 ? 1 : len;
			while (n--)
				*tlast++=*pfirst++;
			}
		else
 ...
 * NAME: sstrip
 *                                                                    
 * FUNCTION: Add SCCS SID (generated by a "get -m" command) to the end of 
 *           each line.  Sequence is as follows for EACH line:
...
{
	int k, l;
	char *c, *pp; 	/* pp will point to the tab or the 8th char;*/

	k = 0;
	c = pp = pfirst;
	while(*c != '\t' && *c != '\n') {
		if (++k <= 8) pp = c;
		if ((l=mblen(c,mb_cur_max))>0)
			c += l;
		else
			c++;
	}
		/* Assert: k   = number of characters up to but not including
		 *                the first \t or \n.
		 *         pp points to (last character saved, up to 8) or
		 *            (first character not saved, \t or \n).
		 */
	if(*c != '\t'){fprintf(stderr,MSGSTR(M_FORMT, "not -s format\r\n")); exit(1);}
		/* Assert: pp points to first character not saved. */
	if (k<=8 && (l=mblen(pp++,mb_cur_max))>1)
		pp += (l-1);

	savek = pp - pfirst;  		/* Tab or the 8th char not saved */
	strncpy(savchr, pfirst, savek);
	if (k > 8){
	 	*(savchr+savek) = '*';
		savek++;
	}
 ...
 * NAME: sadd
 *                                                                    
 * FUNCTION: added save characters to the end of the line
 */  
sadd()
{
	strncpy(plast, savchr, savek); 
	plast = plast + savek;
	*plast = '\n'; 
}
 ...
 * NAME: outputtabs
 *                                                                    
 * FUNCTION: contract according to output tab specifications
 */  
...
int	p;			/* Pointer to tab specification.	*/
{
	...
	while (pfirst <= plast) {
		...
		if (*pfirst != '\b') {
		        if ((n=mblen(pfirst, mb_cur_max))>1) {
				logcol += (len = mbswidth(pfirst,n)) == -1 ? 1 : len;
				while (n--)
					*tlast++=*pfirst++;
				}
