static char sccsid[] = "@(#)60  1.8  src/bos/usr/bin/mail/list.c, cmdmailx, bos411, 9428A410j 11/17/93 10:40:03";
/* 
 * COMPONENT_NAME: CMDMAILX list.c
 * 
 * FUNCTIONS: MSGSTR, check, evalcol, first, getmsglist, getrawlist, 
 *            mark, markall, matchsubj, metamess, ordgetmsglist, 
 *            regret, scan, scaninit, sender, unmark 
 *
 * ORIGINS: 10  26  27  71
 *
 ...
 * store the numbers into vector while keeping the original
 * order.
 *
 * Returns the count of messages picked up or -1 on error.
 */

ordgetmsglist(buf, vector, flags)
	char *buf;
	int *vector;
{
	register int *ip;
	...
	while (*tmp_buf != '\0') {
		...
		if (*tmp_buf == '-') {
			...
			sscanf(tmp_buf, "%d", ip);
			if ((is_range) && (*ip - last_msg > 1)) {
				cur_msg = *ip;
				last_msg++;
				for (;last_msg < cur_msg; last_msg++, ip++)
					*ip = last_msg;
				*ip = cur_msg;
				is_range = FALSE;
			}
			last_msg = *ip;
			ip++;
			while (*tmp_buf != '\0' && isdigit(*tmp_buf))
				tmp_buf++;
		}
	}
	*ip = (int)NULL;
	...
	char *str;
{
	...
	while (*cp2) {
		if (*cp == 0)
			return(1);
		if (raise_alpha(*cp++) != raise_alpha(*cp2++)) {
			cp2 = ++backup;
			cp = str;
		}
	}
