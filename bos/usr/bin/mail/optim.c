static char sccsid[] = "@(#)64  1.8  src/bos/usr/bin/mail/optim.c, cmdmailx, bos411, 9428A410j 3/28/94 11:29:50";
/* 
	...
	char machine[];
{
	...
	*cp2 = 0;
	...
	for (np = netmach; np->nt_mid != 0; np++)
		if (strcasecmp(np->nt_machine, nbuf) == 0)
			return(np->nt_mid);
