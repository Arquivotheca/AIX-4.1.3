static char sccsid[] = "@(#)63  1.8  src/bos/usr/bin/mail/names.c, cmdmailx, bos411, 9428A410j 11/17/93 10:43:02";
/* 
 ...
 *            unpack, usermap, verify, yankword, localcmp
 *
 * ORIGINS: 10  26  27  71
 *
	...
	struct name *np;
{
	...
	if (hflag)
		extra += 2;
	if ((top = (char **) calloc((t + extra), sizeof cp)) == NULL)
	...
	*ap++ = "-i";
	...
	if (verbose)
		*ap++ = "-v";
	*ap++ = "-x";	/* indicate possible extended characters */
#endif SENDMAIL
	if (hflag) {
		*ap++ = "-h";
	...
	struct name *names;
{
	...
	struct name *x;
	int nstrcmp(), localcmp();
	
	int (* addrcmp)() = icequal;

	if (value("allnet") != NOSTR)
		addrcmp = localcmp;
	...
	while (np != NIL) {
		t = np;
		while (t->n_flink!=NIL &&
		    (*addrcmp)(np->n_name,t->n_flink->n_name))
			t = t->n_flink;
		if (t == np || t == NIL) {
			np = np->n_flink;
			continue;
...
/*
 * Compare two addresses, ignoring case, up to the first '@'.
 * This is to implement the allnet variable.
 */
localcmp(s1, s2)
	register char *s1, *s2;
{
	while (raise_alpha(*s1++) == raise_alpha(*s2))
		if (*s2++ == '\0' || *s2 == '@')
			return(1);
	return(0);
}
