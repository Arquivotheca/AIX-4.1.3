static char sccsid[] = "@(#)92	1.12  src/bos/usr/bin/uucp/expfile.c, cmduucp, bos411, 9428A410j 10/11/93 15:02:42";
/* 
 * COMPONENT_NAME: CMDUUCP expfile.c
 * 
...
register int	mask;
{
	...
	/*
	 * Create directory with appropriate permissions and with
	 * owner and group equal to the effective uid and the
	 * effective gid, respectively.  Note if this fails, an 
	 * attempt to restore the parent directory information is
	 * made before returning.  
	 */
	umaskOld = umask(0);
	if (mkdir(name, (mask ^ 0777) & 0777) != 0) { 
		errent(MSGSTR(MSG_UDEFS_4, "CANNOT CREATE"), name, ret, 
	...
	if ((statbuf.st_mode & 0777) != 0777) {
	  ...
	  /* osf note: restored to original setting to support XPG3 */
	    if (chmod(parent, statbuf.st_mode & 0777) != 0) {
		errent(MSGSTR(MSG_UDEFS_9, "CANNOT CHMOD"), parent, 
			errno, sccsid, __FILE__, __LINE__);
