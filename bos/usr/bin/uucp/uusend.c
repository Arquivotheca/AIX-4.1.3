static char sccsid[] = "@(#)95	1.14  src/bos/usr/bin/uucp/uusend.c, cmduucp, bos41J, 9521B_all 5/26/95 11:43:48";
/* 
 * COMPONENT_NAME: CMDUUCP uusend.c
 * 
...
char	*UULIB = "/usr/sbin/uucp";	  /* UUCP lib directory */
...
#ifdef	RUUSEND
		sprintf(cmdbuf,"uux -e -gn -z %s- \"%s!ruusend %s -m %o - (%s)\"",
#else !RUUSEND
		sprintf(cmdbuf, "uux -e -gn -z %s- \"%s!uusend %s -m %o - (%s)\"",
#endif !RUUSEND
...
fflush(dout);
	...
	fclose(in);
	if (excl)   /* if we did popen, exit with the exit status of pclose */
                exit((pclose(out) >> 8) & 0xff);
	exit(0);
}
