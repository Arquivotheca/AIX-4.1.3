static char sccsid[] = "@(#)84	1.1  src/bos/usr/bin/uucp/uucpadm/sysfiles.c, cmduucp, bos411, 9428A410j 8/3/93 16:04:29";
/* 
 * COMPONENT_NAME: CMDUUCP sysfiles.c
 * 
 * FUNCTIONS: setservice, sysaccess, scansys, scancfg, getline,
 *	      namematch, tokenize, nameparse, setfile, sysreset,
 *	      devreset, dialreset, getsysline, nextsystems, getdevline,
 *	      nextdevices, getdialline, nextdialers
 *
 * ORIGINS: 10  27  3 
 *
...
/* private variables */
static void tokenize(), nameparse(), setfile(), scansys();
static int namematch(), nextdialers(), nextsystems(), getline();

/* pointer arrays might be dynamically allocated */
char *fSystems[64] = {0};	/* list of Systems files */
char *fDevices[64] = {0};	/* list of Devices files */
char *fDialers[64] = {0};	/* list of Dialers files */
...
/* export these */
extern void sysreset(), devreset(), dialreset(), setservice();
extern char *strsave();
	...
	int type;
{
	...
	case ACCESS_SYSTEMS:
		return(access(fSystems[nsystems], R_ACC));
	case ACCESS_DEVICES:
		return(access(fDevices[ndevices], R_ACC));
	case ACCESS_DIALERS:
		return(access(fDialers[ndialers], R_ACC));
	case EACCESS_SYSTEMS:
		return(eaccess(fSystems[nsystems], R_ACC));
	case EACCESS_DEVICES:
		return(eaccess(fDevices[ndevices], R_ACC));
	case EACCESS_DIALERS:
		return(eaccess(fDialers[ndialers], R_ACC));
	default:
		(void)sprintf(errformat, "bad access type %d", type);
		fprintf(stderr, errformat, "sysaccess");
		return(FAIL);
	}
}
...
{	FILE *f;
	...
	fSystems[0] = fDevices[0] = fDialers[0] = NULL;
	...
	if (fSystems[0] == NULL) {
		fSystems[0] = strsave(SYSFILE);
		fSystems[1] = NULL;
	}
	if (fDevices[0] == NULL) {
		fDevices[0] = strsave(DEVFILE);
		fDevices[1] = NULL;
	}
	if (fDialers[0] == NULL) {
		fDialers[0] = strsave(DIALERFILE);
		fDialers[1] = NULL;
	}
	return;
}
...
nextsystems()
{
	...
	for ( ; fSystems[nsystems] != NULL; nsystems++)
		if ((fsystems = fopen(fSystems[nsystems], "r")) != NULL)
			return(TRUE);
	return(FALSE);
}
...
nextdevices()
{
	...
	for ( ; fDevices[ndevices] != NULL; ndevices++)
		if ((fdevices = fopen(fDevices[ndevices], "r")) != NULL)
			return(TRUE);
	return(FALSE);
}
...
nextdialers()
{
	...
	for ( ; fDialers[ndialers] != NULL; ndialers++)
		if ((fdialers = fopen(fDialers[ndialers], "r")) != NULL)
			return(TRUE);
	return(FALSE);
}
