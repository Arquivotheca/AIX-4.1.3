static char sccsid[] = "@(#)24	1.17  src/bos/usr/bin/uucp/uucleanup.c, cmduucp, bos411, 9428A410j 11/11/93 15:07:42";
/* 
 * COMPONENT_NAME: CMDUUCP uucleanup.c
 * 
...
#define USAGE	"[-oDAYS] [-mSTRING] [-Cdays] [-Ddays] [-Tdays] [-Wdays] [-Xdays] [-xLEVEL] [-sSYSTEM]"
...
mode_t omask;			/* It's just here to satisfy the linker */
...
char **envp;
{
	...
	while ((i = getopt(argc, argv, "C:D:T:W:X:m:o:s:x:")) != EOF) {
		...
		case 'x':
			Debug = atoi(optarg);
                        if (Debug <= 0 ) {
                                fprintf(stderr, MSGSTR(MSG_UUCLEAN12,
                                "WARNING: %s: invalid debug level %s ignored, using level %s\n"),
                                Progname, optarg, "1");
                                Debug = 1;
			}
                        else if (Debug >= 10) {
		...
		case 'm':
			...
			break;
		default:
			(void) fprintf(stderr, MSGSTR(MSG_UCL_11,
			    "\tusage: %s %s\n"), Progname, 
			    MSGSTR(MSG_UCL_1,USAGE));
			exit(1);
	...
	if (argc != optind) {
		(void) fprintf(stderr, MSGSTR(MSG_UCL_11,
		   "\tusage: %s %s\n"), Progname, MSGSTR(MSG_UCL_1, USAGE));
		exit(1);
	}
...
char *mtext[];
{
	...
	if (system != NULL && *system != '\0') {
		(void) sprintf(cmd, "%s mail %s!", PATH, system);
	}
	else {
		(void) sprintf(cmd, "%s mail ", PATH);
	}
