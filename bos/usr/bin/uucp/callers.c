static char sccsid[] = "@(#)80	1.12  src/bos/usr/bin/uucp/callers.c, cmduucp, bos411, 9428A410j 3/10/94 10:03:50";
/* 
 * COMPONENT_NAME: CMDUUCP callers.c
 * 
...
#if defined(BSD4_2) || defined(AIX_TCP)
#include <netdb.h>
...
static void translate(register char *, register char *);
static gdial(register char *, register char **, register int);
...
/*	set up default "break" routine */
	...
	for (ca = Caller; ca->CA_type != NULL; ca++) {
		...
		if (EQUALS(ca->CA_type, dev[D_CALLER])) {
			...
			if (dev[D_ARG] == NULL) {
				...
				dev[D_ARG+1] = NULL;  /* needed for loop later
							to mark the end */
					dev[D_ARG] = "\\T";
			}
	...
	if (dcf == -1) {
		...
		if ( dcf < 0)
			goto bad;
                ...
                if ( Modemctrl ) {
			/* Turn DCD control off until we are connected */
	 ...
	 * Success at last!
	 */
	/* Restore DCD if we turned it off. */
	if (!Modemctrl)
...
tcflag_t c_cflag;
{
	...
	if ((c_cflag & CBAUD) != 0)
		ttbuf.c_cflag = (tcflag_t)c_cflag;
...
gdial(register char *type, register char *arps[], register int narps)
{
	...
	while (getdialline(info, sizeof(info))) {
		if ((info[0] == '#') || (info[0] == ' ') ||
		    (info[0] == '\t') || (info[0] == '\n'))
			continue;
		if ((na = get_args(info, arps, narps)) == 0)
			continue;
...
/* STANDALONE is defined which compiling as "scallers.c" */
#ifndef STANDALONE
