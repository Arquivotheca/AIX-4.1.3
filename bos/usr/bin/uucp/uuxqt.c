static char sccsid[] = "@(#)43	1.18  src/bos/usr/bin/uucp/uuxqt.c, cmduucp, bos41J, 9515A_all 4/11/95 16:12:30";
/* 
 * COMPONENT_NAME: CMDUUCP uuxqt.c
 * 
...
void	onintr(int);
static xprocess(char *);
static  char    shchar[] = "`';&|^<>(){}\t\n\\\" ";
extern void close_log();
mode_t omask;
int    expand=0;       /* Allow pathname expansion if expand=1 */
...
char	*argv[], *envp[];
{
	...
	(void) signal(SIGDANGER, (void(*)(int)) onintr);
	...
	Ifn = 0;
	dirname[0] = NULLCHAR;
	while ((ret = getopt(argc, argv, "es:x:")) != EOF) {
		switch(ret){
	...
	fp1 = opendir(Spool);
	ASSERT(fp1 != NULL, MSGSTR(MSG_UDEFS_1, "CANNOT OPEN"), 
		Spool, errno);
	...
	if ( strpbrk(dirname, shchar) != NULL  )
					    /* ignore naughty name */
	    {
		    ...
		    errent("BAD REMOTE NAME", dirname, 0, sccsid, 
			    __FILE__, __LINE__);
		    closedir(fp1);
		    cleanup(101);
	    }
...
unmv_Xfiles()
{
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
		...
		/* i know we're in .Xqtdir, but what the heck ... */
		(void) sprintf(tfull, "%s/%s", XQTDIR, tfile);
...
xprocess(char *dirname)
{
	...
	ASSERT(xfp != NULL, 
		MSGSTR(MSG_UDEFS_1, "CANNOT OPEN"), _Xfile, errno);
	...
	*incmd = *retaddr = NULLCHAR;
	...
	/* Check if we're to use a data file for stdin */
	if (!strcmp(incmd, " ") && strcmp(fin, "/dev/null")) {
		if (accessx(fin, R_OK, ACC_SELF) != 0){
			/* 
			 * Force a command failure.
			 */
			strcpy(incmd, "cannot_access ");
			strcat(incmd, fin);
		}
		else {  /* Copy stdinput file into cmd string for checking */
			infp = fopen(fin, "r");
			if (infp != NULL) {
...
*/
        ...
        /*
         * Add "set -f" to turn off shell preprocessing.  Ugly, but bsh has
         * a bug where "-f -c" does not work...  This is for Posix X/Open
         * compliance for uux.  But, for binary compatibility, if the user
         * has selected old behavior to expand shell patterns, such as *
         * (asterisk), then do not add it.
         */
        if (!expand) {
                (void) sprintf(_Cmd,
                "%s UU_MACHINE=%s UU_USER=%s export UU_MACHINE UU_USER PATH; set -f; ",
                PATH, Rmtname, user);
	...
	ASSERT(chdir(XQTDIR) == 0, MSGSTR(MSG_UDEFS_11, "CANNOT CHDIR"), 
		XQTDIR, errno);
	...
	if (ret != 0) {
	    ...
	    dfp = fopen(dfile, "a");
	    ASSERT(dfp != NULL, MSGSTR(MSG_UDEFS_1, "CANNOT OPEN"), 
			dfile, errno);
...
rmfiles:
	...
	xfp = fopen(_Xfile, "r");
	ASSERT(xfp != NULL, MSGSTR(MSG_UDEFS_1, "CANNOT OPEN"), 
		_Xfile, errno);
	while (fgets(buf, BUFSIZ, xfp) != NULL) {
	    if (buf[0] != X_RQDFILE)
		continue;
...
char *prefix, *dir;
{
	...
	pdir = opendir(dir);
	ASSERT(pdir != NULL, MSGSTR(MSG_UDEFS_1, "CANNOT OPEN"), dir, errno);
