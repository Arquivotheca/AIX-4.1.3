#ifndef UUCHECK
static char sccsid[] = "@(#)08	1.13.1.2  src/bos/usr/bin/uucp/permission.c, cmduucp, bos411, 9428A410j 3/17/94 16:29:45";
#endif
/* 
 * COMPONENT_NAME: CMDUUCP permission.c
 * 
...
static userFind(char *, char *, int);
static int validateFind(char *);
static cmdMatch(register char *, char *);
 ...
 */
static
userFind(char *name, char *rmtname, int type)
{
	char *p, *arg;
 ...
 */
static
cmdMatch(register char *name, char *fullname)
{
	register i;
	char *bname;
	int allok = FALSE;
...
char *from, *to, *opt;
{
	...
	/* osf note: chkacc() is AIX specific added to  support XPG3 */
	...
	*lxp = NULLCHAR;
	...
	/* osf note: chkacc() is aix specific added to support XPG3 */
	if (EQUALS(RemSpool, dir) || chkacc(dir, 2) == 0)
	    return(0);
 ...
 * osf note: chkacc() is aix specific added to support XPG3 
 *
 * chkacc - replaces WRITEANY() macro with checks on ACLs
 *
 * mode is the stat modes for other access (e.g. 0xx7)
 */
...
chkacc(path, mode)
	...
	int mode;
{
	struct acl *p = (struct acl *)0;
	int retcode = FAIL;

	errno = 0;
	if (p = (struct acl *)malloc(BUFSIZ))
		if (statacl(path, STX_NORMAL, p, BUFSIZ) == 0)
			if ((p->o_access & mode) == mode)
				retcode = 0;
	if (p) (void)free(p);
	return retcode;
}
