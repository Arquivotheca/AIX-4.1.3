static char sccsid[] = "@(#)83	1.15  src/bos/usr/bin/uucp/cntrl.c, cmduucp, bos411, 9428A410j 3/27/94 12:12:59";
/* 
 * COMPONENT_NAME: CMDUUCP cntrl.c
 * 
...
static stmesg(char *, char *), nospace(char *);
		...
		if (wrktype == RCVFILE) {
			...
			if ( ((fp = fopen(Dfile, "w")) == NULL)
				...
				CDEBUG(1, MSGSTR(MSG_CNTRLCD3, 
					"Failed: No Space!\n"), 0);
				unlinkdf(Dfile);
				assert(MSGSTR(MSG_UDEFS_4,"CANNOT CREATE"),
                                       Dfile, nospace(Dfile),
...
process:
	...
	case HUP:
		...
		if (msg[1] == 'N') {
			ASSERT(role == MASTER, 
				MSGSTR(MSG_UDEFS_13,"WRONG ROLE"), "", role);
			role = SLAVE;
			goto top;
		}
	...
	case SNDFILE:
		...
		if (msg[1] == 'Y') {
			...
			/* give an extra try for each kilobyte    */
	...
	case RCVFILE:
		 ...
		 */
		ASSERT(role == SLAVE, MSGSTR(MSG_UDEFS_13, "WRONG ROLE"),
			 "", role);
		...
		ASSERT(fstat(fileno(fp), &stbuf) == 0, 
			MSGSTR(MSG_UDEFS_7, "CANNOT STAT"), filename, errno);
...
register int role;
{
	...
	if (role == MASTER) {
		RMESG(SLTPTCL, msg);
		if ((str[0] = fptcl(&msg[1])) == (char)NULL) {
