static char sccsid[] = "@(#)05	1.1  src/bos/usr/bin/uucp/sysfiles.c, cmduucp, bos411, 9428A410j 6/17/93 13:26:02";
/* 
 * COMPONENT_NAME: CMDUUCP sysfiles.c
 * 
 * FUNCTIONS: setservice, setdevcfg, sysaccess, scansys, scancfg, getline,
 *	      namematch, tokenize, nameparse, setfile, setioctl, sysreset,
 *	      devreset, dialreset, getsysline, nextsystems, getdevline,
 *	      nextdevices, getdialline, nextdialers, getpop, getpush
 *
 * ORIGINS: 10  27  3 
 *
	...
	int type;
{
	...
	case ACCESS_SYSTEMS:
		return(access(Systems[nsystems], R_ACC));
	case ACCESS_DEVICES:
		return(access(Devices[ndevices], R_ACC));
	case ACCESS_DIALERS:
		return(access(Dialers[ndialers], R_ACC));
	case EACCESS_SYSTEMS:
		return(eaccess(Systems[nsystems], R_ACC));
	case EACCESS_DEVICES:
		return(eaccess(Devices[ndevices], R_ACC));
	case EACCESS_DIALERS:
		return(eaccess(Dialers[ndialers], R_ACC));
	default:
...
{	FILE *f;
	...
	if (Systems[0] == NULL) {
		Systems[0] = strsave(SYSFILE);
		ASSERT(Systems[0] != NULL, 
			MSGSTR(MSG_UDEFS_5, "CANNOT ALLOCATE"), 
			"scansys: Systems", Systems[0]);
		Systems[1] = NULL;
	}
	if (Devices[0] == NULL) {
		Devices[0] = strsave(DEVFILE);
		ASSERT(Devices[0] != NULL, 
			MSGSTR(MSG_UDEFS_5, "CANNOT ALLOCATE"), 
			"scansys: Devices", Devices[0]);
		Devices[1] = NULL;
	}
	if (Dialers[0] == NULL) {
		Dialers[0] = strsave(DIALERFILE);
		ASSERT(Dialers[0] != NULL, 
			MSGSTR(MSG_UDEFS_5, "CANNOT ALLOCATE"), 
			"scansys: Dialers", Dialers[0]);
		Dialers[1] = NULL;
	}
	return;
}
...
{	char **tptr, *tok;
	...
	tok = strtok((char *) NULL, ":")) {
		...
		if (eaccess(expandpath, 4) != 0) {
			...
			msgptr = &errmsg[0];
			strcpy(msgptr, MSGSTR(MSG_UDEFS_1, "CANNOT OPEN"));
			strcat(msgptr, " "); strcat(msgptr, expandpath);
			DEBUG(1, "%s\n", msgptr);
			continue;
		}
