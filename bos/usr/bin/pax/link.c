static char sccsid[] = "@(#)69	1.6  src/bos/usr/bin/pax/link.c, cmdarch, bos412, 9446B 11/11/94 22:06:29";
/*
...
linkmatch(Link *linkptr, Stat *asb)
{
	if ((ar_format == TAR) && (f_extract || f_list))
		return((strcmp(linkname,linkptr->l_name) == 0));
	else
...

{
    ...
    if (f_extract && rplhead != (Replstr *)NULL) {
	strcpy(namebuf, name);
	name = namebuf;
	rpl_name(name);
	if (strlen(name) == 0) {
