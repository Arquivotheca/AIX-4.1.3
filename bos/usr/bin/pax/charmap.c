static char sccsid[] = "@(#)60	1.5  src/bos/usr/bin/pax/charmap.c, cmdarch, bos412, 9446B 11/11/94 21:53:59";
/*
...
static char	 	escape_char='\\'; 	/* escape charater (default '\') */
static int		mbmaxlen = 1;		/* max bytes in multi-byte */
static int		mbminlen = 1;		/* min bytes in multi-byte */
static Charentry	*char_head=NULL;	/* head of Charentry list */
static Charentry	*char_tail=NULL;	/* tail of Charentry list */
static int		entry_count=0;		/* number of entries in charmap */
static Charentry	*charmap;		/* The final sorted charmap table */
static Charentry	search_key;		/* Seach key for bsearch */
...
static int		sym_compare(const Charentry *, const Charentry *);
static int		bval_compare(const Charentry *, const Charentry *);
static int 		byte_match(const Charentry *k1, const Charentry *k2);
...

{
	...
	*bindex = '\0';
	...
	for (bindex = buf; *bindex != '\0'; bindex++) {
		...
		if ((nindex-name) + strlen(sym) > PATH_MAX) {
		    warn(orig_name, MSGSTR(CM_LONG, "Filename becomes too long"));
		    free(orig_name);
		    return(-1);
		}
 ...
 * 	of the entryies, which it sorts.
 * 
...
byte_match(const Charentry *k1, const Charentry *k2)
{
    ...
    return(strncmp((char *)k1->value, (char *)k2->value, (size_t)(item->nbytes)));
}
...
bval_compare(const Charentry *x1, const Charentry *x2)
{
	...
	if (x1->nbytes > x2->nbytes)
		...
		return(strncmp((char *)x1->value, (char *)x2->value, (size_t)(x1->nbytes)));
}
