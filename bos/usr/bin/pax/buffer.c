static char sccsid[] = "@(#)59	1.10  src/bos/usr/bin/pax/buffer.c, cmdarch, bos411, 9428A410j 1/13/94 13:14:51";
/*
...

{
    ...
    /*
     * Only return if openout() was unsuccessful, or if the archive entry
     * is a directory.  Directory modes and ownership will be restored
     * later.  NOTE: a return value of 0 from openout() means that no data
     * is required, so not calling buf_skip() is ok.
     */
    } else if ((ofd < 0) || (S_ISDIR(asb->sb_mode))) {
	 ...
	 */
	if (lchown(name, asb->sb_uid, asb->sb_gid) < 0)
...

{
	...
	if (!done) {
	    got = read(fd, buf, (unsigned int) chunk);
		...
		/*
		 * In datastream mode, early EOF is OK.
		 */
		if (!f_datastream)
		...
		/* Pad buffer with nulls once done */
	if (done) {
	    bzero(buf, chunk);
	    got = chunk;
	}

		/* Use (write) data and decrement 'size' */
	buf_use(got);
	size -= got;
    }
    if (!f_datastream) {
...
/* eotoutflush - flush the output buffer at end of archive.
 *
 * DESCRIPTION
 *
 *	The output buffer is written, if there is anything in it, to the
 *	archive file padded to the blocking size of the format.
 */
...

{
    ...
    buf = bufstart;
    while ((bufidx - buf) > 0) {
	if ((got = write(archivefd, buf, (blocksize - (buf - bufstart)))) > 0) {
	    buf += got;
	} else if ((got < 0) && (errno == ENXIO || errno == ENOSPC)) {
	    next(AR_WRITE);
	} else {
