static char sccsid[] = "@(#)33	1.11  src/bos/usr/bin/mh/sbr/addrsbr.c, cmdmh, bos411, 9428A410j 8/3/92 14:49:01";
/* 
 * COMPONENT_NAME: CMDMH addrsbr.c
 * 
 * FUNCTIONS: MSGSTR, UucpChan, adrsprintf, auxformat, getm, getname, 
 *            ismymbox, mnfree 
 *
 ...
 */
/* static char sccsid[] = "addrsbr.c	8.1 88/04/15 15:49:46"; */
...
#if defined(X400) /* this is probably a win.  It has nothing to do with X400 */
...
/* 	    *nxtout++ = isupper (c) ? tolower (c) : c; */
	...
	case COM+'\\':
	    if ((c = *addrs++) == '\n' || c == '\0') {
		state = EOA;
		err = MSGSTR(ILLEGAL, "illegal \\"); /*MSG*/
	    }
	    *adrcopy++ = c;
#if defined(X400)
	    *nxtout++ = c;
	...
	case ERR+'\0':
	    state = EOA;
	    if (lbcnt)
		err = MSGSTR(MISSING, "missing >"); /*MSG*/
	    break;

	case COM+'\0':
	    state = EOA;
	    err = MSGSTR(MISSING2, "missing )"); /*MSG*/
	    if (nxtout == adr)
		nxtout++;
	    break;
	...
	case QS+'\0':
	    state = EOA;
	    err = MSGSTR(MISSING3, "missing \""); /*MSG*/
	    break;
	}
	if (c != '\0')
	    c = *addrs++;
...
#if defined(X400) /* just a good idea, not X400 related */
	...
	wanthost;
{
	...
	if (eresult)
	    ...
	    if (wanthost == AD_HOST) {
		strcpy(hlds, err);
		admonish (NULLCP, MSGSTR(BADADDR, "bad address '%s' - %s"), str, hlds); /*MSG*/
	    }
	return NULL;
    }
#ifdef	BERK
...
#endif	not BERK
	if (eresult)
	    (void) strcpy (eresult, MSGSTR(NULLADR, "null address")); /*MSG*/
	else
	    if (wanthost == AD_HOST)
		admonish (NULLCP, MSGSTR(NULLADR2, "null address '%s'"), str); /*MSG*/
	return NULL;
    }
#ifndef	BERK
    if (mbox == NULL && grp == NULL) {
	if (eresult)
	    (void) strcpy (eresult, MSGSTR(NOMBOX, "no mailbox in address")); /*MSG*/
	else
	    if (wanthost == AD_HOST)
		admonish (NULLCP, MSGSTR(NOMBOX2, "no mailbox in address '%s'"), str); /*MSG*/
	return NULL;
    }
    ...
    if (mp == NULL) {
	if (eresult)
	   (void) strcpy (eresult, MSGSTR(NOMEM, "insufficient memory to represent address")); /*MSG*/
	else
	    if (wanthost == AD_HOST)
		adios (NULLCP, MSGSTR(NOMEM, "insufficient memory to represent address")); /*MSG*/
	return NULL;
    }

#ifdef	BERK
    mp -> m_type = LOCALHOST;
    if (!index (adr, '@') && !index (adr, '!') && !strstr(str,"/C=") && !strstr(str,"/c="))  {
	(void) strcat(str,hold);
	(void) strcat(adr,hold);
	mp -> m_nohost = 1;
	}
...
register struct mailname *np;
{
    ...
    register struct mailname   *mp;
    static char *am = NULL;
#if defined(X400) /* just a good idea, not realy X400 */
	...
	if ((am = m_find ("alternate-mailboxes")) == NULL)
		...
		if ((mp -> m_next = getm (cp, NULLCP, 0, AD_NAME, NULLCP))
			== NULL)
		    admonish (NULLCP, MSGSTR(ILLADDR, "illegal address: %s"), cp), oops++; /*MSG*/
		else {
		    mp = mp -> m_next;
		    mp -> m_type = W_NIL;
#ifdef	BERK
	    ...
	    if (oops) {
		strcpy (hlds, MSGSTR(FIX, "please fix the %s: entry in your %s file")); /*MSG*/
		advise (NULLCP, hlds, "alternate-mailboxes", mh_profile);
	    }		
	}
    }
    if (np == NULL) /* XXX */
    ...
    strcpy(xname,SystemName());
    if (strlen(xname) > 0)
	    if ((strcmp (cp, mq.m_mbox) == 0) || 
		((strncmp (mq.m_mbox,cp,strlen(mq.m_mbox)) == 0) && 
		(strstr(cp,xname))))
		return 1;
    else
	    if ((strcmp (cp, mq.m_mbox) == 0))
...
#if defined(X400) /* we think this is a bugfix.  It isn't an X400ism */
