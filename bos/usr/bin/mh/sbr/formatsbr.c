static char sccsid[] = "@(#)55	1.10  src/bos/usr/bin/mh/sbr/formatsbr.c, cmdmh, bos411, 9428A410j 2/1/93 16:50:39";
/* 
 * COMPONENT_NAME: CMDMH formatsbr.c
 * 
 * FUNCTIONS: MSGSTR, PUTD, PUTDF, PUTS, PUTSF, fmtscan, match, 
 *            new_fs, normalize 
 *
 ...
 */
/* static char sccsid[] = "formatsbr.c	8.1 88/04/15 15:49:56"; */
               ...
               *def;
{
	...
	if ((fp = fopen (libpath (form), "r")) == NULL)
	    adios (form, MSGSTR(NOOPENFF, "unable to open format file %s"), form); /*MSG*/

	if (fstat (fileno (fp), &st) == NOTOK)
	    adios (form, MSGSTR(NOSTATFF, "unable to stat format file %s"), form); /*MSG*/

	if ((formats = malloc ((unsigned) st.st_size + 1)) == NULLCP)
	    adios (form, MSGSTR(NOFSP, "unable to allocate space for format %s"), form); /*MSG*/

	if (read (fileno(fp), formats, st.st_size) != st.st_size)
	    adios (form, MSGSTR(NORFF, "error reading format file %s"), form); /*MSG*/
...
register char  *cp;
{
	...
	if (*cp != QUOTE)
	    *dp++ = *cp;
	    ...
	    switch (*++cp) {
/* The following needs a fix to cpp to work.  Until then, we'll spell it
   out below. P11584. 
	...
	int	dat[];
{
	...
	case FT_S_NONNULL:
	    value = (str != NULLCP && *str != (char)NULL);
	    break;
    ...
    /* make sure we don't split a multi-byte character with the newline */
    for (i = 0, sp = scanl; sp < cp; ++sp)
	if (i)
	    i = 0;
	else if (NCisshift(*sp))
	    i = 1;
    if (i)  /* last char in string is first char of multi-byte */
	--cp;
