static char sccsid[] = "@(#)09  1.4  src/bos/usr/bin/mh/zotnet/mf.c, cmdmh, bos411, 9428A410j 10/11/90 11:09:15";
/* 
 * COMPONENT_NAME: CMDMH mf.c
 * 
 * FUNCTIONS: add, compress, domain, getadrx, getcpy, isat, isfrom, 
 *            legal_person, lequal, local_part, mfgets, my_lex, 
 *            parse_address, phrase, route, route_addr, seekadrx, 
 *            uucpadrx 
 *
 * ORIGINS: 26  28  35 
 *
...
static int phrase();
static int route_addr();
static int local_part();
static int my_lex();
static int compress();
static int domain();
static int route();
...
/*  */
    ...
    *bp++ = c;
    *bp = (int)NULL;
    for (i = 0; special[i].lx_chr != (char)NULL; i++)
