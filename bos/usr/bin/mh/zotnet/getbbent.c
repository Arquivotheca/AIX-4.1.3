static char sccsid[] = "@(#)06  1.6  src/bos/usr/bin/mh/zotnet/getbbent.c, cmdmh, bos411, 9428A410j 10/11/90 10:54:51";
/* 
 * COMPONENT_NAME: CMDMH getbbent.c
 * 
 * FUNCTIONS: BBread, bblose, bbskip, endbbent, getbbaka, getbbcpy, 
 *            getbbdist, getbbent, getbberr, getbbitem, getbbnam, 
 *            getbbtime, getcpy, ldrbb, ldrchk, make_lower, setbbaux, 
 *            setbbent, setbbfile, setbbinfo, setpwaux, setpwinfo 
 *
 * ORIGINS: 26  28  35 
 *
 */
...
char   *crypt (), *getpass ();
/* The following are declared in pwd.h and grp.h */
...
static int  BBread () {
	...
	if (*bb -> bb_addr == (char)NULL)
	    bb -> bb_addr = bb -> bb_name;

    if (*bb -> bb_file == (char)NULL)
	return;
    if (*bb -> bb_file != '/') {
    ...
    if ((cp = rindex (bb -> bb_file, '/')) == NULL || *++cp == (char)NULL)
