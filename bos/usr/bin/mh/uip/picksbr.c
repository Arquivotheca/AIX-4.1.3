static char sccsid[] = "@(#)70  1.7  src/bos/usr/bin/mh/uip/picksbr.c, cmdmh, bos411, 9428A410j 3/28/91 14:54:39";
/* 
 * COMPONENT_NAME: CMDMH picksbr.c
 * 
 * FUNCTIONS: ANDaction, GREPaction, MSGSTR, NOTaction, ORaction, 
 *            PRaction, TWSaction, advance, args, cclass, exp1, exp2, 
 *            exp3, gcompile, newnexus, nxtarg, parse, pcompile, 
 *            pmatches, prvarg, tcompile, tws_parse, tws_special 
 *
 ...
 */
/* static char sccsid[] = "picksbr.c	7.1 87/10/13 17:32:07";*/
...
static struct	nexus *parse (), *exp1 (), *exp2 (), *exp3 (), 
	        *newnexus ( register int (*)());

static int   ORaction (), ANDaction (), NOTaction (), advance(), 
	     GREPaction (), TWSaction (), gcompile(), cclass(), tcompile();
static	PRaction (register struct nexus *, register int);
char *getenv();
	        ...
	        *date;
{
    ...
    if (*argp) {
	padvise (NULLCP, MSGSTR(UNEXP, "%s unexpected"), *argp); /*MSG*/
	return 0;
    }
...
static struct nexus *parse () {
    ...
    if (*cp != '-') {
	padvise (NULLCP, MSGSTR(UNEXP, "%s unexpected"), cp); /*MSG*/
	return NULL;
    }
	...
	case PROR: 
		...
		return o;
	    padvise (NULLCP, MSGSTR(NODIS, "missing disjunctive")); /*MSG*/
	    return NULL;
...
static struct nexus *exp1 () {
	...
	case PRAND: 
	    ...
	    if (o -> n_R_child = exp1 ())
		return o;
	    padvise (NULLCP, MSGSTR(NOCON, "missing conjunctive")); /*MSG*/
	    return NULL;
...
static struct nexus *exp2 () {
	...
	case PRNOT: 
	    ...
	    if (n -> n_L_child = exp3 ())
		return n;
	    padvise (NULLCP, MSGSTR(NONEG, "missing negation")); /*MSG*/
	    return NULL;
...
static struct nexus *exp3 () {
	...
	case PRLBR: 
	    if ((n = parse ()) == NULL) {
		padvise (NULLCP, MSGSTR(NOGRP2, "missing group")); /*MSG*/
		return NULL;
	    }
	    if ((cp = nxtarg ()) == NULL) {
		padvise (NULLCP, MSGSTR(NORBRC, "missing -rbrace")); /*MSG*/
		return NULL;
	    }
    ...
    header: ;
		...
		padvise (NULLCP, MSGSTR(NOARG, "missing argument to %s"), argp[-2]); /*MSG*/
		return NULL;
	    }
    ...
    pattern: ;
	    if (!gcompile (n, dp)) {
		padvise (NULLCP, MSGSTR(PATERR, "pattern error in %s %s"), argp[-2], cp); /*MSG*/
		return NULL;
	    }
	...
	case PROTHR: 
	    padvise (NULLCP, MSGSTR(INTERR3, "internal error!")); /*MSG*/
	    return NULL;
	...
	case PRBEFR: 
		...
		padvise (NULLCP, MSGSTR(NOPARS, "unable to parse %s %s"), argp[-2], cp); /*MSG*/
		return NULL;
	    }
	    return n;
    }
...
static struct nexus *newnexus (register int  (*action) ())
{
    register struct nexus   *p;

    if ((p = (struct nexus *) calloc ((unsigned) 1, sizeof *p)) == NULL)
	adios (NULLCP, MSGSTR(NOACSTOR, "unable to allocate component storage")); /*MSG*/
...
static	PRaction (register struct nexus *n,
		  register int	level)
{
    register int    i;
	...
	fprintf (stderr, MSGSTR(OR, "OR\n")); /*MSG*/
	PRaction (n -> n_L_child, level + 1);
	PRaction (n -> n_R_child, level + 1);
	return;
    }
    if (n -> n_action == ANDaction) {
	fprintf (stderr, MSGSTR(AND, "AND\n")); /*MSG*/
	PRaction (n -> n_L_child, level + 1);
	PRaction (n -> n_R_child, level + 1);
	return;
    }
    if (n -> n_action == NOTaction) {
	fprintf (stderr, MSGSTR(NOT, "NOT\n")); /*MSG*/
	PRaction (n -> n_L_child, level + 1);
	return;
    }
    if (n -> n_action == GREPaction) {
	strcpy (hlds, MSGSTR(PATT, "PATTERN(%s) %s\n")); /*MSG*/
	fprintf (stderr, hlds, n -> n_header ? MSGSTR(HEAD, "header") : MSGSTR(MBODY, "body"), n -> n_patbuf); /*MSG*/
	return;
    }
    if (n -> n_action == TWSaction) {
	strcpy (hlds, MSGSTR(TEMP, "TEMPORAL(%s) %s: %s\n")); /*MSG*/
	fprintf (stderr, hlds, n -> n_after ? MSGSTR(AFTER, "after") : MSGSTR(BEFORE, "before"), n -> n_datef, dasctime (&n -> n_tws, TW_NULL)); /*MSG*/
	return;
    }
    fprintf (stderr, MSGSTR(UNKNWN, "UNKNOWN(0x%x)\n"), (*n -> n_action)); /*MSG*/
}
               ...
               *aep;
{
	...
	star: 
	    ...
	    default: 
		admonish (NULLCP, MSGSTR(BOTCH, "advance() botch -- you lose big")); /*MSG*/
		return 0;
	}
}
...
plist
{
	...
	switch (state = m_getfld (state, name, buf, sizeof buf, fp)) {
		...
		if (state == LENERR || state == FMTERR)
		    advise (NULLCP, MSGSTR(FORMERR, "format error in message %d"), msgnum); /*MSG*/
		if (bp != NULL)
		    free (bp);
		return 0;
...
/*  */
	...
	advise (NULLCP, MSGSTR(NOPARS2, "unable to parse %s field in message %d, matching..."), n -> n_datef, msgnum), state = 1; /*MSG*/
    else
