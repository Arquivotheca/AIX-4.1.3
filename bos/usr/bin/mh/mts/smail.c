static char sccsid[] = "@(#)28  1.7  src/bos/usr/bin/mh/mts/smail.c, cmdmh, bos411, 9428A410j 3/27/91 14:49:26";
/* 
 * COMPONENT_NAME: CMDMH smail.c
 * 
 * FUNCTIONS: MSGSTR, alrmser, min, rclient, rp_string, sm_end, 
 *            sm_ierror, sm_init, sm_rerror, sm_rrecord, sm_wadr, 
 *            sm_waend, sm_werror, sm_winit, sm_wrecord, sm_wstream, 
 *            sm_wtend, sm_wtxt, smhear, smtalk 
 *
 * ORIGINS: 10  26  28  35 
 *
 */
...
static int sm_ierror();
static int smtalk();
static int sm_wstream();
static int sm_wrecord();
static int sm_werror();
static int smhear();
static int sm_rrecord();
		 ...
		 debug;
{
    ...
    if (pipe (pdi) == NOTOK)
	return sm_ierror (MSGSTR(NOPIPES, "no pipes")); /*MSG*/
    if (pipe (pdo) == NOTOK) {
	(void) close (pdi[0]);
	(void) close (pdi[1]);
	return sm_ierror (MSGSTR(NOPIPES, "no pipes")); /*MSG*/
	...
	case NOTOK: 
	    ...
	    (void) close (pdi[0]);
	    (void) close (pdi[1]);
	    return sm_ierror (MSGSTR(NOFORK, "unable to fork")); /*MSG*/
	...
	default: 
		...
		(void) close (pdo[1]);
		sm_rfp = sm_wfp = NULL;
		return sm_ierror (MSGSTR(NOFDOPEN, "unable to fdopen")); /*MSG*/
	    }
	    sm_alarmed = 0;
	    (void) alarm (SM_OPEN);
	    result = smhear ();
		 ...
		 debug;
{
	...
	(void) close (sd1);
	return sm_ierror (MSGSTR(NODUP, "unable to dup")); /*MSG*/
    }
...
#else	not SMTP
	strlen ((char *)strcpy (sm_reply.text, (sm_wfp == NULL ? "no pipe opened"
...
again: ;
    ...
    sm_reply.text[sm_reply.length = 0] = (int)NULL;
	...
	if (more) {
	    ...
	    more = cont;
	    if (bc <= 0) {
		(void) strcpy (bp = buffer, MSGSTR(NOREPLY, sm_noreply)); /*MSG*/
		bc = strlen (MSGSTR(NOREPLY, sm_noreply)); /*MSG*/
	    }
	}
	if ((i = min (bc, rc)) > 0) {
...
#else	not SMTP
	strlen ((char *)strcpy (sm_reply.text, sm_rfp == NULL ? "no pipe opened"
...
{
#ifndef	BSD42
    signal (SIGALRM, (void(*)(int)) alrmser);
#endif	BSD42
    sm_alarmed++;
