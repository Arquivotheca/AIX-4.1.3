static char sccsid[] = "@(#)46	1.5  src/bos/usr/bin/tip/value.c, cmdtip, bos411, 9428A410j 4/10/91 09:07:16";
/* 
...
static int col = 0;
static int vtoken(register char *), vprint(register value_t *), 
	   vaccess(register unsigned, register unsigned);
...
static int
vaccess(register unsigned mode, 
	register unsigned rw)
{
	if (mode & (rw<<PUBLIC))
		return (1);
	if (mode & (rw<<PRIVATE))
