#if !defined(lint)
static char sccsid[] = "@(#)01	1.19  src/bos/usr/bin/ex/ex_vops.c, cmdedit, bos41J, 9514A_all 3/31/95 12:49:30";
#endif
/*
 ...
 *
 * ex_vops.c  1.6  com/cmd/edit/vi,3.1,9013 3/7/90 10:30:39
 * 
 ...
 *
 */
/* ex_vops.c  Revision: 2.5  (OSF) Date: 90/10/07 16:31:03 */
...
void vUndo(void)
{
	...
	cursor = linebuf;
	/*
	 *  The following ludicrous piece of code is required by POSIX.2.
	 *  With any luck, they will realize how absurd this is and fix
	 *  the standard in POSIX.2b.  The code used to be a single line:
	 *  vfixcurs();        (i.e.  The AUTOINDENT part is the addition.)
	 */
	if (value(AUTOINDENT) && (dot > one)) {
	    int cnt;
	    getline(*(dot-1));
	    cnt = vpastwh(linebuf) - linebuf;
	    getline(*dot);
	    vsetcurs(linebuf+cnt);
	} else
	    vfixcurs();
}
...
void vshftop(void)
{
	...
	/* The following check is for POSIX compliance.  If the line did
	 * not change, then the current line should be shifted.
	 */
	if (wdot == NOLINE)
		wdot = dot;
	if ((cnt = xdw()) < 0)
...
void vfilter(void)
{
	 ...
	 */
	/* Defect 100401 addresses the above BUG and uses undap_tmp
	 * to calculate the correct no. of lines when inside the macro
         */
	if (FIXUNDO)
		vreplace(vcline, cnt, undap2 - undap1);
	else
		vreplace(vcline, cnt, (undap2 - undap_tmp) + cnt);
	dot = addr;
	if (dot > dol) {
		dot--;
		vcline--;
...
static int xdw(void)
{
	...
	if (dot > wdot) {
        ...
        else if ((dot == wdot) && (cursor > wcursor))
        {
		if (wcursor)
          	{
		   cp = cursor; cursor = wcursor; wcursor = cp;
 ...
 */
	...
	/*
	 * Kill buffer code.  If delete operator is c, d, C or S, then save
