%{
/* @(#)00	1.8  src/bos/usr/bin/rdist/gram.y, cmdarch, bos411, 9428A410j 4/25/94 20:13:31 */
/*
...
yylex()
{
	static char yytext[INMAX + MB_LEN_MAX*2];
	register wint_t c;
	register char *cp1, *cp2;
	
again:
	switch (c = getwc(fin)) {
	case WEOF:  /* end of file */
		return(0);

	case '#':  /* start of comment */
		while ((c = getwc(fin)) != WEOF && c != '\n')
			;
		if (c == WEOF)
			return(0);
	case '\n':
	...
	struct subcmd *subcmds;
{
	...
	register struct namelist *h, *tmp;

	files = expand(files, E_VARS|E_SHELL);
	hosts = expand(hosts, E_ALL);
	for (h = hosts; h != NULL; tmp = h, h = h->n_next, free((void *)tmp)) {
		/*
