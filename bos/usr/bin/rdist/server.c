static char sccsid[] = "@(#)03	1.17  src/bos/usr/bin/rdist/server.c, cmdarch, bos411, 9435B411a 8/31/94 14:48:23";
/*
...
char	target[BUFSIZ << 1];	/* target/source directory name */
char	linkdest[BUFSIZ << 1];	/* destination directory name */
char	hostdest[BUFSIZ << 1];	/* host directory name */
...
server()
{
	...
	signal(SIGPIPE, (void (*)(int))cleanup);
	...
	for (;;) {
		...
		*--cp = '\0';
		...
		case 'S':  /* Special. Execute commands */
			   expand_file_var(cp); /* tilde expansion of FILE */
			   dospecial(cp);
	...
	int destdir, opts;
{
	...
	if (rname == NULL)
		return;
	strcpy(hostdest, target);
	strcpy(linkdest, dest);
	tp = target;
	while (*tp)
		tp++;
	/*
	...
	char *local_name;
{
	...
	if (pw == NULL || pw->pw_uid != stb.st_uid)
		if ((pw = getpwuid(stb.st_uid)) == NULL) {
			log(lfp, MSGSTR(NOPASS, "%s: no password entry for uid \n"), target);
			pw = NULL;
			sprintf(user, ":%d", stb.st_uid);
		}
	if (gr == NULL || gr->gr_gid != stb.st_gid)
		if ((gr = getgrgid(stb.st_gid)) == NULL) {
			log(lfp, MSGSTR(NOGROUP, "%s: no name for group %d\n"), target);
			gr = NULL;
		...
		while (dp = readdir(d)) {
			...
			*tp++ = '/';
			...
			while (*tp++ = *cp++)
				;
			tp--;
			sendf(dp->d_name, opts, local_name);
		}
		closedir(d);
		(void) write(rem, "E\n", 2);
		(void) response();
	...
	if (u >= 2) {
		...
		/*
		 * For u==3 (-b flag), 'updated' message will be printed, so
		 * this message will not be necessary.
		 */
		if (u == 2)
...
dospecial:
	for (sc = subcmds; sc != NULL; sc = sc->sc_next) {
		...
		/* "target" has expanded source file name on local host,   */
		/* which is installed on the remote machine. If the source */
		/* file name starts with a tilde then strip the home dir   */
		/* out of target and send a "S" message in the form of     */
		/* "SFILE=~filename;shell commands" for special command    */
		/* processing.				   		   */
		{
		   extern char homedir[];
		   char	remote_target[BUFSIZ];
		   char *tmp;

		   strcpy(remote_target, target);
		   if (*local_name == '~')
     		   {
		      if ((tmp = strstr(target, homedir)) != NULL && tmp == target )
		      {
			tmp += strlen(homedir);
		        sprintf(remote_target, "~%s", tmp); 
		      }
		   }
		   (void) sprintf(buf, "SFILE=%s;%s\n", remote_target, sc->sc_name);
		}
	...
	char *name;
{
	...
	if (catname)
		if ( ((int)(tp - target) + strlen(name) +1 ) >= BUFSIZ )
		{
			error(MSGSTR(TOOLONG, "%s/%s: Name too long\n"), target, name);
			return;
		}
...
ok:
	if (userid)
		uid = -1;
	if ((uid != -1 || gid != -1) && chown(file, uid, gid) < 0) {
		note(MSGSTR(CHOWNF, "%s: chown failed: file %s:  %s"),
			     host, file, strerror(errno));
	}
	else if ((mode & 07000) && chmod(file, (mode_t)mode) < 0) {
		note(MSGSTR(CHOWNF, "%s: chmod failed: file %s:  %s"),
			     host, file, strerror(errno));
	}
	return(0);
}
   ...
   char *src;
{
   ...
   if ((tmp1 = strchr(src, '=')) != NULL)
      if (*++tmp1 == '~')
      {
	 tmp2 = strchr(tmp1, ';') + 1;
	 sprintf(newstr, "%s=", strtok(src, "="));
	 exptilde(&newstr[strlen(newstr)], strtok(tmp1, ";"));
	 sprintf(&newstr[strlen(newstr)], ";%s", tmp2);
	 strcpy(src, newstr);
      }
}
