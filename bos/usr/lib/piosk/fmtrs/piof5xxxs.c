static char sccsid[] = "@(#)75	1.4  src/bos/usr/lib/piosk/fmtrs/piof5xxxs.c, cmdpiosk, bos411, 9428A410j 1/21/93 11:39:02";
/*
 * COMPONENT_NAME: (CMDPIOSK) Printer Backend
 *
 * FUNCTIONS: setup, initialize, lineout, passthru, restore, errorexit2, escseq
 *
 * ORIGINS: 27
 *
...
if (Emphasized     < 0 || Emphasized > 1)     errorexit2(MSG_BADFORMFLAG1,"e");
if (Beginpg       <= 0)                       errorexit2(MSG_BADFORMFLAG2,"g");
if (Init_printer   < 0 || Init_printer >1)    errorexit2(MSG_BADFORMFLAG1,"j");
if (Quality        < 1 || Quality > 2)        errorexit2(MSG_BADQUALITY  ,"q");
if (Lpi          !=  6 && Lpi != 8)           errorexit2(MSG_BADFORMFLAG6,"v");
if (Auto_crlf      < 0 || Auto_crlf > 2)      errorexit2(MSG_BADFORMFLAG7,"x");
#if DRIVER != 5587
if (Doublestrike   < 0 || Doublestrike > 1)   errorexit2(MSG_BADFORMFLAG1,"y");
#endif
if (Restoreprinter < 0 || Restoreprinter > 1) errorexit2(MSG_BADFORMFLAG1,"J");
if (Condensed      < 0 || Condensed > 1)      errorexit2(MSG_BADFORMFLAG1,"K");
if (Wrap           < 0 || Wrap > 1)           errorexit2(MSG_BADFORMFLAG1,"L");
if (Cont_dblwide   < 0 || Cont_dblwide > 1)   errorexit2(MSG_BADFORMFLAG1,"W");

if (Pgwidth < 1)                              errorexit2(MSG_BADFORMFLAG2,"w");
if (Indent >= Pgwidth)                        errorexit2(MSG_BADFORMFLAG8,"i");
if (Indent < 0)                               errorexit2(MSG_BADFORMFLAG3,"i");


if (Doublehigh     < 0 || Doublehigh > 1)     errorexit2(MSG_BADFORMFLAG1,"E");
if (Pitch        != 10 && Pitch != 12)        errorexit2(MSG_BADFORMFLAG4,"p");

...
if (cnt == -2)
    {
    piomsgout("at least one stage 2 translate table must be provided (X or Y)");
    pioexit(PIOEXITBAD);
    }
...
if (*wkbuf != NULL)
    ...
    if ((fileptr = fopen(wkbuf, "r")) == NULL)
        {
        piomsgout("open failed for font file to be downloaded");
        pioexit(PIOEXITBAD);
        }
    while(still_sending)
        {
...
static int cdpg_selected = FALSE;  /*|FALSE until first "select font" command|*/
                                   /*|sent to printer                        |*/
...
case 27:                                                     /* '1B' - Escape */
    switch (escseq(fileptr))
		...
		break;                    /* normal return */
        case EOL:     
		goto LINEFEED;            /* end of line */
        case EOF:     
		goto ENDLINE;             /* end of input file */
        }
    break;
...
GRAPHIC:
    ...
    if ((hpos += Hincr) > Width)        /*|If page width would be exceeded by|*/
        ...
        if (Wrap)                       /*|WRAP is on, stuff the char back   |*/
            ...
            vpos += Vincr * Linespacing;/*|like the char was printed by set- |*/
            goto ENDLINE;               /*|ting the DONE flag to TRUE and    |*/
            }                           /*|exiting.                          |*/
        else goto FINISHED;             /*+----------------------------------+*/
        }
    ...
    /* Get Ready to Run the Circular Chain of Code Pages */
    doingsubchar = FALSE;
...
+-----------------------------------------------------------------------------*/
    ...
    for (cur_infop = sel_infop; done == FALSE; cur_infop = cur_infop->infop,
                first_of_multi = FALSE)
        {
        ...
        switch (c2)             /* special code instead of a code point */
            {
            case CP: /* Copy the Code Point (i.e., use the same code point) */
                c2 = c1;
                break;
...
+-----------------------------------------------------------------------------*/
                    ...
                    else  /* Can't Print Subst Char Either, So Give Up*/
                        {
                        piomsgout( "subst char (mS in d.b.) can't be printed");
                        pioexit(PIOEXITBAD);
                        }
                    }
                else continue;
        ...
        default:
            piomsgout("Unexpected value XXX at YYY in stage 2 trans. tab.");
            pioexit(PIOEXITBAD);
            break;
        }
    }
    ...
    if (cur_infop != sel_infop || !cdpg_selected)
        {
...
(void) sprintf(
        msgbuf, NLgetamsg(PIOMSGCAT, PIOMSGSET, msgnum, defmsg), stringval);
...
case 'I':                                                /* Select print mode */
    ...
    +-------------------------------------------------------------------------*/
        case 1:
            if((n1&0x01)&&((n1!=3)&&(n1!=7))) break;  /* only legal odds are 3,7 */
...
case '[':
    ...
    case '\\':                                          /* Set vertical units */
        ...
        if (Gunit_type == 1) Gunits = ((value==216)?216:((value==180)?180:Gunits));
    ...
    case 'g':                                            /* High res graphics */
        ...
        if (Workint1 == 8) n2 = 60;
        if (Workint1 == 9) n2 = 120;
        if (Workint1 == 11) n2 = 180;
        if (Workint1 == 12) n2 = 360;
        hpos += (cnt / n2) * Hres;
        break;
