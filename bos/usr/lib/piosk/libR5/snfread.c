static char sccsid[] = "@(#)95  1.3  src/bos/usr/lib/piosk/libR5/snfread.c, cmdpiosk, bos411, 9428A410j 5/27/93 06:46:27";
/*
...
#if defined( X11R5BOS )
    memcpy( &charInfo->metrics, &snfCharInfo.metrics, sizeof( xCharInfo ) );
#else /* X11R5BOS */
...
#if defined( X11R5BOS )
    memcpy( charInfo, &snfCharInfo.metrics, sizeof( xCharInfo ) );
#else /* X11R5BOS */
...
#if defined( X11R5BOS )
    memcpy( &pFontInfo->minbounds, &snfInfo->minbounds.metrics, sizeof( xCharInfo ) );
    memcpy( &pFontInfo->maxbounds, &snfInfo->maxbounds.metrics, sizeof( xCharInfo ) );
#else /* X11R5BOS */
