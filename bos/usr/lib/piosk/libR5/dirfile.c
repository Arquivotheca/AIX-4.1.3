static char sccsid[] = "@(#)02  1.3  src/bos/usr/lib/piosk/libR5/dirfile.c, cmdpiosk, bos411, 9428A410j 9/14/93 00:49:55";
/*
    ...
    FontDirectoryPtr	dir;
{
...
#if defined( X11R5BOS )
	char** 	x;
	char*	suf[5] = { ".pcf", ".snf", ".pcf.Z", ".snf.Z", 0 };
#endif /* X11R5BOS */
	...
	for( x = suf; *x; x++ ){
		len = strlen( *x );
		if( strcmp( fileName+ strlen( fileName ) -len, *x ) == 0 ) break;
	}
	if( !*x ) continue;
	len = strlen( fileName ) - len;
#else /* X11R5BOS */
