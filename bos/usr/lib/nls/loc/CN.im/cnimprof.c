static char sccsid[] = "@(#)29	1.2  src/bos/usr/lib/nls/loc/CN.im/cnimprof.c, ils-zh_CN, bos41J, 9510A_all 3/6/95 23:29:33";
/*
   ...
   char            access_mode;
{
   ...
   case ABC_USER_REM:
      ...
      break;
   case FSS_USER_DICT:
      pathfind[0] = (char *) getenv(FSSUSRDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME14) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME14);
      pathfind[2] = DEFAULTFSSUSRDICT;
      break;
   case FS_USER_DICT:
      pathfind[0] = (char *) getenv(FSUSRDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME16) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME16);
      pathfind[2] = DEFAULTFSUSRDICT;
      break;
   case FSPH_USER_DICT:
      pathfind[0] = (char *) getenv(FSPHUSRDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME19) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME19);
      pathfind[2] = DEFAULTFSPHUSRDICT;
      break;
   case ABC_USER_DICT:
      pathfind[0] = (char *) getenv(ABCUSRDICT);
      file = (char *) getenv("HOME");
   ...
   case UD_DICT:
      ...
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME12) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME12);
      pathfind[2] = DEFAULTUDDICT;
      break;
   case PY_SYS_DICT:             /* PY system dictionary path => default only */
      pathfind[0] = DEFAULTPYSYSDICT;
      pathfind[1] = pathfind[2] = NULL;          /* reset pointer */
      break;
   case LE_SYS_DICT:            /* LE system dictionary path => default only */
      pathfind[0] = DEFAULTPYLSYSDICT;
      pathfind[1] = pathfind[2] = NULL;          /* reset pointer */
      break;
   case EN_SYS_DICT:             /* EN system dictionary path => default only */
      ...
      break;
   case FSS_SYS_DICT:
      pathfind[0] = (char *) getenv(FSSSYSDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME13) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME13);
      pathfind[2] = DEFAULTFSSSYSDICT;
      break;
   case FSSJM_SYS_DICT:
      pathfind[0] = (char *) getenv(FSSJMSYSDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME17) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME17);
      pathfind[2] = DEFAULTFSSJMSYSDICT;
      break;
   case FS_SYS_DICT:
      pathfind[0] = (char *) getenv(FSSYSDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME15) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME15);
      pathfind[2] = DEFAULTFSSYSDICT;
      break;
   case FSPH_SYS_DICT:
      pathfind[0] = (char *) getenv(FSPHSYSDICT);
      file = (char *) getenv("HOME");
      pathfind[1] = (unsigned char*)malloc(strlen(file) + sizeof(FILENAME18) + 1);
      (void) sprintf(pathfind[1], "%s/%s", file, FILENAME18);
      pathfind[2] = DEFAULTFSPHSYSDICT;
      break;
   case ABC_SYS_DICT_CWD:       /* ABC system dictionary path => default only */
