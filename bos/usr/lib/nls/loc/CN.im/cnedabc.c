static char sccsid[] = "@(#)97	1.2  src/bos/usr/lib/nls/loc/CN.im/cnedabc.c, ils-zh_CN, bos41J, 9509A_all 2/25/95 13:34:39";
/*
...
/*                                                                            */
/*                     AbcSelect         : Process Input Key On Cand. List Box*/
/*                                                                            */
...
/*									      */
/*		       Analize()	 : Process the biaodian,and analize   */
/*                                         the first byte of the input        */
/*                                         information.                       */
/*									      */
...
/*                                                                            */
/*                     ListBox           : Process PageUp/PageDown On List    */
/*                                         Box                                */
/*									      */
...
unsigned short  key;                   /* Ascii Code                         */
{
    ...
    switch( key )
    {
      ...
      case FIVESTROKE_KEY :
        ...
        case ABC_SET_OPTION_KEY :       /* Set ABC Input Method Option        */
            AbcSetOption(fepcb, key);
            break;
        ...
        case ONINPUT:
            ...
            switch(input_char_type)
            {
                ...
                case STC:              /* input finished */
                    ...
                    {
                        out_result_area[0]=biaodian_value;
                        out_pointer=2;
                        msg_type=0;
                        OutResult(fepcb);
                        step_mode = START;
...
unsigned short input_char;
{
    ...
    default:
        ...
        { 
           if ( input_char != ABC_SET_OPTION_KEY )
           {
        ...
        {                                        /* of Chinese and English   */
            if (result_area_pointer>0)
            {
                ...
                for (i=0; i<result_area_pointer; i=i+2) 
                    out_bfb[i/2]=FindHanzi(result_area[i]*0x100+result_area[i+1]);
            } 
            last_out_length=out_length;
            out_length=result_area_pointer/2;
            AddExtLib();                        /* Adjust word remember area */
            SendMsg(fepcb,out_bfb,result_area_pointer);
        } 
        else
        {                                       /* Output the Convert Results */
            last_out_length=out_length;
            out_length=result_area_pointer/2;
            SendMsg(fepcb,result_area,result_area_pointer);
            if (in.buffer[0]!='v')
            {
                step_mode=0;
                return(0);
...
FEPCB    *fepcb;
{
    ...
    if (msg_type==2)
    {                                               /* Show Mix String    */
        ShowString(fepcb,result_area, result_area_pointer);
        CurPos(fepcb,input_msg_disp+result_area_pointer);
    }
    else
    {        /* Change the convert results from internal code to EUC code */
        for (i=0; i<result_area_pointer; i=i+2)
        {
            x=result_area[i]*0x100+result_area[i+1];
            y=FindHanzi(x);
            disp_bf[i]=HIBYTE(y);
            disp_bf[i+1]=LOBYTE(y);
        }
        lng=in.true_length;
        if (jiyi_mode!=0)
        {
...
FEPCB        *fepcb;
{
    ...
    if (in.buffer[0]!='v')
    {
        ...
        if (in.true_length>=1)
            if (in.buffer[0]!='U')
                if (!cap_mode)                    /* First radical isn't 'U' */
                {                                 /* Not captial mode        */
...
BOOL CinoPunctuation()
{
    ...
    /* Get the Chinese punctuation's EUC code     */
...
FEPCB           *fepcb;                 /* FEP Control Block                  */
{
    ...
    static char title[]={0xd6,0xc7,0xc4,0xdc,0x41,0x42,0x43,0xd1,0xa1,0xd4,0xf1,
                        0x20,0x20,0x20,0xca,0xa3,0xd3,0xe0};
...
unsigned short  key;                    /* Ascii Code ( 1/2/Ecs/Enter )       */
{
    ...
    switch( key )
    {
        ...
        case RETURN_KEY : 
            ...
            buffer[1] = auto_mode;         /* Transfer The Peremeters */
            buffer[2] = bdd_flag;
    ...
    fepcb->auxacsz.itemsize = 22;        /* Length Of Title & Bottom. */
    ...
    for(i = 0; i < 6 ; i++)                    
    {
        ...
        for(j = 0; j < fepcb->auxacsz.itemsize; memcpy(tostring," ",1),
            tostring++,j++);
                                     ...
                                     /* Fill Title                      */
    strncpy(buffer, TITLE, strlen(TITLE));
    buffer[strlen(TITLE)] = NULL;
    ...
    strncpy(buffer, OPTION1, strlen(OPTION1));
    buffer[strlen(OPTION1)] = NULL; 
    if ( bdd_flag == 1 )
        strncat(buffer, "开)", 3);
    else
        strncat(buffer, "关)", 3);
    memcpy(fepcb->auxbufs[2], buffer, sizeof(buffer));

    strncpy(buffer, OPTION2, strlen(OPTION2));
    buffer[strlen(OPTION2)] = NULL; 
    if ( auto_mode == 1 )
        strncat(buffer, "开)", 3);
    else
        strncat(buffer, "关)", 3);
    memcpy(fepcb->auxbufs[3], buffer, sizeof(buffer));

                                       /* Fill Bottom Message          */
    strncpy(buffer, BOTTOM, strlen(BOTTOM));
    buffer[strlen(BOTTOM)] = NULL;
...
WORD       input_char;             /* Ascii Code                         */
{
    ...
    {                         /* 'v2' ~ 'v9' convert to Chinese Graphic char */
        input_char &= 0x000f;           /* get the Row code */
        for (i=1; i<=94; i++)
        {                               /* get Colume code  */
            if (input_char==2)
            {
