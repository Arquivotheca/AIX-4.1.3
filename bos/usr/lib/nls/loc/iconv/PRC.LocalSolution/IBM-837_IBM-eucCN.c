static char sccsid[] = "@(#)33	1.1  src/bos/usr/lib/nls/loc/iconv/PRC.LocalSolution/IBM-837_IBM-eucCN.c, ils-zh_CN, bos41J, 9509A_all 2/19/95 23:32:06";
/*
...
/*                                                                         */
/* MODULE NAME:        IBM-837_IBM-eucCN.c                                 */
/*                                                                         */
 ...
 */ 
	    ...
	    if(*host2in >= 0x4040 && *host2in <= 0x6c9f){
	        ...
	        while((*host2in != hosttoeuc[middle][0])&&(start <= end)){
	            if((*host2in) > hosttoeuc[middle][0])
	                 start = middle + 1;
	            else
	            if((*host2in) < hosttoeuc[middle][0])
	    	         end = middle - 1;
