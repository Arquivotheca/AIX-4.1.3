static char sccsid[] = "@(#)46	1.1  src/bos/usr/lib/nls/loc/iconv/PRC.LocalSolution/UCS-2_GB2312.1980-0.c, ils-zh_CN, bos41J, 9509A_all 2/19/95 23:32:33";
/*
...
/*                                                                         */
/* MODULE NAME:        UCS-2_GB2312.1980-0.c                               */
/*                                                                         */
/* DESCRIPTIVE NAME:   Convert Data Encoding in UCS-2 to gb2312.1980-0     */
/*                                                                         */
...
/* CALL        :                                                          */
/**************************************************************************/
static int _iconv_exec (_LC_ucs_lower_iconv_t *cd,
                                ...
                                unsigned char** outbuf, size_t *outbytesleft)
{
	...
	outtmp = &outbuf1;
		...
		*outtmp = ucstogb[middle][1];
		out[0] = *(unsigned char*)outtmp     | cd->mask;
		out[1] = *((unsigned char*)outtmp+1) | cd->mask;
		out += 2;
		in  += 2;
		continue;
	    }else{
...
/* CALL        :                                                          */
/**************************************************************************/
static _LC_ucs_lower_iconv_t       *_iconv_init (_LC_core_iconv_t *core_cd,
                                 char* toname, char* fromname)
{
        _LC_ucs_lower_iconv_t      *cd;

        if (!(cd = (_LC_ucs_lower_iconv_t *) 
			malloc(sizeof(_LC_ucs_lower_iconv_t))))
                return (NULL);
        cd->core = *core_cd;

	if ( !strncmp(toname+strlen(toname)-3, "-GR", 3) )
		cd->mask = 0x80;
	else
		cd->mask = 0;
        return cd;
