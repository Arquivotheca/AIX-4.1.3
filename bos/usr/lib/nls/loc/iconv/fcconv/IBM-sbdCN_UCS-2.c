static char sccsid[] = "@(#)84	1.1  src/bos/usr/lib/nls/loc/iconv/fcconv/IBM-sbdCN_UCS-2.c, ils-zh_CN, bos41B, 9504A 12/19/94 14:49:29";
/*
...
/* DESCRIPTION : Convert Data Encoding in IBM-sbdCN Codeset to            */
  ...
  *******************/
	...
	ucs = &ucsbuf;
	    ...
	    /* Handle Characters belong to GB */
            if (in[0] < 0x7e || in[0] == 0x7f) {
                errno = EILSEQ;
                ret_value = -1;
                break;
            };

            if (in[1] < 0x60 || in[1] == 0x7f) {
                errno = EILSEQ;
                ret_value = -1;
                break;
            };
 	    ...
 	    /* Obtain One Input Word, Either Hanzi or Graphic */
            ...
            *((unsigned char *)euc2in +1) = in[1];
            ...
            middle = ( start + end ) / 2;
            while( *euc2in != sbdcntoucs[middle][0] && start <= end ) {
                if( *euc2in < sbdcntoucs[middle][0] ) 
                    end = middle - 1;
	        else
                    if( *euc2in > sbdcntoucs[middle][0] ) 
                        start = middle + 1;
...
/* DESCRIPTION : Free Memory Allocated for IBM-sbdCN_UCS-2                */
