static char sccsid[] = "@(#)03	1.1  src/bos/usr/lib/nls/loc/iconv/host_lower/UTF-8_IBM-eucTW.c, ils-zh_CN, bos41B, 9504A 12/20/94 10:04:48";
/*
...
/*                                                                         */
/* MODULE NAME:        utf8_CNS11643.CT-GR.c                               */
/*                                                                         */
/* DESCRIPTIVE NAME:   Convert Data Encoding in UTF to IBM-eucTW           */
/*                                                                         */
                                ...
                                unsigned char** outbuf, size_t *outbytesleft)
{
	...
	/* End of while for searching CNS1 */
	if(*ucs2in == ucstocns1[middle][0]) {
            if(e_out - out < 2) { 
	         errno = E2BIG;
                 ret_value = -1;
	...
	/* Using binary search for CNS-2 */
	    start = BEGIN;
	    end   = END2; 
	    middle = (end + start)/2; 
	    while((*ucs2in!=ucstocns2[middle][0])&&(start<= end)){
		if(*ucs2in > ucstocns2[middle][0])
		    start = middle + 1;
		else
		if(*ucs2in < ucstocns2[middle][0])
		    end = middle-1;
		middle = (end + start)/2;
	    };/* End of while for searching */
	    if(*ucs2in == ucstocns2[middle][0]) {
              	if(e_out - out < 4) { 
		     errno = E2BIG;
                     ret_value = -1;
		...
		*outtmp = ucstocns2[middle][1];
		out[0] = 0x8e;
		out[1] = 0xa2;
		out[2] = *(unsigned char*)outtmp;
		out[3] = *((unsigned char*)outtmp+1);
		out += 4;
		in  += inlen;
	    /* End of while for searching CNS2 */

	    /* Process codepoint outside CNS-1, CNS-2*/
	    }else{
		out[0] = 0xa1;
		out[1] = 0xa1;
		out += 2;
...
/* DESCRIPTION : Free Memory Allocated for UTF8_IBM-eucTW                 */
...
/* DESCRIPTION : Initialize UTF8_IBM-eucTW  Converter-Specific            */
/*               Descriptor                                               */
/* EXTERNAL REFERENCES:                                                   */
