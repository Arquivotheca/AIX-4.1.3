static char sccsid[] = "@(#)43	1.1  src/bos/usr/lib/nls/loc/iconv/fold_lower/UTF-8_CNS11643.CT-GR.c, ils-zh_CN, bos41B, 9504A 12/19/94 15:17:00";
/*
...
*/
	    ...
	    /* End of while for seraching cns1 */
	        if(*ucs2in == ucstocns1[middle][0]) {
                    if(e_out - out < 2) { 
	                errno = E2BIG;
                        ret_value = -1;
...
/*
 Process CNS-2 characters
*/
	    else if(wcsid(*ucs2in)==3) {
	           start = BEGIN;
	           end   = END2; 
	           middle = (end + start)/2; 
