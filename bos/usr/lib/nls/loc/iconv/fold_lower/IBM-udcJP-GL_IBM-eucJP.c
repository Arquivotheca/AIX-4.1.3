static char sccsid[] = "@(#)52  1.6  src/bos/usr/lib/nls/loc/iconv/fold_lower/IBM-udcJP-GL_IBM-eucJP.c, cmdiconv, bos411, 9428A410j 8/26/93 01:09:47";
/*
...
static	size_t	_iconv_exec (
	...
	while (in < e_in) {
		...
		if (c1 < 0x6f) {
			...
			code = ibmsel_udctoeuc [(c1 - 0x79) * 0x5e + c2 - 0x21];
			if (!code) {
				errno = EILSEQ; err_flag = TRUE; break;
			}
			if (e_out - out < 3) {
			...
			out[0] = EUCSS3;
			out[1] = code >> 8 & 0xff | 0x80;
			out[2] = code & 0xff | 0x80;
			in += 2;
			out += 3;
		}
	}
