static char sccsid[] = "@(#)23  1.5  src/bos/usr/lib/nls/loc/iconv/fold_lower/JISX0208.1983-GL_IBM-932.c, cmdiconv, bos411, 9428A410j 8/26/93 01:13:32";
/*
...
static	size_t	_iconv_exec (
	...
	while (1) {
		...
		while (low <= high) {
			i = low	+ high >> 1;
			if (sjiscode < SJIStoCP932[i][FROM])
				high = i - 1;
			else if	(sjiscode > SJIStoCP932[i][FROM])
				low = i	+ 1;
			else {
