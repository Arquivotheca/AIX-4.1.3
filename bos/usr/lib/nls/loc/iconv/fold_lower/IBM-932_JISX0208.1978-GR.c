static char sccsid[] = "@(#)06  1.4  src/bos/usr/lib/nls/loc/iconv/fold_lower/IBM-932_JISX0208.1978-GR.c, cmdiconv, bos411, 9428A410j 8/26/93 01:04:44";
/*
...
static	size_t	_iconv_exec (
	...
	while (1) {
		...
		if (c2 >= 0x80) c2--;
		if (c2 >= 0x9e) {
			out[0] = c1 + 0x81;	/* c1 + 1 | 0x80 */
			out[1] = c2 + 3; 	/* c2 - 0x7d | 0x80 */
		}
		else {
			out[0] = c1 | 0x80;
			out[1] = c2 + 0x61;	/* c2 - 0x1f | 0x80 */
		}
		in += 2;
		out += 2;
	}
