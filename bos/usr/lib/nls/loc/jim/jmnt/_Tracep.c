static char sccsid[] = "@(#)58	1.2.1.1  src/bos/usr/lib/nls/loc/jim/jmnt/_Tracep.c, libKJI, bos411, 9428A410j 7/23/92 03:25:41";
/*
 ...
 * MODULE NAME:         _Tracep
 *
 * DESCRIPTIVE NAME:    trace the data argument points to index of data.
 *
 ...
 *
 * FUNCTION:            put out the trace data to memory or to disk.
 *
 ...
 *
 *  LINKAGE:            _Tracep( troutp,trdp )
 *
 *  INPUT:              troutp  : pointer to trace block.
 *                      trdp    : pointer to trace data pointer.
 *
...
/*
 *      Data Trace Pointer Program.
 */
_Tracep( troutp1,trdp1 )

TRB     *troutp1;           /* pointer to trace block.                  */
char    *trdp1;             /* pointer to trace data pointer.           */
{
        /* Function definition.                                         */
        int     write();
        char    *memcpy();
        ...
        struct   tracex
           {
                ...
                char    *datap;     /* Trace data pointer.              */
           }    *trdp;

        /* Distination trace data block.                                */
        struct  trtophdr
           {
                unsigned long   serno;      /* ID number.               */
...
snap3(SNAP_TRB  ,SNAP_Tracep,"START TRACEP");
        ...
        /* 1.2
         *      Get the trace data from the maine program.
         */
...
/************************************************************************
 *      debug process routine.                                          */
snap3(SNAP_TRB  ,SNAP_Tracep,"ERROR 1.2 TRACEP");
...
/************************************************************************
 *      debug process routine.                                          */
snap3(SNAP_TRB  ,SNAP_Tracep,"ERROR 1.5 TRACEP");
                 ...
                 /* Write trace data hedder.(length,index)              */
                 n  =  sizeof(trdp->length);
                 n +=  sizeof(trdp->index);
                 memcpy(&troutp1->trace[i],trdp,n);
                 i +=  n;

                 /* Write trace data without padding data.              */
                 n  =  trdp->length - padlen - n;
                 memcpy(&troutp1->trace[i],trdp->datap,n);
                 i +=  n;
...
/************************************************************************
 *      debug process routine.                                          */
snap3(SNAP_TRB  ,SNAP_Tracep,"END 1.8 TRACEP");
...
/************************************************************************
 *      debug process routine.                                          */
snap3(SNAP_TRB  ,SNAP_Tracep,"ERROR 1.9 TRACEP");
...
snap3(SNAP_TRB  ,SNAP_Tracep,"ERROR 1.9 TRACEP");
                        ...
                        return(TRFWTE);                  /* REEOR RETURN*/
                        }
                /* Write the trace data.                                */
                n = trdp->length - padlen - n;
                write_rc = write(troutp->filds,trdp->datap,n);
...
/************************************************************************
 *      debug process routine.                                          */
snap3(SNAP_TRB  ,SNAP_Tracep,"END 1.9 TRACEP");
