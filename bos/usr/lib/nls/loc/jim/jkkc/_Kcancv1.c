static char sccsid[] = "@(#)07	1.2 src/bos/usr/lib/nls/loc/jim/jkkc/_Kcancv1.c, libKJI, bos411, 9428A410j 6/4/91 10:09:39";
/*
 ...
 *----------------------------------------------------------------------*/ 
   if(( kcb.env != ENVALPH ) || ( z_cmpflg != ON ))
   {
      kcb.env = ENVALPH;
      _Kcxinia( kcbptr1 );
 ...
 *----------------------------------------------------------------------*/ 
      z_ramktb = _Kcamktb( kcbptr1 );
      if ( z_ramktb != SUCCESS )
         return(z_ramktb);              /* _Kcamktb() error             */
      ...
      else if ( z_rnmtch.rc != Z_NOT_EXIST )
