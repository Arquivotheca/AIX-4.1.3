static char sccsid[] = "@(#)44	1.1  src/bos/usr/lib/nls/loc/locale/zh_TW/__mbstowcs_euctw.c, libmeth, bos411, 9428A410j 5/25/92 16:02:50";
/*
 * COMPONENT_NAME: (LIBMETH) LIBC Code-Point/Process-Code Conversion Functions
 *
 * FUNCTIONS: mbstowcs_euctw
 *
 * ORIGINS: 27
 *
...
#include <stdlib.h>
#include <ctype.h>
size_t __mbstowcs_euctw(_LC_charmap_objhdl_t handle, wchar_t *pwcs, const char *s, size_t n)
{
    int len = n;
    int rc;
    int cnt;
    ...
    **********/
    ...
    while (len-- > 0) {
        ...
        **********/
        ...
        if ((cnt = __mbtowc_euctw(handle,pwcs, s, MB_CUR_MAX)) < 0)
            return(-1);
        s += cnt;
        ++pwcs;
    }
