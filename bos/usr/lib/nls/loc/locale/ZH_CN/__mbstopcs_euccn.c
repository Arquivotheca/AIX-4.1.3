static char sccsid[] = "@(#)45	1.1  src/bos/usr/lib/nls/loc/locale/zh_CN/__mbstopcs_euccn.c, ils-zh_CN, bos41B, 9504A 12/20/94 10:25:28";
/*
 *   COMPONENT_NAME: ils-zh_CN
 *
 *   FUNCTIONS: __mbstopcs_euccn
 *
 *   ORIGINS: 27
 *
...
size_t __mbstopcs_euccn(_LC_charmap_objhdl_t handle, wchar_t *pwcs, size_t pwcs_len,
                         const char *s, size_t s_len, char stopchr, char **endptr, int *err)
{
    ...
    *err = 0;
    ...
    while (1) {
        ...
        **********/
        if ((cnt = __mbtopc_euccn(handle,&(pwcs[pwcs_cnt]), &(s[s_cnt]), (s_len - s_cnt), err)) == 0) {
            *endptr = &(s[s_cnt]);
            break;
        }
