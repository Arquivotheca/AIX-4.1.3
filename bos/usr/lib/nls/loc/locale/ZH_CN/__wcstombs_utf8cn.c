static char sccsid[] = "@(#)37	1.1  src/bos/usr/lib/nls/loc/locale/ZH_CN/__wcstombs_utf8cn.c, ils-zh_CN, bos41B, 9504A 12/20/94 10:24:36";
/*
 *   COMPONENT_NAME: ils-zh_CN
 *
 *   FUNCTIONS: __wcstombs_utf8cn
 *
 *   ORIGINS: 27
 *
...
size_t  __wcstombs_utf8cn(_LC_charmap_objhdl_t handle, char *s, const wchar_t *pwcs, size_t n)
{
    int cnt=0;
    int len=0;
    int i=0;
    ...
    if ( s == (char *)NULL) {
        ...
        while (*pwcs != (wchar_t)'\0') {
           /* if ((len = wctomb(tmps, *pwcs)) == -1) */
            if ( (len = __wctomb_utf8cn(handle,tmps, *pwcs)) == -1 )
                return(-1);
            cnt += len;
            pwcs++;
        }
