static char sccsid[] = "@(#)36	1.1  src/bos/usr/lib/nls/loc/locale/ko_KR/__wcstombs_euckr.c, libmeth, bos411, 9428A410j 5/25/92 16:00:06";
/*
 * COMPONENT_NAME:	LIBMETH
 *
 * FUNCTIONS: __wcstombs_euckr
 *
 * ORIGINS: 27
 *
...
#include <stdlib.h>
#include <ctype.h>
size_t 	__wcstombs_euckr(_LC_charmap_objhdl_t handle, char *s, const wchar_t *pwcs, size_t n)
{
    int cnt=0;
    int len=0;
    int i=0;
    ...
    **********/
	...
	while (*pwcs != (wchar_t)'\0') {
	    if ((len = __wctomb_euckr(handle, tmps, *pwcs)) == -1)
		return(-1);
	    cnt += len;
	    pwcs++;
	}
