static char sccsid[] = "@(#)30	1.1  src/bos/usr/lib/nls/loc/locale/ko_KR/__mbstopcs_euckr.c, libmeth, bos411, 9428A410j 5/25/92 15:59:06";
/*
 * COMPONENT_NAME:	LIBMETH
 *
 * FUNCTIONS: __mbstopcs_euckr
 *
 * ORIGINS: 27
 *
...
#include <stdlib.h>
#include <ctype.h>
size_t __mbstopcs_euckr(_LC_charmap_objhdl_t handle, wchar_t *pwcs, size_t pwcs_len,
			 const char *s, size_t s_len, char stopchr, char **endptr, int *err)
{
	...
	**********/
	if ((cnt = __mbtopc_euckr(handle, &(pwcs[pwcs_cnt]), &(s[s_cnt]), (s_len - s_cnt), err)) == 0) {
	    *endptr = &(s[s_cnt]);
	    break;
	}
