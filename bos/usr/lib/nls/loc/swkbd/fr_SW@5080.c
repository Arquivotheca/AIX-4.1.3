static char sccsid[] = "@(#)19  1.7  src/bos/usr/lib/nls/loc/swkbd/fr_SW@5080.c, lftdd, bos411, 9428A410j 10/25/93 15:37:15";
/*
...
{graphic,norm,0,p0,0xA4},      /* intl currency symbol               pos 5   */
...
main ()
{
    ...
    fd = open("fi_FI.ISO8859-1@alt.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
    ...
    fd = open("sv_SE.ISO8859-1@alt.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
