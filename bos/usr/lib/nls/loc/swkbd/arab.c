static char sccsid[] = "@(#)84	1.2  src/bos/usr/lib/nls/loc/swkbd/arab.c, lftdd, bos411, 9428A410j 10/27/93 13:27:54";
/*
 * COMPONENT_NAME: (LFTDD) Low Function Terminal - arab.c
 *
 * ORIGINS: 27
 *
...
{graphic,norm,0,p0,IC_1QUOT},  /* '                                  pos 41  */
{graphic,norm,0,p0,IC_LQUOT},  /* grave accent                       pos 42  */
...
{graphic,norm,0,p0,IC_AND},     /* ^ {circumflex accent)              pos 7   */
...
{graphic,norm,0,p0,IC_LBRACE},  /* Left curly Brace                   pos 27  */
{graphic,norm,0,p0,IC_RBRACE},  /* Right curly Brace                  pos 28  */
{es_func,norm,0, 1,KF_IGNORE},  /* not avail on WT KBD                pos 29  */
{es_func,norm,0, 1,KF_IGNORE},  /* caps Lock key; do not alter        pos 30  */
...
{graphic,norm,0,p0,IC_APPROX},  /* tilde                              pos 42  */
{sgl_ctl,norm,0, 0,IC_CR},      /* carriage Return                    pos 43  */
...
main ()
{
    ...
    fd = open("Ar_AA.IBM-1046.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
    ...
    rc = write(fd,&arab_key_map,sizeof(lft_swkbd_t));
    ...
    arab_key_map.kbdname[2] = 'a';
    arab_key_map.kbdname[3] = 'l';
    arab_key_map.kbdname[4] = 't';

    fd = open("Ar_AA.IBM-1046@alt.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
    ...
    arab_key_map.disp_set[0] = '8';
    arab_key_map.disp_set[1] = '8';
    arab_key_map.disp_set[2] = '5';
    arab_key_map.disp_set[3] = '9';
    arab_key_map.disp_set[4] = '-';
    arab_key_map.disp_set[5] = '6';
    arab_key_map.disp_set[6] = '\0';
    arab_key_map.disp_set[7] = '\0';
    arab_key_map.kbdname[0] = 'a';
    arab_key_map.kbdname[2] = '\0';
    arab_key_map.kbdname[3] = '\0';
    arab_key_map.kbdname[4] = '\0';
    fd = open("ar_AA.ISO8859-6.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
    ...
    fd = open("ar_AA.ISO8859-6@alt.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
