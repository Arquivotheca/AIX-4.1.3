static char sccsid[] = "@(#)96  1.17  src/bos/usr/lib/nls/loc/swkbd/belgium.c, lftdd, bos411, 9428A410j 4/26/94 10:22:02";
/*
...
{graphic,norm,0,p0,IC_SSTWO},  /* superscript 2                     pos 1   */
{graphic,norm,0,p0,IC_AMP},    /* &                                 pos 2   */
{graphic,norm,0,p0,IC_LCEACC}, /* e-acute                           pos 3   */
{graphic,norm,0,p0,IC_2QUOT},  /* "                                 pos 4   */
{graphic,norm,0,p0,0x27},      /* '                                 pos 5   */
{graphic,norm,0,p0,IC_LPAR},   /* (                                 pos 6   */
{graphic,norm,0,p0,IC_SECT},   /* section symbol                    pos 7   */
{graphic,norm,0,p0,IC_LCEGRV}, /* e-grave accent                    pos 8   */
{graphic,norm,0,p0,IC_EXC},    /* !                                 pos 9   */
{graphic,norm,0,p0,IC_LCCCED}, /* c-cedilla small                   pos 10  */
{graphic,norm,0,p0,IC_LCAGRV}, /* a-grave accent                    pos 11  */
...
{graphic,norm,0,p0,IC_LCUGRV}, /* u-grave accent                    pos 41  */
{graphic,norm,0,p0,IC_LCMU},   /* Greek mu small                    pos 42  */
...
{graphic,norm,0,p0,IC_SSTHRE}, /* superscript 3                     pos 1   */
...
{graphic,norm,0,p0,IC_DEGREE}, /* degree                            pos 12  */
...
{graphic,norm,0,p0,IC_STRLNG}, /* pound sterling                    pos 42  */
{sgl_ctl,norm,0, 0,IC_CR},     /* carriage Return                   pos 43  */
...
{graphic,dead,0,p0,IC_ACUTE},  /* acute accent                       pos 41  */
...
main ()
{
    ...
    fd = open("Fr_BE.IBM-850.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
    ...
    belgium_key_map.kbdname[3] = 'B';
    belgium_key_map.kbdname[4] = 'E';
    fd = open("Nl_BE.IBM-850.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
    ...
    belgium_key_map.kbdname[3] = 'B';
    belgium_key_map.kbdname[4] = 'E';
    fd = open("De_BE.IBM-850.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
    ...
    fd = open("Nl_NL.IBM-850.lftkeymap",O_CREAT|O_RDWR, 0777);
    if (fd == -1) {
        printf("ERROR:  bad open of software keyboard output file.\n");
        printf("        errno=%d\n",errno);
        exit(-1);
