/*
 * $Log: idl_base.h,v $
 * Revision 1.16  1994/02/01  21:46:06  ddj
 * Deleted bad prototypes for strcpy and strlen and replaced with <string.h> incl.
 *
 * Revision 1.15  1993/11/24  18:43:39  ddj
 * Fixed types for compatibility with 64 bit long compiler on Alpha OSF/1.
 *
 * Revision 1.14  1993/08/25  20:07:27  ddj
 * Changed define for hp from hp9000s800 to hppa.
 *
 * Revision 1.13  1993/04/22  12:05:32  ddj
 * Renamed ultrix.h to dollars.h and no longer include it for Ultrix builds.
 *
 * Revision 1.12  1992/12/01  16:52:03  ddj
 * Use volatile for Ultrix ports again, since conflicts have been eliminated.
 *
 * Revision 1.11  1992/11/23  21:24:47  ddj
 * Complete rework of globalref/globaldef stuff for VMS port.
 * It still boils down to "extern" or nothing when compiled under unix.
 *
 * Revision 1.10  1992/07/02  15:29:19  ddj
 * Fixes for VAX/VMS port.
 *
 * Revision 1.9  1992/06/25  14:58:32  ddj
 * Workaround for IBM/AIX defining both LITTLE_ENDIAN and BIG_ENDIAN (is BIG).
 *
 * Revision 1.8  1992/04/14  22:11:06  ddj
 * Changed HPUX to hpux to decide on including "ultrix.h".
 *
 * Revision 1.7  1992/04/01  22:13:11  ddj
 * Don't use volatile definitions for ultrix since the compiler is too picky.
 *
 * Revision 1.6  1992/02/06  18:44:00  ddj
 ...
 *
 * Revision 1.5  1991/11/06  22:27:29  ddj
 * Made definition of volatile and Volatile clearer and added ultrix to list
 * that support volatile declarations.
 *
 * Revision 1.4  1991/07/29  16:01:09  ddj
 * Added include of ultrix.h so we can use digital-provided ncs library.
 * Added MIPSEL to list of little-endian machines.
 *
 * Revision 1.3  1991/04/10  14:18:24  ddj
 * Added SYS5 to list of compilers which do not handle left-hand-side casting.
 * This cuts down tremendously on the number of warnings on SVR4, SCO and clix.
 *
 * Revision 1.2  1991/04/01  21:32:12  ddj
 * Add clipper to list of LITTLE_ENDIAN ports.
 * Removed declaration of strlen which conflicted with SCO prototypes.
 *
 ...
 *   hppa
 *      Means that the system is a HP9000/8xx or HP9000/7xx
 *   __STDC__
...
#if defined(vaxc) && !defined(__ALPHA)
#  if CC$gfloat != 1
 ...
 * GlobaldefA is useful for initialized aggregates (struct/array)
 * where the initializer will not work as a macro argument.
 */

#if !defined(vax) && !defined(__VAX)
#  define globaldef
#  define globalref extern
#  define GLOBALDEF(TYPE, NAME, VALUE) TYPE NAME = VALUE
#  define GLOBALREF(TYPE, NAME) extern TYPE NAME
#  define GLOBALDEFA(TYPE, NAME, DECL) TYPE NAME DECL
#  define GLOBALREFA(TYPE, NAME, DECL) extern TYPE NAME DECL
#else
#  if defined(vaxc) || defined(__DECC)
#    define GLOBALDEF(TYPE, NAME, VALUE) globaldef TYPE NAME = VALUE
#    define GLOBALREF(TYPE, NAME) globalref TYPE NAME
#    define GLOBALDEFA(TYPE, NAME, DECL) globaldef TYPE NAME DECL
#    define GLOBALREFA(TYPE, NAME, DECL) globaldef TYPE NAME DECL
#  endif
#  ifdef __GNUC__
#    define __STR(sym) #sym
#    define GLOBALDEF(TYPE, NAME, VALUE) \
       TYPE NAME __asm("_$$PsectAttributes_GLOBALSYMBOL$$" ## __STR(NAME)) = VALUE
#    define GLOBALREF(TYPE, NAME) \
       TYPE NAME __asm("_$$PsectAttributes_GLOBALSYMBOL$$" ## __STR(NAME))
#    define GLOBALDEFA(TYPE, NAME, DECL) \
       TYPE NAME DECL __asm("_$$PsectAttributes_GLOBALSYMBOL$$" ## __STR(NAME))
#    define GLOBALREFA(TYPE, NAME, DECL) \
       TYPE NAME DECL __asm("_$$PsectAttributes_GLOBALSYMBOL$$" ## __STR(NAME))
#  endif
#endif
...
#else
#  if !defined(__STDC__)
#    if !defined(vaxc) && !defined(__DECC) && !defined(hpux) && !defined(ultrix)
#      define volatile
#    endif
#  endif
#endif
...
/*
 * Define standard types for data that must be an exact size.
 */

#if !defined(ifor_types_defined)
typedef short int16;
...
#define rpc__is_comm_failure_status(st)\
    (((st).all & 0xffff0000) == rpc__mod)
...
#define rpc__equal_drep(drep1, drep2)\
...
#ifdef apollo
#  define rpc__local_drep_packed  rpc__local_drep_p_pure_data_
#  define rpc__local_drep         rpc__local_drep_pure_data_
#  define rpc__ascii_to_ebcdic    rpc__ascii_to_ebcdic_pure_data_
#  define rpc__ebcdic_to_ascii    rpc__ebcdic_to_ascii_pure_data_
#endif

/*
 * Hack because AIX defines both LITTLE_ENDIAN and BIG_ENDIAN in <sys/machine.h>
 * 
 */
...
/*
 * The following conditional avoids loading the NCK library in
 * some of the smaller tool applications, when compiling under VMS.
 */
#if !defined(vms) || (defined(NCK) || defined(NIDL_GENERATED))
/*
...
GLOBALREF(rpc__drep_t, rpc__local_drep);
...
GLOBALREFA(char, rpc__ascii_to_ebcdic, []);
GLOBALREFA(char, rpc__ebcdic_to_ascii, []);
#endif
 ...
 */
#define ndr__boolean        unsigned char
#define ndr__false          ((unsigned char) 00)
#define ndr__true           ((unsigned char) 0xff)
#define ndr__byte           unsigned char
#define ndr__char           unsigned char
#define ndr__small_int      short int           /* 24/32 bits => XMP/2 */
#define ndr__short_int      short int           /* 24/32 bits => XMP/2 */
#define ndr__long_int       int                 /* 64/32 bits => XMP/2 */
#define ndr__hyper_int      long int
#define ndr__usmall_int     unsigned short int  /* 24/32 bits => XMP/2 */
#define ndr__ushort_int     unsigned short int  /* 24/32 bits => XMP/2 */
#define ndr__ulong_int      unsigned int        /* 64/32 bits => XMP/2 */
#define ndr__uhyper_int     unsigned long int
#define ndr__short_float    float
#define ndr__long_float     float
...
#if defined(i386) || defined(vax) || defined(M_I86) || defined (clipper) || defined(LITTLE_ENDIAN) || defined(MIPSEL) || defined(__alpha) || defined(__ALPHA)

#undef USE_DEFAULT_NDR_REPS

#define ndr__boolean        unsigned char
#define ndr__false          ((unsigned char) 0x00)
#define ndr__true           ((unsigned char) 0xff)
#define ndr__byte           unsigned char
#define ndr__char           unsigned char
#define ndr__small_int      char
#define ndr__usmall_int     unsigned char
#define ndr__short_int      short int
#define ndr__ushort_int     unsigned short int
#if defined(__alpha) && defined(__osf__)
#define ndr__long_int       int
#define ndr__ulong_int      unsigned int

#define ndr__hyper_int      long
#define ndr__uhyper_int     unsigned long
#else
#define ndr__long_int       long int
#define ndr__ulong_int      unsigned long int

/* the reps for hyper must match the little-endian NDR rep since
   defined(vax) || defined(M_I86) => defined(ALIGNED_SCALAR_ARRAYS) */
struct ndr__hyper_int_rep   {ndr__ulong_int low; ndr__long_int high;};
#define ndr__hyper_int      struct ndr__hyper_int_rep
struct ndr__uhyper_int_rep  {ndr__ulong_int low; ndr__ulong_int high;};
#define ndr__uhyper_int     struct ndr__uhyper_int_rep
#endif	/* __alpha && __osf__ */

#define ndr__short_float    float
#define ndr__long_float     double
...
struct ndr__hyper_int_rep   {ndr__long_int high; ndr__ulong_int low;};
#define ndr__hyper_int      struct ndr__hyper_int_rep
struct ndr__uhyper_int_rep  {ndr__ulong_int high; ndr__ulong_int low;};
#define ndr__uhyper_int     struct ndr__uhyper_int_rep
...
#if !defined(vms) || (defined(__GNUC__) && (__GNUC__ < 2))
#ifdef __STDC__
#include <string.h>
#else
    char *strcpy();
...
void *rws__alloc_heap_pool (
        rws__pool_t     &apool ,
        long            &bytes
);

void rws__release_heap_pool (
        void                 *&p ,    /* must be ptr returned by alloc */
        rws__pool_t          &apool ,
        long /* status_$t */ *status
);

#define rpc__malloc(n) \
    rws__alloc_heap_pool(rws__stream_tm_pool, (long) (n))

#define rpc__free(p) { \
    long status; \
    rws__release_heap_pool((p), rws__stream_tm_pool, &status); \
}
...
typedef char *rpc__mp_t;

#define rpc__init_mp(mp, dbp, bufp, offset)\
    mp = (rpc__mp_t)bufp;\
    normalize_ptr (mp);\
    mp += offset;\
    normalize_ptr (mp);\
...
#define rpc__advance_mp(mp, delta)\
    mp += delta

#define rpc__align_ptr_relative(p, base, alignment)\
...
#if defined(vaxc) || defined(__DECC) || defined(MSDOS) || defined(apple_aux) || defined(hppa) || defined(__hppa) || defined(SYS5)

#undef USE_DEFAULT_MACROS

#define rpc__marshall_boolean(mp, src)\
    *(ndr__boolean *)mp = src

#define rpc__unmarshall_boolean(mp, dst)\
    dst = *(ndr__boolean *)mp

#define rpc__convert_boolean(src_drep, dst_drep, mp, dst)\
    rpc__unmarshall_boolean(mp, dst)



#define rpc__marshall_byte(mp, src)\
    *(ndr__byte *)mp = src

#define rpc__unmarshall_byte(mp, dst)\
    dst = *(ndr__byte *)mp

#define rpc__convert_byte(src_drep, dst_drep, mp, dst)\
    rpc__unmarshall_byte(mp, dst)



#define rpc__marshall_char(mp, src)\
    *(ndr__char *)mp = src

#define rpc__unmarshall_char(mp, dst)\
    dst = *(ndr__char *)mp

#define rpc__convert_char(src_drep, dst_drep, mp, dst)\
    if (src_drep.char_rep == dst_drep.char_rep)\
        rpc__unmarshall_char(mp, dst);\
    else if (dst_drep.char_rep == rpc__drep_char_ascii)\
        dst = rpc__ebcdic_to_ascii [*(ndr__char *)mp];\
    else\
        dst = rpc__ascii_to_ebcdic [*(ndr__char *)mp]



#define rpc__marshall_small_int(mp, src)\
    *(ndr__small_int *)mp = src

#define rpc__unmarshall_small_int(mp, dst)\
    dst = *(ndr__small_int *)mp

#define rpc__convert_small_int(src_drep, dst_drep, mp, dst)\
    rpc__unmarshall_small_int(mp, dst)



#define rpc__marshall_usmall_int(mp, src)\
    *(ndr__usmall_int *)mp = src

#define rpc__unmarshall_usmall_int(mp, dst)\
    dst = *(ndr__usmall_int *)mp

#define rpc__convert_usmall_int(src_drep, dst_drep, mp, dst)\
    rpc__unmarshall_usmall_int(mp, dst)



#define rpc__marshall_short_int(mp, src)\
    *(ndr__short_int *)mp = src

#define rpc__unmarshall_short_int(mp, dst)\
    dst = *(ndr__short_int *)mp

#define rpc__convert_short_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep)\
        rpc__unmarshall_short_int(mp, dst);\
    else {\
        ndr__byte *_d = (ndr__byte *) &dst;\
        ndr__byte *_s = (ndr__byte *) mp;\
        _d[0]=_s[1]; _d[1]=_s[0];\
        }



#define rpc__marshall_ushort_int(mp, src)\
    *(ndr__ushort_int *)mp = (ndr__ushort_int)src

#define rpc__unmarshall_ushort_int(mp, dst)\
    *((ndr__ushort_int *)&dst) = *(ndr__ushort_int *)mp

#define rpc__convert_ushort_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep)\
        rpc__unmarshall_ushort_int(mp, dst);\
    else {\
        ndr__byte *_d = (ndr__byte *) &dst;\
        ndr__byte *_s = (ndr__byte *) mp;\
        _d[0]=_s[1]; _d[1]=_s[0];\
...
#define rpc__marshall_long_int(mp, src)\
    *(ndr__long_int *)mp = src

#define rpc__unmarshall_long_int(mp, dst)\
    dst = *(ndr__long_int *)mp

#define rpc__convert_long_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep)\
        rpc__unmarshall_long_int(mp, dst);\
    else {\
        ndr__byte *_d = (ndr__byte *) &dst;\
        ndr__byte *_s = (ndr__byte *) mp;\
...
#define rpc__marshall_ulong_int(mp, src)\
    *(ndr__ulong_int *)mp = (ndr__ulong_int)src

#define rpc__unmarshall_ulong_int(mp, dst)\
    *((ndr__ulong_int *)&dst) = *(ndr__ulong_int *)mp

#define rpc__convert_ulong_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep)\
        rpc__unmarshall_ulong_int(mp, dst);\
    else {\
        ndr__byte *_d = (ndr__byte *) &dst;\
        ndr__byte *_s = (ndr__byte *) mp;\
        _d[0]=_s[3]; _d[1]=_s[2]; _d[2]=_s[1]; _d[3]=_s[0];\
...
#define rpc__marshall_hyper_int(mp, src) {\
    *(ndr__hyper_int *)mp = *(ndr__hyper_int *)&src;\
    }

#define rpc__unmarshall_hyper_int(mp, dst) {\
    *(ndr__hyper_int *)&dst = *(ndr__hyper_int *)mp;\
    }

#define rpc__convert_hyper_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep)\
        rpc__unmarshall_hyper_int(mp, dst)\
    else {\
        ndr__byte *_d = (ndr__byte *) &dst;\
        ndr__byte *_s = (ndr__byte *) mp;\
...
#define rpc__marshall_uhyper_int(mp, src) {\
    *(ndr__uhyper_int *)mp = *(ndr__uhyper_int *)&src;\
    }

#define rpc__unmarshall_uhyper_int(mp, dst) {\
    *(ndr__uhyper_int *)&dst = *(ndr__uhyper_int *)mp;\
    }

#define rpc__convert_uhyper_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep)\
        rpc__unmarshall_uhyper_int(mp, dst)\
    else {\
        ndr__byte *_d = (ndr__byte *) &dst;\
        ndr__byte *_s = (ndr__byte *) mp;\
        _d[0]=_s[7]; _d[1]=_s[6]; _d[2]=_s[5]; _d[3]=_s[4];\
...
#define rpc__marshall_short_float(mp, src) {\
    ndr__short_float tmp;\
    tmp = src;\
    *(ndr__short_float *)mp = tmp;\
    }

#define rpc__unmarshall_short_float(mp, dst)\
    dst = *(ndr__short_float *)mp

#define rpc__convert_short_float(src_drep, dst_drep, mp, dst)\
    if ((src_drep.float_rep == dst_drep.float_rep) &&\
        (src_drep.int_rep   == dst_drep.int_rep))\
        rpc__unmarshall_short_float(mp, dst);\
    else {\
        rpc__cvt_short_float (src_drep, dst_drep,\
            (rpc__short_float_p_t)mp,\
            (rpc__short_float_p_t)&dst);\
        }



#define rpc__marshall_long_float(mp, src)\
    *(ndr__long_float *)mp = src

#define rpc__unmarshall_long_float(mp, dst)\
    dst = *(ndr__long_float *)mp

#define rpc__convert_long_float(src_drep, dst_drep, mp, dst)\
    if ((src_drep.float_rep == dst_drep.float_rep) &&\
        (src_drep.int_rep   == dst_drep.int_rep))\
        rpc__unmarshall_long_float(mp, dst);\
    else\
        rpc__cvt_long_float (src_drep, dst_drep,\
            (rpc__long_float_p_t)mp,\
            (rpc__long_float_p_t)&dst)



#define rpc__marshall_string(lenvar, maxlen, mp, src)\
    {\
    if ((lenvar = strlen((ndr__char *)src)) >= maxlen)\
        SIGNAL(nca_status__string_too_long)\
    rpc__marshall_ushort_int (mp, lenvar);\
    rpc__advance_mp(mp, 2);\
    strcpy ((ndr__char *)mp, (ndr__char *)src);\
    }

#define rpc__unmarshall_string(lenvar, mp, dst)\
    {\
    rpc__unmarshall_ushort_int (mp, lenvar);\
    rpc__advance_mp(mp, 2);\
    strcpy ((ndr__char *)dst, (ndr__char *)mp);\
    }

#define rpc__convert_string(src_drep, dst_drep, lenvar, mp, dst)\
    {\
    rpc__convert_ushort_int (src_drep, dst_drep, mp, lenvar);\
    rpc__advance_mp(mp, 2);\
    if (src_drep.char_rep == dst_drep.char_rep)\
        strcpy ((ndr__char *)dst, (ndr__char *)mp);\
    else\
        rpc__cvt_string (src_drep, dst_drep, (rpc__char_p_t)mp, (rpc__char_p_t)dst);\
    }
 ...
 */
#define rpc__put_2(mp, src)\
    {\
    unsigned long buf = src; /* necessary since src isn't always an lvalue */\
    mp[0] = ((rpc__mp_t)&buf)[6];\
    mp[1] = ((rpc__mp_t)&buf)[7];\
    }

#define rpc__get_2(mp, dst)\
    {\
    ((rpc__mp_t)&dst)[6] = mp[0];\
    ((rpc__mp_t)&dst)[7] = mp[1];\
    }

#define rpc__get_2_swab(mp, dst)\
    {\
    ((rpc__mp_t)&dst)[6] = mp[1];\
    ((rpc__mp_t)&dst)[7] = mp[0];\
    }

#define rpc__put_4(mp, src)\
    {\
    unsigned long buf = src; /* necessary since src isn't always an lvalue */\
    mp[0] = ((rpc__mp_t)&buf)[4];\
    mp[1] = ((rpc__mp_t)&buf)[5];\
    mp[2] = ((rpc__mp_t)&buf)[6];\
    mp[3] = ((rpc__mp_t)&buf)[7];\
    }

#define rpc__get_4(mp, dst)\
    {\
    ((rpc__mp_t)&dst)[4] = mp[0];\
    ((rpc__mp_t)&dst)[5] = mp[1];\
    ((rpc__mp_t)&dst)[6] = mp[2];\
    ((rpc__mp_t)&dst)[7] = mp[3];\
    }

#define rpc__get_4_swab(mp, dst)\
    {\
    ((rpc__mp_t)&dst)[4] = mp[3];\
    ((rpc__mp_t)&dst)[5] = mp[2];\
    ((rpc__mp_t)&dst)[6] = mp[1];\
    ((rpc__mp_t)&dst)[7] = mp[0];\
    }

#define rpc__get_8_swab(mp, dst)\
    {\
    ((rpc__mp_t)&dst)[0] = mp[7];\
    ((rpc__mp_t)&dst)[1] = mp[6];\
    ((rpc__mp_t)&dst)[2] = mp[5];\
    ((rpc__mp_t)&dst)[3] = mp[4];\
    ((rpc__mp_t)&dst)[4] = mp[3];\
    ((rpc__mp_t)&dst)[5] = mp[2];\
    ((rpc__mp_t)&dst)[6] = mp[1];\
    ((rpc__mp_t)&dst)[7] = mp[0];\
...
#define rpc__convert_char(src_drep, dst_drep, mp, dst)\
    if (src_drep.char_rep == dst_drep.char_rep){\
        ...
        (ndr__char)dst = rpc__ebcdic_to_ascii [*(ndr__char *)mp];\
    else\
        (ndr__char)dst = rpc__ascii_to_ebcdic [*(ndr__char *)mp]
...
#define rpc__marshall_hyper_int(mp, src)\
    *(ndr__hyper_int *)mp = src

#define rpc__unmarshall_hyper_int(mp, dst)\
    dst = *(ndr__hyper_int *)mp

#define rpc__convert_hyper_int(src_drep, dst_drep, mp, dst)\
    if (src_drep.int_rep == dst_drep.int_rep){\
        rpc__unmarshall_hyper_int(mp, dst);\
    } else {\
        rpc__get_8_swab(mp, dst);\
    }



#define rpc__marshall_uhyper_int(mp, src)\
    *(ndr__uhyper_int *)mp = src

#define rpc__unmarshall_uhyper_int(mp, dst)\
    dst = *(ndr__uhyper_int *)mp
...
{\
    ...
    extern void rpc__cray64_to_ieee32();\
\
    rpc__cray64_to_ieee32(&src, &ieee_32);\
    rpc__put_4(mp, ieee_32);\
}
...
{\
    unsigned long ieee_32;\
    extern void rpc__ieee32_to_cray64();\
\
    rpc__get_4(mp, ieee_32);\
    rpc__ieee32_to_cray64(&ieee_32, &dst);\
}
...
#define rpc__convert_short_float(src_drep, dst_drep, mp, dst)\
    if ((src_drep.float_rep == dst_drep.float_rep) &&\
        ...
        unsigned long buf;\
        rpc__get_4(mp, buf);\
        rpc__cvt_short_float (\
            src_drep,\
            dst_drep,\
            (rpc__short_float_p_t)&buf,\
            (rpc__short_float_p_t)&dst); \
    }
