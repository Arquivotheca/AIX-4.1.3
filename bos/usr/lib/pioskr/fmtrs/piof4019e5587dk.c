static char sccsid[] = "@(#)73  1.4  src/bos/usr/lib/pioskr/fmtrs/piof4019e5587dk.c, cmdpioskr, bos411, 9428A410j 6/3/94 12:58:39";
/******************************************************************************
*                                                                             *
...
#if !defined( X11R5BOS )
snffontRec snffont1;                             /* structure for snf font */
#endif /* X11R5BOS */
...
#if !defined( X11R5BOS )
snffontRec snffont2;                             /* structure for snf font */
#endif /* X11R5BOS */
...
#if !defined( X11R5BOS )
snffontRec snffont3;                             /* structure for snf font */
#endif /* X11R5BOS */
...
# if defined( X11R5BOS )
	...
	set_font_files ( User_font.ptr );
	init_Fontlist( u_font1, FI );
	init_Fontlist( u_font2, FI );
	if( u_font3 ){
		uuse++;
		init_Fontlist( u_font3, FI );
	}
	fix_JPkana( CDI, FI );
	...
	lc_p_code_change( CSI, "IBM-850" );	/* printer codeset is ibm-850*/
...
#if !defined( X11R5BOS )
#if defined(EMUDEBUG)
	fprintf(debfp, "_F string=%s\n", User_font.ptr);
#endif

	set_font_files ( User_font.ptr );

#if defined(EMUDEBUG)
	User_font.len = strlen ( User_font.ptr );
	fprintf(debfp, "User_font.ptr=%s\n", User_font.ptr);
	fprintf(debfp, "BY set_font_files()\n");
	fprintf(debfp, "u_font1=%s\n", u_font1);
	fprintf(debfp, "u_font2=%s\n", u_font2);
	fprintf(debfp, "u_font3=%s\n", u_font3);
	/*u_font1 = Kanji_font.ptr;
	u_font2 = Kana_font.ptr;
	u_font3 = User_font_default.ptr;
	fprintf(debfp, "u_font1=%s\n", u_font1);
	fprintf(debfp, "u_font2=%s\n", u_font2);
	fprintf(debfp, "u_font3=%s\n", u_font3);*/
	fflush(debfp);
#endif
        if (init_snf (u_font1, &u_ptr1, &snffont1 ) < 0) {
            if (u_ptr1) {
                free ((void *)u_ptr1);
                errorexit(MSG_BADUSERFONT6, u_font1);
            } else {
		warnmsg(MSG_BADUSERFONT6, u_font1);
	    }
        }
#if defined(EMUDEBUG)
	fprintf(debfp, "End of fonts setup\n" );
	fflush( debfp );
#endif

        if (init_snf (u_font2, &u_ptr2, &snffont2 ) < 0) {
            if (u_ptr2) {
                free ((void *)u_ptr2);
                errorexit(MSG_BADUSERFONT6, u_font2);
            } else {
		warnmsg(MSG_BADUSERFONT6, u_font2);
	    }
        }
...
#endif
        if (strlen(u_font3)) {
            if (init_snf (u_font3, &u_ptr3, &snffont3 ) < 0) {
                if (u_ptr3) {
                    free ((void *)u_ptr3);
                    errorexit(MSG_BADUSERFONT6, u_font3);
                } else {
		    warnmsg(MSG_BADUSERFONT6, u_font3);
		}
            } else {
            	uuse++;
	    }
...
/* extern char *Codepagename.ptr; ibm932 or eucjp*/
#endif /* X11R5BOS */
...
/**************************************************/
int     fontout1( UdfInfoRec *puinfo )
{
        char *pimg, imgbuf[256];
        char *pbicmd;
        int     xs, ys, xd, yd;
        ...
        bheight = ((*puinfo).height+7)>>3;
        bwidth  = ((*puinfo).width +7)>>3;
        if( (*puinfo).width <= 16 && (*puinfo).height <=16 ){
                dpi = 120;
                pbicmd = DDBIT_IMG_CMD;
        } else {
                dpi = 240;
        ...
        xs = Hincr - (*puinfo).width*1440/dpi/2;
        xd = -( bwidth*8*1440/dpi );
        ys = Vincr/2 - (*puinfo).height*1440/dpi/2;
        yd = 8*1440/dpi;
        ...
        for (i=1; i<bheight; i++){
                ...
                pput1 ( bwidth*8 );
                pput1 (  0 );
                pputn ( bwidth*8, (*puinfo).image+bwidth*8*i );
        }
...
/**************************************************/
int     fontout2( UdfInfoRec *puinfo )
{
        char *pimg, imgbuf[256];
        char *pbicmd;
        int     xs, ys, xd, yd;
        ...
        xs = Hincr/2 - (*puinfo).width*1440/dpi/2; 		  /* a half of CPI */
        xd = -( bwidth*8*1440/dpi );
        ys = Vincr/2 - (*puinfo).height*1440/dpi/2;
        yd = 8*1440/dpi;
...
static int set_font_files ( char *s )
{
        char    *end, *delim=",";
        int     n;
        int     l;
...
* NAME: delblank
* 
* FUNCTION: delete blanks from string
**********************************************************************/

static int     delblank(char   *s)
{
        char    *from, *to;
...
int init_snf( char *u_font, char **pudf_ptr, snffontRec *psnffont )
{
        FontPropPtr  ptr;
        char         *name, *value;
        int          rc1, rc2, i;
	...
	fflush( debfp );
        ...
        if (!(buf.st_mode & 0x8000)) {
#if defined( EMUDEBUG )
	fprintf( debfp, "bat statmode\n" );
	fflush( debfp );
#endif
            return(-1);
        }
        *pudf_ptr = (char *)malloc ((unsigned int)(buf.st_size));
        if (!*pudf_ptr) {
#if defined( EMUDEBUG )
	fprintf( debfp, "bat pudf_ptr1\n" );
	fflush( debfp );
#endif
            return(-1);
        }
        if ((u_des = open (u_font, O_RDONLY)) == -1) {
            free( (void *)(*pudf_ptr) );
#if defined( EMUDEBUG )
	fprintf( debfp, "bat pudf_ptr2\n" );
	fflush( debfp );
#endif
            return(-1);
        }
        if (read (u_des, *pudf_ptr, (unsigned int)(buf.st_size)) == -1) {
            free ( (void *)(*pudf_ptr) );
#if defined( EMUDEBUG )
	fprintf( debfp, "bat pudf_ptr3\n" );
	fflush( debfp );
#endif
            return(-1);
        }
/* check version1 to see if this is an snf file of correct version */
        if (*(unsigned int *)(*pudf_ptr)!= FONT_FILE_VERSION) return(-1);

#if defined( EMUDEBUG )
	fprintf( debfp, "font file version o.k.\n" );
	fflush( debfp );
#endif

/* initialize and set the snffont structure */
        (*psnffont).pFI = (FontInfoPtr)(*pudf_ptr);
        (*psnffont).pCI = (CharInfoPtr)(ADDRCharInfoRec((*psnffont).pFI));
        (*psnffont).pGlyphs = (char *)(ADDRCHARGLYPHS((*psnffont).pFI));
        (*psnffont).pFP = (FontPropPtr)(ADDRXFONTPROPS((*psnffont).pFI));
        (*psnffont).strings = (char *)(ADDRSTRINGTAB((*psnffont).pFI));

/* double check if this is an snf file of correct version */
        if ((*psnffont).pFI->version2 != FONT_FILE_VERSION) return(-1);

#if defined( EMUDEBUG )
	fprintf( debfp, "All o.k.\n" );
	fflush( debfp );
#endif
...
int getfont( char *pcode, unsigned int codeoffset, int flag ,
             snffontRec *psnffont, UdfInfoRec *puinfo )
{
        CharInfoRec   cinfo;
        unsigned char *glyph;
        ...
        void          rmpad(), c_image(), c_image_us();
        unsigned int  pc2jis();


        if ( flag == 2 ) {  /* Kanji */
          (*puinfo).code = pc2jis( (unsigned int)*(unsigned short *)pcode );
        } else if ( flag == 1 ) { /* Kana */
          (*puinfo).code = (unsigned int)*(unsigned char *)pcode;
        } else {
          return (-1);
        }
        if ((*puinfo).code == 0) return (-1);
        (*puinfo).code -= codeoffset;

/* check if the char for the JIS code exists */
        code_row = ((*puinfo).code >> 8) & 0xff;
        code_col = (*puinfo).code & 0xff;
        if ((code_row < (*psnffont).pFI->firstRow) ||
            (code_row > (*psnffont).pFI->lastRow ) ||
            (code_col < (*psnffont).pFI->firstCol) ||
            (code_col > (*psnffont).pFI->lastCol )) return(-1);

/* get the CharInfoRec structure for the JIS code */
        code_row -= (*psnffont).pFI->firstRow;
        code_col -= (*psnffont).pFI->firstCol;
        code_npr  = (*psnffont).pFI->lastCol - (*psnffont).pFI->firstCol + 1;
        cinfo = *((*psnffont).pCI + code_col + code_row * code_npr);
        if (!cinfo.exists) return(-1);

/* get character width */
        (*puinfo).width  = cinfo.metrics.rightSideBearing -
                       cinfo.metrics.leftSideBearing - 2;
             /* there is one blank col at both side of the char box. */
        if ((*puinfo).width <= 0) return(-1);

/* get character height */
        (*puinfo).height = cinfo.metrics.ascent +
                       cinfo.metrics.descent - 3;
             /* there are two blank lines at the bottom, one at the  */
             /* top of the char box.                                 */
        if ((*puinfo).height <= 0) return(-1);

/* get the length of the image data */
        bwidth = ((*puinfo).width+7) >> 3;        /* char width in byte  */
        bheight = ((*puinfo).height+7) >> 3;      /* char height in byte */
        (*puinfo).bytes = bwidth * ((*puinfo).height);
        if ((*puinfo).bytes <= 0) return(-1);

/* get the addr of the glyph data */
        glyph = (*psnffont).pGlyphs + cinfo.byteOffset;

        rmpad(glyph, bwidth, image_buf, puinfo);
        c_image_us(image_buf, (*puinfo).image, bwidth, bheight);
...
void rmpad(glyph, bwidth, image_buf, puinfo)
char *glyph;
int  bwidth;
char image_buf[];
UdfInfoRec *puinfo;
{
        ...
        bpr = GLWIDTHBYTESPADDED((*puinfo).width+2, DEFAULTGLPAD);
...
/* data, and store the new data into image_buf[].                       */
        from = bpr;
        to   = (*puinfo).height*bpr+bpr-1;  /* maybe from 4 to 99, when 24x24 */
        i = 0;
        for (j=from; j<=to ; j++) {
           image_buf[i] = glyph[j];
           i++;
...
/* remove left edge colum, and shift left 1 bit */
        i = 0;
        if (!( ((*puinfo).width)%8 )) {  /* 16x16,24x24,32x32,16x8,32x16 */
           for (j=0; j<(*puinfo).height; j++) {
              for (k=0; k<bwidth; k++) {
                 index = k + j * bpr;
...
#else /* X11R5BOS */
void c_image_us (indata, outdata, bwidth, bheight)
char indata[], outdata[];
int  bwidth, bheight;
{
        int i, j, m, n, indexin, indexout;
