static char sccsid[] = "@(#)45  1.2  ucfgbaud.c, bos, bos320 4/14/93 16:40:52";
/*
** FUNCTIONS: cfgbaud -l baudX
**
*/
   ...
   char **envp;
{
    ...
    if (cusobj.status == AVAILABLE) {     /* bauddd is configured */
        ...
        if ((majorno = genmajor("bauddiagdd")) == -1) {
            printf("majorno error = %x/n",majorno);
            err_exit(E_MAJORNO);
            }
        ...
        if (minor_list == NULL || how_many == 0) {
                printf("minorlist error\n");
                printf("howmany = %x,  minor_list = %x\n",how_many,minor_list);
                err_exit(E_MINORNO);
                }
        ...
        /* call loadext to unload the device driver */
        if ((dd.kmid = loadext("/etc/drivers/bauddiagdd",FALSE, FALSE)) == NULL)
            {
            /* error loading device driver */
            printf("ERROR UNLOADING DD\n");
            err_exit(E_LOADEXT);
            };
