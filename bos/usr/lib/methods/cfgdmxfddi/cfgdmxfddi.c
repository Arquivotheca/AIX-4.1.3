static char sccsid[] = "@(#)37  1.1  src/bos/usr/lib/methods/cfgdmxfddi/cfgdmxfddi.c, sysxfddi, bos411, 9428A410j 11/29/93 10:15:48";
/*
 * COMPONENT_NAME: (sysxfddi) cfgdmxfddi.c - FDDI Demuxer Method
 *
 * FUNCTIONS: main
 *
 * ORIGINS: 27
...
#include "cfgdmxfddi.h"          /* Ethernet specific information */
...
char *argv[];
{
	...
	switch (command) {
		...
		if (sysconfig(SYS_CFGKMOD,&cfg_k,sizeof(struct cfg_kmod)) == -1) {
			...
			/* Do not unload driver if any error.  Devie has   */
			/* not unpinned and unload will crash system.      */
			exit(E_CFGTERM);
		}
