static char sccsid[] = "@(#)76 1.1 src/bos/usr/lib/methods/chgtty/mattrval.c, cfgtty, bos41J, 9520A_all 4/27/95 14:08:13";
/*
 ...
 *  putype  -- parent Device uniquetype
 ...
 *  all
 */
int mattrval(utype, putype, pattr, errattr)
	char	*utype;
	char	*putype; /* parent unique type */
	char	*pattr;
	char	**errattr;
{
	...
	while (get_next_pair(&input_ptr, &parsed_ok)) { 
		...
		if (rc != 0) {
			...
			rc = get_values_and_flags(putype, AttrName, values);
			if (rc != 0) {
				errproc();	/* put attr in error buf. */
				continue;
			}
		...
		if (GFlag) {
			...
			for (i = 0 ; i < num ; i++) {
				...
				if (rc < 0) {
					rc = chk_attr_in_group(putype, inpval, subvals[i]);
					if (rc < 0) {
