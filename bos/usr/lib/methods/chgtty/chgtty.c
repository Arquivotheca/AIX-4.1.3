static char sccsid[] = "@(#)53  1.30  src/bos/usr/lib/methods/chgtty/chgtty.c, cfgtty, bos41J, 9520A_all 4/27/95 14:03:58";
/*
 * COMPONENT_NAME: (CFGTTY) chgtty.c  
 *
...
/* This is a copy of chgdevice.c modified to handle the type=E
 * attributes defect 158660
 */

/*
   Interface:

	chgtty -l <logical name> [-p <parent>] [-w <location>]
...
struct	PdDv	preobj,		/* predefined device object         */
           	parpreobj;	/* parent's predefined device object  */
...
struct	objlistinfo list_info;	/* Info about old_attrs */
	...
	/* get parent device's predefined object */
	sprintf(sstring,"uniquetype = '%s'",parobj.PdDvLn_Lvalue);
	rc = (int)odm_get_first(PdDv_CLASS,sstring,&parpreobj);
	if (rc==0) {
		/* failed to get an object */
		DEBUG_0("chgdevice: No PdDv object\n")
		err_exit(E_NOPdDv);
	...
	if ( ( attrs!=NULL ) || new_location) {
		...
		rc = check_parms(attrs,Pflag,(int)0,&cusobj,&parobj,parent,loc,
			badattlist);

		if (rc > 0) {
			DEBUG_0("chgdevice: check_parms() error\n")
			if( badattlist[0] != '\0' )
				list_badattr( &preobj,&parpreobj,badattlist);
			err_exit(rc);
		}
	}
	else	/* Nothing to do */
	...
	if (attrs!=NULL) {
		rc = update_dev(&cusobj, &parobj, attrs);
		if (rc != 0) {
...
int
update_dev(cusobj, parobj, attrs)
struct  CuDv *cusobj;
struct  CuDv *parobj;
struct  attr *attrs;
{
...
int     rc;
	...
	do {
		...
		if (rc == NULL) {
		sprintf(sstring,"uniquetype = '%s' AND attribute = '%s'",
				parobj->PdDvLn_Lvalue,attrs->attribute);
...
list_badattr( pre_dev, parpre_dev, badattr )
struct PdDv * pre_dev;    /* predefined object */
struct PdDv * parpre_dev; /* parent predefined object */
char	*badattr;         /* list of bad attributes */
{
	char	*attrname;
	char    *catalog;
	int     setno;
	char	sstring[100];
	struct	PdAt	PdAtobj;
	struct	PdDv  * tmp_obj; /* current obj */
	int		rc;
	nl_catd cat_fd;
	char cat[80];
	...
	for( attrname = strtok(badattr,", ");
		...
		if (rc==0) {
			/*look under parent */
			sprintf( sstring, "uniquetype = %s AND attribute = %s AND type='E'",parpre_dev->uniquetype, attrname );
			rc = (int)odm_get_first(PdAt_CLASS,sstring,&PdAtobj);
		...
		if ((cat_fd = catopen( tmp_obj->catalog, NL_CAT_LOCALE )) == -1)
		{	/* can't access it - try an explicit path */
			sprintf(cat,"%s/%s",DEFAULT_DEVICES_CAT_DIR,tmp_obj->catalog);
			cat_fd = catopen( cat, NL_CAT_LOCALE );
		}
				
		fprintf( stderr, "     %s     %s\n", attrname,
			catgets( cat_fd,tmp_obj->setno,PdAtobj.nls_index,"" ));
