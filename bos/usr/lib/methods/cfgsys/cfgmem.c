static char sccsid[] = "@(#)14	1.16.1.5  src/bos/usr/lib/methods/cfgsys/cfgmem.c, cfgmethods, bos411, 9428A410j 5/12/94 10:14:48";
/*
...
uint		model;			/* Model type we are dealing with  */
{
   ...
   if (rc == -1)
   {
	if (model & RSC_MODEL)
	{
		/* process memory based on RSC configuration */
		DEBUG_0("Calling cfgmem_RSC\n")
		rc = cfgmem_RSC(pname, iplcb_dir);
	}
	else if (model & (RS1_MODEL | RS1_XIO_MODEL))
	{
		DEBUG_0("Calling cfgmem_RS1\n")
	 	rc = cfgmem_RS1(pname, iplcb_dir, iplcb_info);
	}
	else
	{
	...
	{	
		...
		if (memdata.state != IS_EMPTY)
		{
			...
			if (cudv.status != AVAILABLE)
			{
				...
				/* Reset the vpd array from last memory card. */
				...
				/* set up VPD  			*/
				/* preface size with "SZ" 	*/
				sprintf( tmpstr, "%d", card_size );
				add_descriptor( vpd, "SZ", tmpstr );

				/* preface type with "Z3"	*/
				sprintf( tmpstr, "%02X", memdata.PD_bits );
				add_descriptor( vpd, "Z3", tmpstr );

				/* preface EC level with "EC"	*/
				sprintf( tmpstr, "%02X", memdata.EC_level );
				add_descriptor( vpd, "EC", tmpstr );
				
				/* now write the vpd information out to the database */
				rc = update_vpd(cudv.name,vpd);
 ...
 * cfgmem_RSC
 *
 * FUNCTION: Configures the memory for a RSC model.
 *
 * RETURNS:  error code.  0 means no error.
 */
cfgmem_RSC(pname, iplcb_dir)
char	*pname;				/* parent name, i.e. sys object name */
IPL_DIRECTORY	*iplcb_dir;		/* Pointer to IPL Control block Dir */
{
	struct CuDv	cudv;		/* structure for CuDv object */
	struct PdDv	pddv;
	char	sstring[256];		/* ODM search string */
	char	location[NAMESIZE];	/* storage for location code */
	RAM_DATA	iplcb_simms;	/* RSC SIMM information */
	int	rc;			/* return code */
	int	card_size;		/* Current card's size */
	int	bad_simms;		/* num of bad simms on card */
	int	slot;			/* Slot number, 0-7 */

	memory_model = RSC;

	/* get the pddv for this memory card/simm */
	get_pddv(SIMM, &pddv);
	...
	/* Discover what memory cards/simms are in system */
	for (slot=0; slot<8; slot++) {

		card_size = iplcb_simms.simm_size[slot];
		bad_simms = iplcb_simms.bad_simm_report[slot];

		if (card_size == 0) {
			DEBUG_1("Card in slot %d is not present\n",slot)
			continue;
		}
		DEBUG_1("Card in slot %d is present\n",slot)

		rc = get_cudv(&pddv, memslots[slot],card_size,&cudv,pname);
		if (rc) {
			DEBUG_1("get_cudv failed, rc = %d\n",rc)
			return(rc);
		}
		...
		/* set up location */
		sprintf(location,"00-0%c",memslots[slot]);

		/* update cudv and make available */
		rc = update_cudv(&cudv, location, pname,
			mem_ut, &location[4], TRUE);
		if (rc)
			return(rc);

		/* If bad simms, report it */
		if (bad_simms) {
			DEBUG_1("SIMM in slot %d is bad\n",slot)
			/* Use slot number for simm number too */
			report_bad_simm(&memslots[slot],slot);
		}
	}		
 ...
 * cfgmem_RS1
 *
 * FUNCTION: Configures the memory for an RS1 model.
 *
 * RETURNS:  error code.  0 means no error.
 */
cfgmem_RS1(pname, iplcb_dir, iplcb_info)
char	*pname;				/* parent name, i.e. sys object name */
IPL_DIRECTORY	*iplcb_dir;		/* Pointer to IPL Control block Dir */
IPL_INFO	*iplcb_info;		/* Pointer to IPL Control block Info*/
{
	...
	char	sstring[256];		/* ODM search string */
	int	rc;			/* subroutine return codes */
	char	*bit_map_ptr;		/* ptr to bit map in IPL control blk */
	int	slot;			/* Slot number, 0-7 */
	int	card_size;		/* Current card's size */
	int	card_type;		/* Current card's type */
	int	card_ec;		/* Current card's EC */
	int	bad_bits;		/* total bad bits in extent */
	int	bad_simms;		/* num of bad simms on card */
	int	bad_simm_no;		/* bad simm number */
	char	vpd[VPDSIZE];		/* storage for mem card VPD */
	char	location[NAMESIZE];	/* storage for location code */

	if( iplcb_info->cache_line_size == 128 ) {
		/* 128 byte cache line memory model */
		DEBUG_0("128-byte cache line, 2 cards / extent\n")
		memory_model = cache_128;
	} else if (iplcb_info->IO_planar_level_reg & 0x80000000) {
		/* 64 byte cache line - table top memory model */
		DEBUG_0("64-byte cache line, 1 card / extent (Table model)\n")
		memory_model = cache_64_table;	
	} else {
		/* 64 byte cache line - floor standing memory model */
		DEBUG_0("64-byte cache line, 1 card / extent (Floor model)\n")
		memory_model = cache_64_floor;
	}

	/* get pddv for this memory card */
	get_pddv(CARD, &pddv);

	/* Get RAM bit map from IPL control block */
	bit_map_ptr = malloc(iplcb_dir->bit_map_size);
	rc = mdd_get(bit_map_ptr,iplcb_dir->bit_map_offset,
				iplcb_dir->bit_map_size,MIOIPLCB);
	if (rc) {
		return(rc);
	}
	...
	for (slot=0; slot<8; slot++) {
		...
		card_size = get_card_size(slot,&bad_bits,
					iplcb_info,bit_map_ptr);
		...
		set_msg_no(cudv.name, MEM_UTYPE, card_size);

		if (iplcb_info->soft_ipl_flag == 0) 
		{
			/* The following attributes and VPD apply ONLY	*/
			/* if NOT a warm boot so card VPD will be valid.*/

			/* Set card type attribute */
			card_type = get_desc_val("Z3",slot,iplcb_info);
			sprintf(sstring,"0x%x",card_type);
			setattr(cudv.name,"type",sstring);
	
			/* Set card EC attribute */
			card_ec = get_desc_val("EC",slot,iplcb_info);
			sprintf(sstring,"0x%x",card_ec);
			setattr(cudv.name,"cardec",sstring);
	
			/* Get memory card VPD and save in database */
			memset(vpd, 0, VPDSIZE);
			get_mem_vpd(iplcb_info, slot, card_size, vpd );
			rc = update_vpd(cudv.name,vpd);
		}
		...
		/* Mark CuDv as AVAILABLE */
		rc = update_cudv(&cudv,location,pname,mem_ut,&location[4], TRUE);
		if (rc)
			return(rc);

		/* If bad bits, may need to log an error */
		if (bad_bits > 0) {
			/* scan the simm table for the card */
			bad_simms= find_bad_simms(slot,&bad_simm_no,iplcb_info);
			cbadsimms[slot] = bad_simms;
			DEBUG_1(" Number of bad simms: %d\n", bad_simms)

			if (bad_simms == 1)
				report_bad_simm(&memslots[slot],bad_simm_no);
			else if (bad_simms > 1)
				report_bad_card(&memslots[slot] );
			else {
				/* Bad bits but no bad SIMMs! */
				if (memory_model == cache_128) {
				    if ( slot > 3 ) {
					/* See if other card in pair had bad
					   SIMMs */
					if (cbadsimms[slot-4]==0) {
						/* call out both cards */
						report_pair_failure(slot-4);
					}
				    }
				} else
					report_bad_card(&memslots[slot]);
 ...
 * NAME: get_mem_vpd
 *
 * FUNCTION: Reads the VPD for a memory card.
 *
 * RETURNS: NONE
 */

void
get_mem_vpd( iplcb_info, slot_no, card_size, vpd )
IPL_INFO *iplcb_info;		/* info section of IPL control block */
int slot_no;			/* slot number of card, i.e. 0-7 */
int card_size;			/* size of card in MB */
char *vpd;			/* ptr to where VPD is to be returned */

{
	char	tmpstr[10];	/* temp string storage */
	int	desc_no;	/* VPD descriptor number */
	char	*mnem;		/* ptr to VPD mnemonic */

	for( desc_no = 0; vpd_element[desc_no].vpd_mnem[0] != '\0'; desc_no++ )
	{
		if( !vpd_element[desc_no].store_in_CuVPD )
			continue;

		mnem = vpd_element[desc_no].vpd_mnem;
		sprintf( tmpstr, "%02X", get_desc_val(mnem,slot_no,iplcb_info));
		DEBUG_3("get_mem_vpd(): Card %d *%s=%s\n",slot_no,mnem,tmpstr)
		add_descriptor( vpd, mnem, tmpstr );
	}
	sprintf( tmpstr, "%d", card_size );
	add_descriptor( vpd, "SZ", tmpstr );
	return;
