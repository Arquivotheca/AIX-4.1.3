static char sccsid[] = "@(#)71	1.1  src/bos/usr/lib/methods/defmpqd/defmpqd.c, cfgmethods, bos411, 9428A410j 8/2/92 15:04:19";
/*
 * COMPONENT_NAME: (CFGMETH) defmpqd.c - MPQP Driver Define Method
 *
...
/* interface:
   defmpqd -c <class> -s <subclass> -t <type> -p <parent> -w <connection>
...
int     kflag;                  /* Only one device per parent connection */
	...
	if (errflg) {
		...
		DEBUG_0("defmpqd: command line error\n");
		exit(E_ARGS);
	}
	...
	if (class==NULL || subclass==NULL || type==NULL) {
		DEBUG_0("defmpqd: class, subclass, and type names must be specified\n");
		exit(E_TYPE);
	}
	...
	if (nflag) {
		if (parent != NULL || connect != NULL) {
			DEBUG_0("defmpqd: -p and -w not to be used with this device type\n");
			exit(E_PARENT2);
		}
	} else {
		if (parent == NULL || connect == NULL) {
			DEBUG_0("defmpqd: parent and connection must to be specified\n");
			exit(E_PARENT);
		}
	}
	...
	if (lname!=NULL && uflag!=0) {
		DEBUG_0("defmpqd: Device name cannot be specified for this type\n");
	     /*   exit(E_UNAME);   */
		exit(E_LNAME);
	}
	...
	if (odm_initialize() == -1) {
		...
		DEBUG_0("defmpqd: odm_initialize() failed\n")
		exit(E_ODMINIT);
	}
	...
	if (rc==0) {
		...
		DEBUG_0("defmpqd: No PdDv object\n")
		err_exit(E_NOPdDv);
	}
	else if (rc==-1) {
		/* ODM error */
		DEBUG_0("defmpqd: ODM error getting PdDv object\n")
		err_exit(E_ODMGET);
	}
	...
	if ((int)(cusdev=odm_open_class(CuDv_CLASS)) == -1) {
		/* error opening class */
		DEBUG_0("defmpqd: open of CuDv failed\n")
		err_exit(E_ODMOPEN);
	}
	...
	if (lname == NULL) {
		...
		if ((seqno=genseq(PdDv.prefix))<0) {
			...
			DEBUG_0("defmpqd: error making logical name\n");
			err_exit(E_MAKENAME);
		}
		if (oflag!=0 && seqno>0) {
			/* only one of this type device is allowed */
			DEBUG_0("defmpqd: Not allowed to add another device of this type\n");
		...
		if (rc==-1) {
			/* ODM error */
			DEBUG_0("defmpqd: ODM error getting CuDv object\n")
			err_exit(E_ODMGET);
		}
		else if (rc != 0) {
			/* Device name already in use */
			DEBUG_0("defmpqd: Specified name already in use\n")
		    /*    err_exit(E_NAMEUSED);   */
			err_exit(E_LNAME);
		}
	...
	if (nflag == 0) {
        	...
        	/* read parent attribute from CuAt, and PdAt tables */
            	if ( (( cusatt = get_attrval( parent, "_subtype",
			sstring, &ulong_val, &rc ) ) != NULL ) &&
                	( rc == E_OK ) ) {

                	rc = strcmp( cusatt->value, PM_4PORT_MPQP_MSG );
                	if( rc != 0 ) { /* this is not a MPQP adapter */
                        	DEBUG_2("defmpqd: wrong parent adapter lname=%s _subtype =%s\n", lname,cusatt->value)
                        	err_exit(E_WRONGDEVICE);
                	}
		}
		else {
		/* failed to get the _subtype attribute of the parent adapter */
			DEBUG_0("defmpqd: Failed to get parent _subtype\n")
			err_exit(rc);
		}
		...
		if (rc==0) {
			/* failed to get an object */
			DEBUG_0("defmpqd: No parent CuDv object\n")
		...
		if (rc==0) {
			/* invalid connection */
			DEBUG_0("defmpqd: Invalid connection\n")
		...
		if (kflag != 0) {
			...
			if (rc) {
				...
				DEBUG_0("defmpqd: Already defined device at connection\n")
				err_exit(E_INVCONNECT);
			}
		}
	}
	...
	if (odm_add_obj(cusdev,&CuDv) == -1) {
		/* error: add failed */
		DEBUG_0("defmpqd: ODM error creating CuDv object\n")
		err_exit(E_ODMADD);
	}
	...
	if (odm_close_class(CuDv_CLASS) == -1) {
		/* ODM error */
		DEBUG_0("defmpqd: failure closing CuDv object class\n")
		err_exit(E_ODMCLOSE);
	}
