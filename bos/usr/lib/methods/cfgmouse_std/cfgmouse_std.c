static char sccsid[] = "@(#)60  1.2  src/bos/usr/lib/methods/cfgmouse_std/cfgmouse_std.c, inputdd, bos41J, 9510A_all 3/7/95 09:56:07";
/*
 ...
 *
 * NOTES :
 *	This function gets the values of the attributes for the mouse 
...
{
	struct	ktsmdds	*dds;	  	/* pointer to mouse_dds structure*/
	char	dev_type[20];
	...
	dds->device_class = MOUSE;
	/* set the device type */ 
	if ( !strcmp( dev_type, "mse_3b" ) )
	   dds->device_type = MOUSE3B;
	else
	   if ( !strcmp( dev_type, "mse_2b" ) )
	      dds->device_type = MOUSE2B;
	   else {
 ...
 * NAME     : generate_minor
 *
 * FUNCTION : This function generates a minor number for the mouse
 *
 * RETURNS :
 * 	0 = success
 *	>0 = errno.
 ...
 *
 * FUNCTION : This function creates a special file for the mouse
 *
 * NOTES :
 *	Logical name of mouse device and device number are passed as
...
{
	DEBUG_0(" creating special file for mouse\n")
 ...
 *
 * FUNCTION : This function downloads microcode if applicable 
 *
 * NOTES : There is no microcode provision for mouse devices so this 
 ...
 *
 * NOTES : There is no VPD returned for the mouse
 *
 * RETURNS : 0
 */
 ...
 *
 * NOTES :
 *	The mouse has no children so this routine does nothing
 *
 * RETURNS : 0
 */
