static char sccsid[] = "@(#)58	1.6.1.3	src/bos/usr/lib/pios/fmtrs/piofasci.c, cmdpios, bos411, 9433B411a 8/17/94 12:52:40";
/*
 * COMPONENT_NAME: (CMDPIOS) Printer Backend
 *
 * FUNCTIONS: setup, initialize, lineout, passthru, restore, errorexit
 *
 * ORIGINS: 27
 *
...
/*** piofasci.c - Print formatter for simple ascii data stream ***/
...
/* Fix for Defect 7657 */
/*
#define CHARIN(x)           { x = getc(fileptr); \
                              charcount++; }
*/
...
int substchar;		/* substitute character to use when character in      */
			/* input data stream can't be printed		      */
...
sharevars._vincr_cmd = (&Vincr_cmd)->ptr; /*|vertical increment cmd          |*/
sharevars._ff_cmd    = (&Ff_cmd)->ptr;    /*|form feed command               |*/
...
| NAME:        initialize                                                      |
|                                                                              |
| FUNCTION:    initialize the printer and download fonts if necessary          |
|                                                                              |
| RETURNS:     zero                                                            |
|                                                                              |
+-----------------------------------------------------------------------------*/
...
| NAME:         lineout                                                        |
|                                                                              |
| FUNCTION:     read and handle characters from the data stream                |
|                                                                              |
...
struct stage2_info *cur_infop;	   /*|pointer to stage 2 info. for code      |*/
				   /*|page currently being looked at         |*/
struct transtab *tabentp;	   /*|pointer to stage 2 table entry         |*/
...
for (charcount = 0;;){
	...
	switch (c1) {
		...
		case 11:                               /* '0B' - Vertical Tab */
			...
			/* tab stop found */
			if (tabs[cnt] != 0) vpos = tabval + vtab_base;
...
+-----------------------------------------------------------------------------*/
	...
	if (!(&Codepagename)->len) {pioputchar(c1);} /*No codepage translation*/
			      ...
			      /* Get Ready to Run Circular Chain of Codepages */
		doingsubchar = FALSE;

STARTAGAIN:

		done = FALSE;
					    /* If multiple code pages chained */
		if (sel_infop->infop != sel_infop)
...
+-----------------------------------------------------------------------------*/
		...
		for (cur_infop = sel_infop;
			...
			if (c1 < cur_infop->num_tabentries) {
				...
				tabentp = NULL;
			      /* can't print the character, so use substitute */
				c2 = SC;
			}

			if (c2 < 0) {
				      /* special code instead of a code point */
				switch (c2) {
...
+-----------------------------------------------------------------------------*/
	...
	/* Output the Character and th Optional Prefix and/or Suffix Commands */
...
| NAME:       passthru                                                         |
|                                                                              |
| FUNCTION:   reads characters from the file pointer (could be stdin)          |                                                       |
|             and immediately outputs them - performs no checking of           |
|             any kind                                                         |
|                                                                              |
| RETURNS:    zero                                                             |
|                                                                              |
