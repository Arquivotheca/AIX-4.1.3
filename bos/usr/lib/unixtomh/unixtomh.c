static char sccsid[] = "@(#)12	1.4  src/bos/usr/lib/unixtomh/unixtomh.c, cmdmailx, bos411, 9428A410j 11/12/93 11:00:14";
/* 
 * COMPONENT_NAME: CMDMH unixtomh.c
 * 
 * FUNCTIONS: MSGSTR, Munixtomh, any, cmatch, copy, copyin, dohead, 
 *            fail, gword, isdate, ishead, isname, nextword, 
 *            nullgets, parse, raise, rev, savestr, word 
 *
 * ORIGINS: 10  26  27 
 *
	...
	char **argv;
{
	...
	if (argc > 3) {
		fprintf(stderr,  MSGSTR(USAGE, "Usage: unixtomh name1 name2\n") );
		exit(1);
	}
	outf = inf = NULL;
	if (argc < 3)
	...
	char str[];
{
	...
	if (top == NOSTR) {
		fprintf(stderr,  MSGSTR(NOMEM, "unixtomh:  Ran out of memory\n") );
		exit(1);
	}
	copy(str, top);
	return(top);
