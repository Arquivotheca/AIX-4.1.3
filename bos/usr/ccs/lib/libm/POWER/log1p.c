static char sccsid[] = "@(#)39	1.9  src/bos/usr/ccs/lib/libm/POWER/log1p.c, libm, bos411, 9428A410j 11/29/93 10:34:05";
/*
 * COMPONENT_NAME: (LIBM) math library
 *
 * FUNCTIONS: log1p, loginner
 *
 * ORIGINS: 27
 *
 ...
 * NAME: log1p
 *                                                                    
 * FUNCTION: COMPUTES THE LOG OF (X + 1)
 *                                                                    
 * NOTES:
 *
 * RETURNS: log of (1+x)
 *
 * If x is less than -1, then log returns the value QNaN.
 * If x equals -1, then log returns the value -HUGE_VAL.
 *
 */

double log1p(double zx)
{
	long flag;
	double p, result, tail, fmode;

	flag = (zx == zx);
	flag = (zx > -1.0);
	flag = (zx == DBL_INFINITY);
	if (zx == zx)
	{
		p = 1.0 + zx;
		flag = (p == DBL_INFINITY);
		if (zx > -1.0)
		{
		   ...
		   if (p == DBL_INFINITY)  
			return(p);
		   fmode = __setrnd(0); /* force round-to-near */
					/* must use RIOS encoding - not ANSI */
		   result = loginner(1.0, zx, &tail);
		   __setflm(fmode);         /* restore rounding mode  */
		   return(result);
		}
	}

	/* here for NaN or zx <= -1.0 */

	if (zx == -1.0) 
		return(-DBL_INFINITY);  /* log(0) = -infinity  */

	if (zx < -1.0)
        {
		errno = EDOM;		/* per COSE (UU) API */
		return(DBL_INFINITY - DBL_INFINITY);   /*  log(neg)= NaN; */
	}
	/* if here argument must be NaN */
	return (zx + 1.0);
