static char sccsid[] = "@(#)68	1.4.1.4  src/bos/usr/ccs/lib/libc/ndbm.c, libcndbm, bos411, 9428A410j 1/24/94 11:56:25";
/* 
 * COMPONENT_NAME: (LIBCNDBM) New Data Base Management Library
 * 
 * FUNCTIONS: additem, chkblk, dbm_access, dbm_close, dbm_delete, dbm_fetch, 
 *            dbm_firstkey, dbm_forder, dbm_nextkey, dbm_open, dbm_store, 
 *            dcalchash, delitem, finddatum, getbit, hashinc, makdatum, 
 *            setbit 
 *
 * ORIGINS: 26 
 *
	...
	int flags, mode;
{
	...
	/* audit: FILE_Open generates too many failure entries */
        if ( !(flags & (O_CREAT | O_RDWR | O_WRONLY)) &&
        	accessx(db->dbm_pagbuf, R_ACC, ACC_SELF) )
		db->dbm_pagf = -1;
	else
...
loop:
	...
	if (!additem(db->dbm_pagbuf, key, dat))
		goto split;
	db->dbm_pagbno = db->dbm_blkno;
	if (lseek(db->dbm_pagf, db->dbm_blkno*PBLKSIZ, L_SET) != db->dbm_blkno*PBLKSIZ) {
		/*
		 * lseek failed.  assume we tried to go past the maximum
		 * filesize.  recover, but return an error to let the caller
		 * know this store failed.  however, don't mark the db as
		 * IOERR since we could theoretically add another record
		 * somewhere else later...
		 */
		db->dbm_pagbno = -1;	/* mark page buf invalid */
		errno = EFBIG;
		return (-1);
		}
	if (write(db->dbm_pagf, db->dbm_pagbuf, PBLKSIZ) != PBLKSIZ) {
...
split:
	...
	/*
	 * attempt to write the overflow buffer first so we can save
	 * the original buffer....
	 */
	if (lseek(db->dbm_pagf, (db->dbm_blkno+db->dbm_hmask+1)*PBLKSIZ, L_SET) !=
	     (off_t)(db->dbm_blkno+db->dbm_hmask+1)*PBLKSIZ) {
		/*
		 * lseek failed.  assume we tried to go past the maximum
		 * filesize.  recover, but return an error to let the caller
		 * know this store failed.  however, don't mark the db as
