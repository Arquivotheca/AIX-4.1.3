static char sccsid[] = "@(#)64	1.6.1.7  src/bos/usr/ccs/lib/libc/__fnmatch_sb.c, libcpat, bos411, 9428A410j 5/4/94 21:53:57";
/*
...
int
__fnmatch_sb(_LC_collate_objhdl_t hdl, const char *ppat, const char *string, 
 ...
 */
		case '*':
			...
			while (*pstr != '\0')
				{
				stat = __fnmatch_sb(hdl, ppat, string, pstr,
						    flags);
				if (stat != FNM_NOMATCH)
					return (stat);
				if (*pstr == '/')
 ...
 */
			stat = bracket(hdl, ppat+1, &ppat, (wchar_t)*pstr, 
				       ucoll_s, flags);
			if (stat == 0)
				pstr = pse;
			else if (stat > 0)
