static char sccsid[] = "@(#)49	1.9  src/bos/usr/ccs/lib/libc/random.c, libcgen, bos411, 9428A410j 1/7/94 16:27:57";
/*
...
static int __randtbl_size = sizeof(randtbl);
 ...
 * The function changed from return type 'int' to return type 'void'
 * by the COSE (UU) standard.  It never did return anything meaningful.
 */
...
srandom(unsigned x)
{
	...
	TS_EINVAL(rand_data == 0);

#ifdef _THREAD_SAFE
	/**********
	  If this is the first time in srandom_r , get space for the table
	  and set it up
	**********/
	if ( rand_data->state == NULL ) {
		if ((randtbl_ptr = malloc(__randtbl_size)) == NULL) {
			errno = EINVAL;
			return (-1);
		}
		memcpy(randtbl_ptr, randtbl, __randtbl_size );

		initstate_r(1, randtbl_ptr, __randtbl_size, &retval, rand_data);
	}
#endif /* _THREAD_SAFE */
...
#ifdef _THREAD_SAFE
int
random_r(long *retval, struct random_data *rand_data)
#else
long
random(void)
#endif	/* _THREAD_SAFE */
...
#ifdef _THREAD_SAFE
	/**********
	  If this is the first time in random_r, get space for the table
	  and set it up
	**********/
	if ( rand_data->state == NULL ) {
		if ((randtbl_ptr = malloc(__randtbl_size)) == NULL) {
		...
		initstate_r(1, randtbl_ptr, __randtbl_size, &retval_ptr, rand_data);
	}
#endif /* _THREAD_SAFE */
