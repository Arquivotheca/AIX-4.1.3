static char sccsid[] = "@(#)66        1.24  src/bos/usr/ccs/lib/libc/ruserpass.c, libcnet, bos411, 9439B411a 9/28/94 17:30:37";
/* 
 * COMPONENT_NAME: LIBCNET ruserpass.c
 * 
 * FUNCTIONS: blkencrypt, deblkclr, deblknot, enblkclr, enblknot, 
 *            getutmp, mkenvkey, mkpwclear, mkpwunclear, nbs8decrypt, 
 *            nbs8encrypt, nbsdecrypt, nbsencrypt, nbssetkey, renv, 
 *            renvlook, rnetrc, ruserpass, sreverse, token 
 *
 * ORIGINS: 26  27  71
 *
 * (C) COPYRIGHT International Business Machines Corp. 1985, 1994
 * All Rights Reserved
 * Licensed Materials - Property of IBM
 *
 * US Government Users Restricted Rights - Use, duplication or
 * disclosure restricted by GSA ADP Schedule Contract with IBM Corp.
 *
 * Copyright (c) 1983 Regents of the University of California.
 * All rights reserved.  The Berkeley software License Agreement
 * specifies the terms and conditions for redistribution.
 *
 * (c) Copyright 1990, 1991, 1992, 1993 OPEN SOFTWARE FOUNDATION, INC.
 * ALL RIGHTS RESERVED
 *
 */

#include <sys/types.h>
#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <utmp.h>
#include <ctype.h>
#include <sys/stat.h>
#include <errno.h>
#include <sys/access.h>
#include <nl_types.h>
#include <netdb.h>
#include <arpa/nameser.h>
#include <sys/socket.h>
#include "libc_msg.h"

#include "ts_supp.h"
#include "push_pop.h"

#ifdef _THREAD_SAFE
#include "rec_mutex.h"
extern struct rec_mutex _environ_rmutex;
extern struct rec_mutex _netrc_rmutex;
#endif /* _THREAD_SAFE */

int netrc_restricted = 0;

void 	mkpwclear(char *, char, char *);
void	mkpwunclear(char *, char, char *);
char	*renvlook(), *index();

static	renv(), rnetrc(), token(), getutmp();
void	enblkclr(), enblknot(), blkencrypt(), nbssetkey();

check_netrc(char *host, char **aname, char **apass)
{

	TS_LOCK(&_netrc_rmutex);
	if (!netrc_restricted) {
		TS_PUSH_CLNUP(&_netrc_rmutex);
		renv(host, aname, apass);
		if (*aname == 0 || *apass == 0)
			rnetrc(host, aname, apass);
		TS_POP_CLNUP(0);
	}
	TS_UNLOCK(&_netrc_rmutex);
}

void
ruserpass(char *host, char **aname, char **apass)
{
	nl_catd catd;
#ifdef _THREAD_SAFE
	char myname_buf[64];
	char *myname = myname_buf;
#endif /* _THREAD_SAFE */

	check_netrc(host, aname, apass);
	if (*aname == 0) {
#ifdef _THREAD_SAFE
		getlogin_r(myname_buf, 64);
#else
		char *myname = getlogin();
#endif /* _THREAD_SAFE */
		if (!(*aname = malloc(64)))
			perror("ruserpass:malloc() failed");
                catd = catopen(MF_LIBC,NL_CAT_LOCALE);
		printf(catgets(catd,LIBCNET,NET33,
					"Name (%s:%s): "), host, myname);
		fflush(stdout);
		if (read(2, *aname, (unsigned)64) <= 0)
			exit(1);
		if ((*aname)[0] == '\n') {
#ifdef _THREAD_SAFE
			strncpy(*aname, myname_buf, 64);
		}
#else
			free(aname);
			*aname = myname;
		}
#endif /* _THREAD_SAFE */
		else
			if (index(*aname, '\n'))
				*index(*aname, '\n') = 0;
	}
	if (*aname && *apass == 0) {
#ifdef _THREAD_SAFE
		/*
		 * Since getpass() is not a thread-safe function, 
		 * set password to NULL.
		 */
		*apass = (char *)0;
#else
                catd = catopen(MF_LIBC,NL_CAT_LOCALE);
		printf(catgets(catd,LIBCNET,NET34,
					"Password (%s:%s): "), host, *aname);
		fflush(stdout);
		*apass = getpass("");
#endif /* _THREAD_SAFE */
	}
}

static
renv(char *host, char **aname, char **apass)
{
	register char *cp;
	char *stemp, fgetlogin, *comma;

	cp = renvlook(host);
	if (cp == NULL)
		return;
	if (!isalpha(cp[0]))
		return;
	comma = index(cp, ',');
	if (comma == 0)
		return;
	if (*aname == 0) {
		if (!(*aname = malloc(comma - cp + 1)))
			perror("renv:malloc() failed");
		strncpy(*aname, cp, (size_t)(comma - cp));
	} else
		if (strncmp(*aname, cp, (size_t)(comma - cp)))
			return(0);
	comma++;
	if (!(cp = malloc(strlen(comma)+1)))
		perror("renv:malloc() failed");	
	strcpy(cp, comma);
	if (!(*apass = malloc(16)))
		perror("renv:malloc() failed");	
	mkpwclear(cp, host[0], *apass);
	return(0);
}

static
char *
renvlook(char *host)
{
	register char *cp, **env;
	extern char **environ;


	TS_LOCK(&_environ_rmutex);
	env = environ;
	for (env = environ; *env != NULL; env++)
		if (!strncmp(*env, "MACH", (size_t)4)) {
			cp = index(*env, '=');
			if (cp == 0)
				continue;
			if (strncmp(*env+4, host, (size_t)(cp-(*env+4))))
				continue;
			TS_UNLOCK(&_environ_rmutex);
			return (cp+1);
		}
		TS_UNLOCK(&_environ_rmutex);
	return (NULL);
}

#define	DEFAULT	1
#define	LOGIN	2
#define	PASSWD	3
#define	NOTIFY	4
#define	WRITE	5
#define	YES	6
#define	NO	7
#define	COMMAND	8
#define	FORCE	9
#define	ID	10
#define	MACH	11

static char tokval[MAXDNAME];

static struct toktab {
	char *tokstr;
	int tval;
} toktab[]= {
	"default",	DEFAULT,
	"login",	LOGIN,
	"password",	PASSWD,
	"notify",	NOTIFY,
	"write",	WRITE,
	"yes",		YES,
	"y",		YES,
	"no",		NO,
	"n",		NO,
	"command",	COMMAND,
	"force",	FORCE,
	"machine",	MACH,
	0,		0
};

static int
rnetrc(char *host, char** aname, char **apass)
{
	char *hdir, buf[BUFSIZ];
	int t;
	struct stat stb;
	char ldomain[MAXDNAME], *domainp = NULL, lhost[MAXDNAME];
	long addr;
	struct hostent *hp;
	nl_catd catd;
#ifdef _THREAD_SAFE
	struct hostent htent;
	struct hostent_data ht_data;
#endif /* _THREAD_SAFE */
	
	char tokval[MAXDNAME];
	FILE *cfile;
	
	hdir = getenv("HOME");
	if (hdir == NULL)
		hdir = ".";
	sprintf(buf, "%s/.netrc", hdir);
	cfile = fopen(buf, "r");
	if (cfile == NULL) {
		if (errno != ENOENT)
			perror(buf);
		return(0);
	}

	/*
	 * get local domain name
	 */
	if (!gethostname(ldomain, sizeof(ldomain))) {
		ldomain[sizeof(ldomain)-1] = '\0';
		domainp = index(ldomain, '.');
	}

	/*
	 * check for ip address and do a lookup to convert to hostname
	 */
#ifdef _THREAD_SAFE
	bzero(&ht_data, sizeof(ht_data));
#define GETHOSTBYADDR(a, l, t)	(hp = &htent, gethostbyaddr_r(a, l, t, hp, &ht_data))
#else 
#define	GETHOSTBYADDR(a, l, t)	(hp = gethostbyaddr(a, l, t))
#endif /* _THREAD_SAFE */

	if (isinet_addr(host) && (addr = inet_addr(host)) != -1 &&
	    (GETHOSTBYADDR(&addr, sizeof(addr), AF_INET) != TS_FAILURE)) {
		strcpy(lhost, hp->h_name);
		host = lhost;
	}

	/*
	 * add local domain to domainless hostname
	 */
	if (domainp && !index(host, '.')) {
		strcpy(lhost, host);
		strcat(lhost, domainp);
		host = lhost;
	}

next:
	while ((t = token(cfile, tokval))) switch(t) {

	case DEFAULT:
		(void) token(cfile, tokval);
		continue;

	case MACH:
		if (token(cfile, tokval) != ID)
			continue;

		/*
		 * NB: if we change the hostname-comparison logic here,
		 * do the same in sockcmd/ftp/ruserpass.c, so that ftp
		 * and rexec process the .netrc file the same way.
		 */

		/*
		 * check for ip address and do a lookup to convert to hostname
		 */
		if (isinet_addr(tokval) && (addr = inet_addr(tokval)) != -1
		    && (GETHOSTBYADDR(&addr, sizeof(addr), AF_INET) != TS_FAILURE))
			strcpy(tokval, hp->h_name);

		/*
		 * add local domain to domainless hostname
		 */
		if (domainp && !index(tokval, '.'))
			strcat(tokval, domainp);

		if (strcasecmp(host, tokval))
			continue;

		while ((t = token(cfile, tokval)) && t != MACH) switch(t) {

		case LOGIN:
			if (token(cfile, tokval))
				if (*aname == 0) { 
					if (!(*aname = malloc(strlen(tokval) + 1)))
						perror("rnetrc:malloc() failed");
					strcpy(*aname, tokval);
				} else {
					if (strcmp(*aname, tokval))
						goto next;
				}
			break;
		case PASSWD:
			if ((faccessx(fileno(cfile), R_ACC, ACC_OTHERS) == 0) ||
			    (faccessx(fileno(cfile), W_ACC, ACC_OTHERS) == 0) ||
			    (faccessx(fileno(cfile), X_ACC, ACC_OTHERS) == 0)) {
                		catd = catopen(MF_LIBC,NL_CAT_LOCALE);
				fprintf(stderr, catgets(catd,LIBCNET,NET29,
				   "Error - .netrc file not correct mode.\n"));
				fprintf(stderr, catgets(catd,LIBCNET,NET30,
				   "Remove password or correct mode.\n"));
				exit(1);
			}
			if (token(cfile, tokval) && *apass == 0) {
				if (!(*apass = malloc(strlen(tokval) + 1)))
					perror("rnetrc:malloc() failed");
				strcpy(*apass, tokval);
			}
			break;
		case COMMAND:
		case NOTIFY:
		case WRITE:
		case FORCE:
			(void) token(cfile, tokval);
			break;
		default:
                	catd = catopen(MF_LIBC,NL_CAT_LOCALE);
			fprintf(stderr, catgets(catd,LIBCNET,NET31,
					"Unknown .netrc option %s\n"), tokval);
			break;
		}
		goto done;
	}
done:
	fclose(cfile);
	return (0);
}

static int
token(FILE *cfile, char *tokval)
{
	char *cp;
	int c;
	struct toktab *t;

	if (feof(cfile))
		return (0);
	while ((c = getc(cfile)) != EOF &&
	    (c == '\n' || c == '\t' || c == ' '))
		continue;
	if (c == EOF)
		return (0);
	cp = tokval;
	if (c == '"') {
		while ((c = getc(cfile)) != EOF && c != '"') {
			if (c == '\\')
				c = getc(cfile);
			*cp++ = c;
		}
	} else {
		*cp++ = c;
		while ((c = getc(cfile)) != EOF
		    && c != '\n' && c != '\t' && c != ' ') {
			if (c == '\\')
				c = getc(cfile);
			*cp++ = c;
		}
	}
	*cp = 0;
	if (tokval[0] == 0)
		return (0);
	for (t = toktab; t->tokstr; t++)
		if (!strcmp(t->tokstr, tokval))
			return (t->tval);
	return (ID);
}
/* rest is nbs.c stolen from berknet */

char *deblknot(), *deblkclr();
char *nbs8decrypt(), *nbs8encrypt();

/*
 * The E bit-selection table.
 */
static char	e[] = {
	32, 1, 2, 3, 4, 5,
	 4, 5, 6, 7, 8, 9,
	 8, 9,10,11,12,13,
	12,13,14,15,16,17,
	16,17,18,19,20,21,
	20,21,22,23,24,25,
	24,25,26,27,28,29,
	28,29,30,31,32, 1,
};

static char *
nbsencrypt(char *str, char *key, char *result)
{ 
	char buf[20],oldbuf[20];
	char retbuf[30];
	register int j;
	result[0] = 0;
	strcpy(oldbuf,key);

	while(*str){
		for(j=0;j<10;j++)buf[j] = 0;
		for(j=0;j<8 && *str;j++)buf[j] = *str++;
		strcat(result,nbs8encrypt(buf,oldbuf,retbuf));
		strcat(result,"$");
		strcpy(oldbuf,buf);
		}
	return(result);
}

static char *
nbsdecrypt(char *cpt, char *key, char *result)
{
	char *s;
	char c,oldbuf[20];
	char retbuf[30];
	result[0] = 0;
	strcpy(oldbuf,key);
	while(*cpt){
		for(s = cpt;*s && *s != '$';s++);
		c = *s;
		*s = 0;
		strcpy(oldbuf,nbs8decrypt(cpt,oldbuf,retbuf));
		strcat(result,oldbuf);
		if(c == 0)break;
		cpt = s + 1;
		}
	return(result);
}

static char *
nbs8encrypt(char *str, char *key, char *retbuf)
{
	static char keyblk[100], blk[100], E[48];
	/*
	 * The key schedule.
	 * Generated from the key.
	 */
	char KS[16][48];
	register int i;

	enblkclr(keyblk,key);
	nbssetkey(keyblk, KS);

	for(i=0;i<48;i++) E[i] = e[i];
	enblkclr(blk,str);
	blkencrypt(blk,0,KS,E);			/* forward dir */

	return(deblknot(blk,retbuf));
}

static char *
nbs8decrypt(char *crp, char *key, char *retbuf)
{
	char keyblk[100], blk[100], KS[16][48], E[48];
	register int i;

	enblkclr(keyblk,key);
	nbssetkey(keyblk, KS);

	for(i=0;i<48;i++) E[i] = e[i];
	enblknot(blk,crp);
	blkencrypt(blk,1,KS,E);			/* backward dir */

	return(deblkclr(blk,retbuf));
}

static void
enblkclr(char *blk, char *str)		/* ignores top bit of chars in string str */
{
	register int i,j;
	char c;

	for(i=0;i<70;i++)blk[i] = 0;
	for(i=0; (c= *str) && i<64; str++){
		for(j=0; j<7; j++, i++)
			blk[i] = (c>>(6-j)) & 01;
		i++;
	}
}

static char *
deblkclr(char *blk, char *iobuf)
{
	register int i,j;
	char c;

	for(i=0; i<10; i++){
		c = 0;
		for(j=0; j<7; j++){
			c <<= 1;
			c |= blk[8*i+j];
			}
		iobuf[i] = c;
	}
	iobuf[i] = 0;
	return(iobuf);
}

static void
enblknot(char *blk, char *crp)
{
	register int i,j;
	char c;

	for(i=0;i<70;i++)blk[i] = 0;
	for(i=0; (c= *crp) && i<64; crp++){
		if(c>'Z') c -= 6;
		if(c>'9') c -= 7;
		c -= '.';
		for(j=0; j<6; j++, i++)
			blk[i] = (c>>(5-j)) & 01;
	}
}

static char *
deblknot(char *blk, char *iobuf)
{
	register int i,j;
	char c;

	for(i=0; i<11; i++){
		c = 0;
		for(j=0; j<6; j++){
			c <<= 1;
			c |= blk[6*i+j];
			}
		c += '.';
		if(c > '9')c += 7;
		if(c > 'Z')c += 6;
		iobuf[i] = c;
	}
	iobuf[i] = 0;
	return(iobuf);
}

/*
 * This program implements the
 * Proposed Federal Information Processing
 *  Data Encryption Standard.
 * See Federal Register, March 17, 1975 (40FR12134)
 */

/*
 * Initial permutation,
 */
static	char	IP[] = {
	58,50,42,34,26,18,10, 2,
	60,52,44,36,28,20,12, 4,
	62,54,46,38,30,22,14, 6,
	64,56,48,40,32,24,16, 8,
	57,49,41,33,25,17, 9, 1,
	59,51,43,35,27,19,11, 3,
	61,53,45,37,29,21,13, 5,
	63,55,47,39,31,23,15, 7,
};

/*
 * Final permutation, FP = IP^(-1)
 */
static	char	FP[] = {
	40, 8,48,16,56,24,64,32,
	39, 7,47,15,55,23,63,31,
	38, 6,46,14,54,22,62,30,
	37, 5,45,13,53,21,61,29,
	36, 4,44,12,52,20,60,28,
	35, 3,43,11,51,19,59,27,
	34, 2,42,10,50,18,58,26,
	33, 1,41, 9,49,17,57,25,
};

/*
 * Permuted-choice 1 from the key bits
 * to yield C and D.
 * Note that bits 8,16... are left out:
 * They are intended for a parity check.
 */
static	char	PC1_C[] = {
	57,49,41,33,25,17, 9,
	 1,58,50,42,34,26,18,
	10, 2,59,51,43,35,27,
	19,11, 3,60,52,44,36,
};

static	char	PC1_D[] = {
	63,55,47,39,31,23,15,
	 7,62,54,46,38,30,22,
	14, 6,61,53,45,37,29,
	21,13, 5,28,20,12, 4,
};

/*
 * Sequence of shifts used for the key schedule.
*/
static	char	shifts[] = {
	1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1,
};

/*
 * Permuted-choice 2, to pick out the bits from
 * the CD array that generate the key schedule.
 */
static	char	PC2_C[] = {
	14,17,11,24, 1, 5,
	 3,28,15, 6,21,10,
	23,19,12, 4,26, 8,
	16, 7,27,20,13, 2,
};

static	char	PC2_D[] = {
	41,52,31,37,47,55,
	30,40,51,45,33,48,
	44,49,39,56,34,53,
	46,42,50,36,29,32,
};

/*
 * Set up the key schedule from the key.
 */

static void
nbssetkey(char *key, char KS[16][48])
{
	register i, j, k;
	int t;
	/*
	 * The C and D arrays used to calculate the key schedule.
	 */
	char C[28];
	char D[28];

	/*
	 * First, generate C and D by permuting
	 * the key.  The low order bit of each
	 * 8-bit char is not used, so C and D are only 28
	 * bits apiece.
	 */
	for (i=0; i<28; i++) {
		C[i] = key[PC1_C[i]-1];
		D[i] = key[PC1_D[i]-1];
	}
	/*
	 * To generate Ki, rotate C and D according
	 * to schedule and pick up a permutation
	 * using PC2.
	 */
	for (i=0; i<16; i++) {
		/*
		 * rotate.
		 */
		for (k=0; k<shifts[i]; k++) {
			t = C[0];
			for (j=0; j<28-1; j++)
				C[j] = C[j+1];
			C[27] = t;
			t = D[0];
			for (j=0; j<28-1; j++)
				D[j] = D[j+1];
			D[27] = t;
		}
		/*
		 * get Ki. Note C and D are concatenated.
		 */
		for (j=0; j<24; j++) {
			KS[i][j] = C[PC2_C[j]-1];
			KS[i][j+24] = D[PC2_D[j]-28-1];
		}
	}
}


/*
 * The 8 selection functions.
 * For some reason, they give a 0-origin
 * index, unlike everything else.
 */
static	char	S[8][64] = {
	14, 4,13, 1, 2,15,11, 8, 3,10, 6,12, 5, 9, 0, 7,
	 0,15, 7, 4,14, 2,13, 1,10, 6,12,11, 9, 5, 3, 8,
	 4, 1,14, 8,13, 6, 2,11,15,12, 9, 7, 3,10, 5, 0,
	15,12, 8, 2, 4, 9, 1, 7, 5,11, 3,14,10, 0, 6,13,

	15, 1, 8,14, 6,11, 3, 4, 9, 7, 2,13,12, 0, 5,10,
	 3,13, 4, 7,15, 2, 8,14,12, 0, 1,10, 6, 9,11, 5,
	 0,14, 7,11,10, 4,13, 1, 5, 8,12, 6, 9, 3, 2,15,
	13, 8,10, 1, 3,15, 4, 2,11, 6, 7,12, 0, 5,14, 9,

	10, 0, 9,14, 6, 3,15, 5, 1,13,12, 7,11, 4, 2, 8,
	13, 7, 0, 9, 3, 4, 6,10, 2, 8, 5,14,12,11,15, 1,
	13, 6, 4, 9, 8,15, 3, 0,11, 1, 2,12, 5,10,14, 7,
	 1,10,13, 0, 6, 9, 8, 7, 4,15,14, 3,11, 5, 2,12,

	 7,13,14, 3, 0, 6, 9,10, 1, 2, 8, 5,11,12, 4,15,
	13, 8,11, 5, 6,15, 0, 3, 4, 7, 2,12, 1,10,14, 9,
	10, 6, 9, 0,12,11, 7,13,15, 1, 3,14, 5, 2, 8, 4,
	 3,15, 0, 6,10, 1,13, 8, 9, 4, 5,11,12, 7, 2,14,

	 2,12, 4, 1, 7,10,11, 6, 8, 5, 3,15,13, 0,14, 9,
	14,11, 2,12, 4, 7,13, 1, 5, 0,15,10, 3, 9, 8, 6,
	 4, 2, 1,11,10,13, 7, 8,15, 9,12, 5, 6, 3, 0,14,
	11, 8,12, 7, 1,14, 2,13, 6,15, 0, 9,10, 4, 5, 3,

	12, 1,10,15, 9, 2, 6, 8, 0,13, 3, 4,14, 7, 5,11,
	10,15, 4, 2, 7,12, 9, 5, 6, 1,13,14, 0,11, 3, 8,
	 9,14,15, 5, 2, 8,12, 3, 7, 0, 4,10, 1,13,11, 6,
	 4, 3, 2,12, 9, 5,15,10,11,14, 1, 7, 6, 0, 8,13,

	 4,11, 2,14,15, 0, 8,13, 3,12, 9, 7, 5,10, 6, 1,
	13, 0,11, 7, 4, 9, 1,10,14, 3, 5,12, 2,15, 8, 6,
	 1, 4,11,13,12, 3, 7,14,10,15, 6, 8, 0, 5, 9, 2,
	 6,11,13, 8, 1, 4,10, 7, 9, 5, 0,15,14, 2, 3,12,

	13, 2, 8, 4, 6,15,11, 1,10, 9, 3,14, 5, 0,12, 7,
	 1,15,13, 8,10, 3, 7, 4,12, 5, 6,11, 0,14, 9, 2,
	 7,11, 4, 1, 9,12,14, 2, 0, 6,10,13,15, 3, 5, 8,
	 2, 1,14, 7, 4,10, 8,13,15,12, 9, 0, 3, 5, 6,11,
};

/*
 * P is a permutation on the selected combination
 * of the current L and key.
 */
static	char	P[] = {
	16, 7,20,21,
	29,12,28,17,
	 1,15,23,26,
	 5,18,31,10,
	 2, 8,24,14,
	32,27, 3, 9,
	19,13,30, 6,
	22,11, 4,25,
};

/*
 * The payoff: encrypt a block.
 */

static void
blkencrypt(char *block, int edflag, char KS[16][48], char *E)
{
	int i, ii;
	register t, j, k;

	/*
	 * The combination of the key and the input, before selection.
	 */
	char	preS[48];

	/*
	 * The current block, divided into 2 halves.
	 */
	char	L[32], R[32];
	char	tempL[32];
	char	f[32];

	/*
	 * First, permute the bits in the input
	 */
	for (j=0; j<64; j++)
		L[j] = block[IP[j]-1];
	/*
	 * Perform an encryption operation 16 times.
	 */
	for (ii=0; ii<16; ii++) {
		/*
		 * Set direction
		 */
		if (edflag)
			i = 15-ii;
		else
			i = ii;
		/*
		 * Save the R array,
		 * which will be the new L.
		 */
		for (j=0; j<32; j++)
			tempL[j] = R[j];
		/*
		 * Expand R to 48 bits using the E selector;
		 * exclusive-or with the current key bits.
		 */
		for (j=0; j<48; j++)
			preS[j] = R[E[j]-1] ^ KS[i][j];
		/*
		 * The pre-select bits are now considered
		 * in 8 groups of 6 bits each.
		 * The 8 selection functions map these
		 * 6-bit quantities into 4-bit quantities
		 * and the results permuted
		 * to make an f(R, K).
		 * The indexing into the selection functions
		 * is peculiar; it could be simplified by
		 * rewriting the tables.
		 */
		for (j=0; j<8; j++) {
			t = 6*j;
			k = S[j][(preS[t+0]<<5)+
				(preS[t+1]<<3)+
				(preS[t+2]<<2)+
				(preS[t+3]<<1)+
				(preS[t+4]<<0)+
				(preS[t+5]<<4)];
			t = 4*j;
			f[t+0] = (k>>3)&01;
			f[t+1] = (k>>2)&01;
			f[t+2] = (k>>1)&01;
			f[t+3] = (k>>0)&01;
		}
		/*
		 * The new R is L ^ f(R, K).
		 * The f here has to be permuted first, though.
		 */
		for (j=0; j<32; j++)
			R[j] = L[j] ^ f[P[j]-1];
		/*
		 * Finally, the new L (the original R)
		 * is copied back.
		 */
		for (j=0; j<32; j++)
			L[j] = tempL[j];
	}
	/*
	 * The output L and R are reversed.
	 */
	for (j=0; j<32; j++) {
		t = L[j];
		L[j] = R[j];
		R[j] = t;
	}
	/*
	 * The final output
	 * gets the inverse permutation of the very original.
	 */
	for (j=0; j<64; j++)
		block[j] = L[FP[j]-1];
}
/*
	getutmp()
	return a pointer to the system utmp structure associated with
	terminal sttyname, e.g. "/dev/tty3"
	Is version independent-- will work on v6 systems
	return NULL if error
*/
static int
getutmp(char *sttyname, struct utmp *utmpstr_ptr)
{
	FILE *fdutmp;

	if(sttyname == NULL || sttyname[0] == 0)
		return(-1);

	fdutmp = fopen("/etc/utmp","r");
	if(fdutmp == NULL)
		return(-1);

	while(fread((void *)utmpstr_ptr,(size_t)1,(size_t)sizeof(struct utmp),fdutmp) == sizeof(struct utmp))
		if(strcmp(utmpstr_ptr->ut_line,sttyname+5) == 0){
			fclose(fdutmp);
			return(0);
		}
	fclose(fdutmp);
	return(-1);
}

static void
sreverse(register char *sto, register char *sfrom)
{
	register int i;

	i = strlen(sfrom);
	while (i >= 0)
		*sto++ = sfrom[i--];
}

#ifdef _THREAD_SAFE
#define TTYNAME(n, s)	ttyname_r(n, s, sizeof(s))
#else
#define TTYNAME(n, s)	strcpy(s, ttyname(n))
#endif /* _THREAD_SAFE */

static char *
mkenvkey(char mch, char *skey)
{
	struct utmp putmp;
	char stemp[40], stemp1[40], sttyname[30];
	register char *sk,*p;

	if (isatty(2))
		TTYNAME(2, sttyname);
	else if (isatty(0))
		TTYNAME(0, sttyname);
	else if (isatty(1))
		TTYNAME(1, sttyname);
	else
		return (NULL);
	if (getutmp(sttyname, &putmp) != 0)
		return (NULL);
	sk = skey;
	p = putmp.ut_line;
	while (*p)
		*sk++ = *p++;
	*sk++ = mch;
	sprintf(stemp, "%ld", putmp.ut_time);
	sreverse(stemp1, stemp);
	p = stemp1;
	while (*p)
		*sk++ = *p++;
	*sk = 0;
	return (skey);
}

void
mkpwunclear(char *spasswd, char mch, char *sencpasswd)
{
	char skey_buf[40];
	register char *skey;
	nl_catd catd;

	if (spasswd[0] == 0) {
		sencpasswd[0] = 0;
		return;
	}
	skey = mkenvkey(mch, skey_buf);
	if (skey == NULL) {
                catd = catopen(MF_LIBC,NL_CAT_LOCALE);
		fprintf(stderr, catgets(catd,LIBCNET,NET32,"Can't make key\n"));
		exit(1);
	}
	nbsencrypt(spasswd, skey, sencpasswd);
}

void
mkpwclear(char *sencpasswd,char mch, char *spasswd)
{
	char skey_buf[40];
	register char *skey;
	nl_catd catd;

	if (sencpasswd[0] == 0) {
		spasswd[0] = 0;
		return;
	}
	skey = mkenvkey(mch, skey_buf);
	if (skey == NULL) {
                catd = catopen(MF_LIBC,NL_CAT_LOCALE);
		fprintf(stderr, catgets(catd,LIBCNET,NET32,"Can't make key\n"));
		exit(1);
	}
	nbsdecrypt(sencpasswd, skey, spasswd);
}
