static char sccsid[] = "@(#)66	1.4.2.5  src/bos/usr/ccs/lib/libc/__strcoll_sb.c, libcstr, bos411, 9428A410j 5/4/94 16:04:37";
/*
...
int backward_collate_sb(_LC_collate_objhdl_t hdl, char *str1,
			    char *str2, int order);

int fast_backward_collate_sb(_LC_collate_objhdl_t hdl, char *str1,
			    char *str2, int order);

int forward_collate_sb(_LC_collate_objhdl_t hdl, char *str1,
			   char *str2, int order);

int fast_forward_collate_sb(_LC_collate_objhdl_t hdl, char *str1,
			   char *str2, int order);
...
int __strcoll_sb(_LC_collate_objhdl_t hdl, const char *str1, const char *str2)
{
	...
	sort_mod = __OBJ_DATA(hdl)->co_sort.n[cur_order];
        ...
        **********/
        if (__OBJ_DATA(hdl)->co_nsubs && !(sort_mod & _COLL_NOSUBS_MASK)) {
            ...
            /**********
              alloca 2 * the length of the string if it has not been done
            **********/
            if (! str1_rep) {
	...
	**********/
	if (sort_mod == 0) {
	    rc = char_collate_sb(hdl, str1, str2);
	    return(rc);
	}
	...
	**********/
		...
		if (fast_locale)
		    rc = fast_back_pos_collate_sb(hdl, str1, str2, cur_order);
		else
		    rc = back_pos_collate_sb(hdl, str1, str2, cur_order);
		}
	    else {
		if (fast_locale)
		    rc = fast_backward_collate_sb(hdl, str1, str2, cur_order);
		else
		    rc = backward_collate_sb(hdl, str1, str2, cur_order);
		}
	}
	...
	**********/
	    ...
	    if (sort_mod & _COLL_POSITION_MASK) {
		if (fast_locale)
		    rc = fast_forw_pos_collate_sb(hdl, str1, str2, cur_order);
		else
		    rc = forw_pos_collate_sb(hdl, str1, str2, cur_order);
		}
	    else {
		if (fast_locale)
		    rc = fast_forward_collate_sb(hdl, str1, str2, cur_order);
		else
		    rc = forward_collate_sb(hdl, str1, str2, cur_order);
		}
	}
...
/* The backward_collate_sb and fast_backward_collate_sb are identical
 * routines except that fast_backward_collate_sb only looks at one
 * weight, whereas backward_collate_sb has to look through all weights
 ...
 * on.  The exact same is true of back_pos_collate_sb and 
 * fast_back_pos_collate_sb.
 * Actually, these routines are further shared with __strcoll_std.c
...
#undef __FAST__
#define BACKWARD_COLLATE	backward_collate_sb
#define BACK_POS_COLLATE	back_pos_collate_sb
#include "strcoll_backwd_routines.c"

#define __FAST__
#undef BACKWARD_COLLATE
#undef BACK_POS_COLLATE
#define BACKWARD_COLLATE	fast_backward_collate_sb
#define BACK_POS_COLLATE	fast_back_pos_collate_sb
#include "strcoll_backwd_routines.c"

/*
 * Once again, source is being shared with the _std path.
...
#define FORWARD_COLLATE		forward_collate_sb
#define FAST_FORWARD_COLLATE	fast_forward_collate_sb
#define FORW_POS_COLLATE   	forw_pos_collate_sb
#define FAST_FORW_POS_COLLATE	fast_forw_pos_collate_sb
