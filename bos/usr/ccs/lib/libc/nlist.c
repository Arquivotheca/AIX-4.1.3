static char sccsid[] = "@(#)85	1.3  src/bos/usr/ccs/lib/libc/nlist.c, libcproc, bos411, 9428A410j 3/22/94 17:04:45";
/*
 * COMPONENT_NAME: (LIBCPROC) Standard C Library General Functions
 *
 * FUNCTIONS: nlist
 *
 * ORIGINS: 3 27
 ...
 *
 * RETURN VALUE DESCRIPTION:	0 on successful completion, -1 if
 *		an error occurs
 */
...
struct nlist *nl;		/* the symbols to retrieve...	*/
{
	...
	(void) fread((void *) &hdr, (size_t)sizeof hdr, 1ul, fp);
	if( BADMAG(hdr) )
	{   fclose(fp);
	    return(-1);
	}
	...
	for (nsyms = A_NSYMS(hdr); nsyms > 0 &&
	     fread((void *) symp, (size_t)SYMESZ, 1ul, fp) == 1 && nnls > 0; nsyms--) {
		...
		{   
			...
			(void) fseek(fp, A_NAMEPOS(hdr)+symp->n_offset,
					SEEK_SET);
			if ((len = fread((void *) strbuf, 1ul, (size_t)STRBUF, fp)) <= 0)
