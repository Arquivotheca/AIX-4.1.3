static char sccsid[] = "@(#)70	1.7.2.4  src/bos/usr/ccs/lib/libc/__strxfrm_std.c, libcstr, bos410 4/6/94 21:53:06";
/*
...
int forward_xfrm_std(_LC_collate_objhdl_t hdl, const char *str_in, 
			char *str_out, int count, int n, int order);

int fast_forward_xfrm_std(_LC_collate_objhdl_t hdl, const char *str_in, 
			char *str_out, int count, int n, int order);

int forw_pos_xfrm_std(_LC_collate_objhdl_t hdl, const char *str_in, 
			char *str_out, int count, int n, int order);

int fast_forw_pos_xfrm_std(_LC_collate_objhdl_t hdl, const char *str_in, 
			char *str_out, int count, int n, int order);
...
size_t __strxfrm_std(_LC_collate_objhdl_t hdl, char *str_out, 
				const char *str_in, size_t n)
{
	...
	**********/
	if (sort_mod == 0) {
	    ...
	    ***********/
	    if (fast_locale)
	        count = fast_forward_xfrm_std(hdl, str_in, str_out, count,
				     n, cur_order);
	    else
	        count = forward_xfrm_std(hdl, str_in, str_out, count,
				     n, cur_order);
	    if (count == (size_t)-1)
		return((size_t)-1);
	}
	...
	****************/
		...
		if (fast_locale)
	    	    count = fast_forw_pos_xfrm_std(hdl, str_in, str_out, count,
				     n, cur_order);
		else
	    	    count = forw_pos_xfrm_std(hdl, str_in, str_out, count,
				     n, cur_order);
		xfrm_byte_count = 4;
	    }
	    else {
...
/* The forward_xfrm_std and fast_forward_xfrm_std are identical
 * routines except that fast_forward_xfrm_std only looks at one
 * weight, whereas forward_xfrm_std has to look through all weights
 ...
 * stored in the strxfrm_std_routines.c file.  This file is then 
 * included once with __FAST__ turned off and once with it turned
 * on.  The exact same is true of forw_pos_xfrm_std and 
 * fast_forw_pos_xfrm_std.
 * Actually, these routines are further shared with __strxfrm_sb.c
...
#undef __FAST__
#define FORWARD_XFRM            forward_xfrm_std
#define FORW_POS_XFRM           forw_pos_xfrm_std
#include "strxfrm_routines.c"

#define __FAST__
#undef FORWARD_XFRM
#undef FORW_POS_XFRM
#define FORWARD_XFRM            fast_forward_xfrm_std
#define FORW_POS_XFRM           fast_forw_pos_xfrm_std
#include "strxfrm_routines.c"
