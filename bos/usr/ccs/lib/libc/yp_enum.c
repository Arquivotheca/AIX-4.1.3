static char sccsid[] = "@(#)27	1.6  src/bos/usr/ccs/lib/libc/yp_enum.c, libcyp, bos411, 9432B411a 7/25/94 14:28:43";
/* 
 * COMPONENT_NAME: (LIBCYP) Yellow Pages Library
 * 
 * FUNCTIONS: v1dofirst, v1donext, v1filter, v1prot_dofirst, v1prot_donext, 
 *	      v2dofirst, v2donext, yp_first, yp_next 
 *
 * ORIGINS: 24 
 *
 *                  SOURCE MATERIALS
...
extern struct timeval yprpc_time();
	...
	int  *vallen;		/* return: bytes in val */
{
	...
	for (;;) {
		...
		reason = (*dofun)(domain, map, pdomb, yprpc_time(-1),
	...
	int  *vallen;		/* return: bytes in val */
{
	...
	for (;;) {
		...
		reason = (*dofun)(domain, map, inkey, inkeylen, pdomb,
		    yprpc_time(-1), outkey, outkeylen, val, vallen);
