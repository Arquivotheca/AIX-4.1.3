static char sccsid[] = "@(#)13  1.12  src/bos/usr/ccs/lib/libc/svc_udp.c, libcrpc, bos411, 9428A410j 1/28/94 09:35:07";
/*
...
static char sccsid[] = 	"@(#)svc_udp.c	1.5 90/07/19 4.1NFSSRC Copyr 1990 Sun Micro";
#endif
	...
	u_int sendsz, recvsz;
{
	...
	if (sock == RPC_ANYSOCK) {
		...
		if (sock < 0) {
			(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC92,
	...
	if (getsockname(sock, (struct sockaddr *)&addr, &len) != 0) {
		(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC93,
	...
	if (xprt == NULL) {
		(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC44,
	...
	u_long size;
{
	...
	if (su->su_cache != NULL) {
		(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC45,
			    "enablecache: cache already enabled"));
		return(0);	
	}
	uc = ALLOC(struct udp_cache, 1);
	if (uc == NULL) {
		(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC46,
	...
	if (uc->uc_entries == NULL) {
		(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC47,
	...
	if (uc->uc_fifo == NULL) {
		(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC48,
	...
	u_long replylen;	
{
	...
	if (victim != NULL) {
		...
		for (vicp = &uc->uc_entries[loc]; 
		  *vicp != NULL && *vicp != victim; 
		  vicp = &(*vicp)->cache_next) 
				;
		if (*vicp == NULL) {
			(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC49,
		...
		*vicp = victim->cache_next;	/* remote from cache */
		...
		if (victim == NULL) {
			(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC50,
                                 "cache_set: victim alloc failed"));
			return;
		}
		newbuf = mem_alloc(su->su_iosz);
		if (newbuf == NULL) {
			(void) syslog(LOG_ERR, (char *)oncmsg(LIBCRPC,RPC51,
...
#	define EQADDR(a1, a2)	(bcmp((char*)&(a1), (char*)&(a2), sizeof(a1)) == 0)
	...
	for (ent = uc->uc_entries[loc]; ent != NULL; ent = ent->cache_next) {
		if (ent->cache_xid == msg->rm_xid &&
		  ...
		  EQADDR(ent->cache_addr, xprt->xp_raddr)) {
