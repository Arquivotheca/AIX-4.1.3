static char sccsid[] = "@(#)30  1.3  src/bos/usr/ccs/lib/libcurses/infocmp.c, libcurses, bos411, 9428A410j 6/7/94 17:05:33";
/*
 *   COMPONENT_NAME: LIBCURSES
 *
 *   FUNCTIONS: terminfo compare
 *
 *   ORIGINS: 4, 27
 *
...
#include "print.h"
#include <fcntl.h>
#include "infocmp_msg.h"

#define MSGSTR(num,str) catgets(catd,MS_INFOCMP,num,str)
nl_catd catd;
...
#define LOOKINGAT(str)	{ if (verbose) \
        (void) fprintf(trace, MSGSTR(LOOKAT, "looking at '%s'\n"), (str)); }


void badmalloc()
{
    (void) fprintf(stderr, MSGSTR(SPACE, "infocmp: malloc is out of space!\n"));
    exit (1);
}
...
int argc, firstoptind;
{
    ...
    if (verbose)
	{
	(void) fprintf(trace, MSGSTR(BOOLCAP,
	    "There are %d boolean capabilities.\n"), numbools);
	(void) fprintf(trace, MSGSTR(NUMCAP,
	    "There are %d numeric capabilities.\n"), numnums);
	(void) fprintf(trace, MSGSTR(STRCAP,
...
register char *term;
{
    ...
    if (verbose)
	(void) fprintf(trace, MSGSTR(SETUP,
	...
	if ((length = strlen(ttytype)) >= TTYLEN)
	    ...
	    if (savettytype == NULL)
		{
		(void) fprintf(stderr, MSGSTR(SPACE,
			"infocmp: malloc is out of space\n"));
	...
	if (verbose)
	    (void) fprintf(trace, "%s='%s'.\n", str[i].infoname, TPR(str[i].val));
	}
...
int n;
{
    ...
    if (verbose)
	(void) fprintf(trace, MSGSTR(ADDING,
	...
	{
	if (Aflag && Bflag)
	    (void) printf(MSGSTR(COMP1,
		"comparing %s (TERMINFO=%s) to %s (TERMINFO=%s).\n"),
	        firstterm, term1info, nterm, term2info);
	else if (Aflag)
	    (void) printf(MSGSTR(COMP2,
		"comparing %s (TERMINFO=%s) to %s.\n"),
	        firstterm, term1info, nterm);
	else if (Bflag)
	    (void) printf(MSGSTR(COMP3,
		"comparing %s to %s (TERMINFO=%s).\n"),
		firstterm, nterm, term2info);
	else
	    (void) printf(MSGSTR(COMP4,
		"comparing %s to %s.\n"), firstterm, nterm);
	(void) printf(MSGSTR(COMPBOOL, "    comparing booleans.\n"));
	}
	...
	if (verbose)
	    (void) fprintf(trace, MSGSTR(CHGSEEN,
		    "%s: %d:%d,changed=%d,seen=%d.\n"),
    ...
    if (diff || common || neither)
	(void) printf(MSGSTR(COMPNUMS, "    comparing numbers.\n"));
	...
	if (use)
		...
		if ((numval > -1) && (numval != num[i].secondval))
		    {
		    (void) fprintf(stderr, MSGSTR(ORDERDEP,
			"infocmp: use= order dependency found:\n"));
		    (void) fprintf (stderr, MSGSTR(NUMHAS1,
			"    %s: %s has %d, %s has %d.\n"),
    ...
    if (diff || common || neither)
	(void) printf(MSGSTR(COMPSTR, "    comparing strings.\n"));
	...
	if (use)
		...
		if (!EQUAL (strval, str[i].secondval))
		    {
		    char tmp_str[1024]; /* same size as buffer in iexpand */
		    (void) fprintf(stderr, MSGSTR(ORDERDEP,
			"infocmp: use= order dependency found:\n"));
		    (void)strcpy(tmp_str,TPR(strval));
		    /* two TPR()'s should not be in the same printf() */
		    (void) fprintf(stderr, MSGSTR(NUMHAS2,
			"    %s: %s has '%s', %s has '%s'.\n"),
			str[i].capname, str[i].secondname,
			TPR(str[i].secondval), nterm, tmp_str);
		    }
		}
	    else
		{
	...
	if (diff && !EQUAL (str[i].val, strval))
	    {
	    (void) printf ("\t%s: '%s',", str[i].infoname, TPR(str[i].val));
	    (void) printf ("'%s'.\n", TPR(strval));
	    /* two TPR calls should not be in the same printf() call!! */
	    }
	if (verbose)
	    (void) fprintf(trace, MSGSTR(CHGSEEN2,
		    "%s: '%s':'%s',changed=%d,seen=%d.\n"),
		    str[i].infoname, TPR(str[i].val), strval,
...
register char **argv;
{
	...
	if (used[i - firstoptind])
	    ...
	    (void) fprintf(stderr, MSGSTR(NOTADD,
		"infocmp: 'use=%s' did not add anything to the description.\n"),
		argv[i]);
}
...
register char *termNinfo;
{
	...
	{
	if (verbose)
	    (void) fprintf(trace, MSGSTR(SETTING, "setting TERMINFO=%s.\n"),
		termNinfo);
...
void usage()
{
    (void) fprintf(stderr, MSGSTR(USAGE01,
	"usage: infocmp [-ducnILC1Vv] [-s d|i|l|c] [-A dir] [-B dir] term-names ...\n"));
    (void) fprintf(stderr, MSGSTR(USAGE02,
	"\t-d\tprint differences (the default for >1 term-name)\n"));
    (void) fprintf(stderr, MSGSTR(USAGE03,
	"\t-u\tproduce relative description\n"));
    (void) fprintf(stderr, MSGSTR(USAGE04, "\t-c\tprint common entries\n"));
    (void) fprintf(stderr, MSGSTR(USAGE05, "\t-n\tprint entries in neither\n"));
    (void) fprintf(stderr, MSGSTR(USAGE06,
	"\t-I\tprint terminfo entries (the default for 1 term-name)\n"));
    (void) fprintf(stderr, MSGSTR(USAGE07, "\t-C\tprint termcap entries\n"));
    (void) fprintf(stderr, MSGSTR(USAGE08,
	"\t-L\tprint long C variable names\n"));
    (void) fprintf(stderr, MSGSTR(USAGE09, "\t-1\tsingle column output\n"));
    (void) fprintf(stderr, MSGSTR(USAGE10, "\t-V\tprint program version\n"));
    (void) fprintf(stderr, MSGSTR(USAGE11, "\t-v\tverbose debugging output\n"));
    (void) fprintf(stderr, MSGSTR(USAGE12, "\t-s\tchange sort order\n"));
    (void) fprintf(stderr, MSGSTR(USAGE13,
	"\t-A\tset $TERMINFO for first term-name\n"));
    (void) fprintf(stderr, MSGSTR(USAGE14,
	"\t-B\tset $TERMINFO for other term-names\n"));
    exit (1);
}

main (argc, argv)
...
char **argv;
{
    ...
    catd = catopen(MF_INFOCMP, NL_CAT_LOCALE);
	...
	switch (c)
		...
		break;
	    case 'V':
		(void) printf(MSGSTR(VERSION,
		    "infocmp: version infocmp.c %s\n"), "1.10");
		exit (0);
	    case '?':
		usage();
	    }
	...
	{
	(void) fprintf(stderr, MSGSTR(TWOTERM,
	    "infocmp: must have at least two terminal names for a comparison to be done.\n"));
	usage();
	}
