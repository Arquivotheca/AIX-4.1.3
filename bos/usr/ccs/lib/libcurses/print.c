static char sccsid[] = "@(#)57  1.2  src/bos/usr/ccs/lib/libcurses/print.c, libcurses, bos411, 9428A410j 11/19/93 11:04:23";
/*
 *   COMPONENT_NAME: LIBCURSES
 *
 *   FUNCTIONS:  Functions for printing termcap/terminfo information.
 *
 *   ORIGINS: 4, 27
 *
...
#include "curses.h"
#include "print.h"
#include "printinfo_msg.h"

#define MSGSTR(num,str) catgets(prcatd,MS_PRINTINFO,num,str)
nl_catd prcatd;
...
enum printtypes type;
{
    ...
    prcatd = catopen(MF_PRINTINFO, NL_CAT_LOCALE);
    printing = type;
}
...
register char *term, *synonyms;
{
	...
	case (int) pr_longnames:
	    (void) printf (MSGSTR(TERMTYPE, "Terminal type %s\n"), term);
	    (void) printf ("  %s\n", synonyms);
	    break;
	}
}
...
void pr_bheading()
{
	...
	(void) printf (MSGSTR(FLAGS, "flags\n"));
    printed = 0;
}
...
void pr_nheading()
{
    if (printing == pr_longnames) {
	(void) printf ("\n");
	(void) printf (MSGSTR(NUMBERS, "numbers\n"));
    }
    printed = 0;
}
...
void pr_sheading()
{
    if (printing == pr_longnames) {
	(void) printf ("\n");
	(void) printf (MSGSTR(STRINGS, "strings\n"));
    }
    printed = 0;
}
...
void pr_sfooting()
{
    ...
    if (caplen >= 1024)
        {
	(void) fprintf (stderr, MSGSTR(TOOLONG,
	    "WARNING: termcap entry is too long!\n"));
	}

    if (printing == pr_longnames)
	(void) printf (MSGSTR(ENDOFSTR, "end of strings\n"));
}
