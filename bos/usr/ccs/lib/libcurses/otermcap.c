static char sccsid[] = "@(#)50  1.1  src/bos/usr/ccs/lib/libcurses/otermcap.c, libcurses, bos411, 9428A410j 9/3/93 15:10:29";
/*
 *   COMPONENT_NAME: LIBCURSES
 *
 *   FUNCTIONS:  Routines for dealing with the terminal capability data base.
 *
 *   ORIGINS: 4, 27
 *
	...
	char *bp, *name;
{
	...
	*/
	if (strlen(bp) >= (TBUFSIZE/2))
	    warning("tgetent()", MSGSTR(TOOLONG,
		"Termcap entry is too long.\n"));
	return ret;
}
...
#define TERMNAMESIZE 16
	...
	*q = 0;
	if (++hopcount > MAXHOP) {
		warning("tnchktc()", MSGSTR(INFLOOP,
		    "Infinite tc= loop\n"));
		return (0);
	}
	...
	if (l > TBUFSIZE) {
		warning("tnchktc()", MSGSTR(TOOLONG,
		    "Termcap entry is too long.\n"));
