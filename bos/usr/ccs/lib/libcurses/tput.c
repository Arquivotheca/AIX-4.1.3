static char sccsid[] = "@(#)07  1.3  src/bos/usr/ccs/lib/libcurses/tput.c, libcurses, bos411, 9435B411a 8/31/94 09:59:04";
/*
 *   COMPONENT_NAME: LIBCURSES
 *
 *   FUNCTIONS: print terminal attribute
 *
 *   ORIGINS: 4, 27
 *
...
/* tput - print terminal attribute
   ...
   return codes for command line arguments:
...
char **argv;
{
	...
	switch (i)
		...
		(void) fprintf(stderr, MSGSTR(USAGE,
		    "usage: \"tput [-S] [-T term] capname [tparm argument...]\"\n"));
		exit(2);
	    }
	...
	{
	case -2:
	    (void) fprintf(stderr, MSGSTR(UNREAD,
		"tput: unreadable terminal descriptor \"%s\"\n"), term);
	    exit(3);
	case -1:
	    (void) fprintf(stderr, MSGSTR(NOINFO,
		"tput: no terminfo database\n"));
	    exit(3);
	case 0:
	    (void) fprintf(stderr, MSGSTR(UNKTERM,
...
char **argv;
{
    ...
    (void) fprintf(stderr, MSGSTR(UNKCAP,
	"tput: unknown terminfo capability '%s'\n"), cap);
    exit (4);
}
...
char *file;				/* File to copy. */
{
	...
	(void) fprintf(stderr, MSGSTR(CANTOPEN,
	    "tput: Cannot open initialization file %s.\n"), file);
    else
	{
