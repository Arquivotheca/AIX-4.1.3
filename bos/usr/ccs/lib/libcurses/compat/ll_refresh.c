static char sccsid[] = "@(#)77  1.6.1.1  src/bos/usr/ccs/lib/libcurses/compat/ll_refresh.c, libcurses, bos411, 9428A410j 4/29/92 14:49:46";
/*
...
int	use_idl;
{
...
#ifdef DEBUG
	if (outf) fprintf(outf,
		"virt cursor at y=%d, x=%d, length %d, length-1 %d\n",
	...
	/*
	 * If able to scroll at all, check to see if
	 * scrolling one line is useful.
	 */
	if ((use_idl && _cost(ilfixed) < INFINITY) || scroll_forward)
	{
		for( n = 1; n <= lines; n++ )
		{
			if( SP->cur_body[n] == 0 )
