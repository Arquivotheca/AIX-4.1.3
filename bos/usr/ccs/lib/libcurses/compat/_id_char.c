static char sccsid[] = "@(#)95	1.12  src/bos/usr/ccs/lib/libcurses/compat/_id_char.c, libcurses, bos411, 9428A410j 4/7/94 14:30:09";
/*
...
#define		costic(nchars)	(SP->term_costs.icfixed + \
	...
	/*
	 * On HP terminals we will draw the entire line if there are attributes
	 * turned on anywhere in the new line.  This is so we can turn 
	 * attributes off at the end of the line which will avoid problems 
	 * with terminals that have problems doing cursor positioning with 
	 * attributes turned on.
	 */
	if( ceol_standout_glitch ) 
		for( i=0; i < new->length; i++)
			if ( new->body[i] & A_ATTRIBUTES) {
				ochanged += com_head+com_tail;
				nchanged += com_head+com_tail;
				oc_end += com_tail;
				nc_end += com_tail;
