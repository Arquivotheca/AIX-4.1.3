...
static char sccsid[] = "@(#)73  1.12  src/bos/usr/ccs/lib/libcurses/setupterm.c, libcurses, bos411, 9428A410j 9/3/93 14:48:04";
/*
...
#ifdef	JWINSIZE		/* for BLIT, same as ifndef NOBLIT	*/
...
char	*Def_term = "unknown",	/* default terminal type */
	term_parm_err[32], ttytype[128], _frst_tblstr[2048];
...
out_err:
	...
	*errret = 1;
    ...
    /*
     * ioctls for Sun and AIX.  In the past, we have called
     * termdef for AIX.  termdef called ioctl.  If unsuccessful,
     * it called hfquery.  If that was unsuccessful, it did a getenv
     * for LINES and COLUMNS if set.  
     */
    {
	struct	ttysize	w;
...
#endif	/* defined(TIOCGSIZE) */

    /*
     * Check $LINES and $COLUMNS.  Override if set.
     */
    {
