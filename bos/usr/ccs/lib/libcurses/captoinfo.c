static char sccsid[] = "@(#)98  1.3  src/bos/usr/ccs/lib/libcurses/captoinfo.c, libcurses, bos411, 9428A410j 1/26/94 13:15:21";
/*
 *   COMPONENT_NAME: LIBCURSES
 *
 *   FUNCTIONS:  Convert a termcap description to a terminfo description.
 *
 *   ORIGINS: 4, 27
 *
...
#include "otermcap.h"
#include "print.h"
#include "captoinfo_msg.h"
#include <errno.h>

#define MSGSTR(num,str) catgets(catd,MS_CAPTOINFO,num,str)
nl_catd catd;

#define trace stderr			/* send trace messages to stderr */

#define TRACE(fmt, a1, a2, a3) \
    { if (verbose) (void) fprintf(trace, fmt, a1, a2, a3); }

#define TRACE2(fmt, a1, a2, a3) \
    { if (verbose>1) (void) fprintf(trace, fmt, a1, a2, a3); }

#define TPR(str)	((str) ? iexpand(str) : "NULL")

#define CAP_TYPE(type) \
	   ((type == tbool)   ? MSGSTR(TYPE_BOO, "boolean")  : \
	    (type == tnum)    ? MSGSTR(TYPE_NUM, "numeric")  : \
	    (type == tstr)    ? MSGSTR(TYPE_STR, "string")   : \
	    (type == tcancel) ? MSGSTR(TYPE_CAN, "canceled") : \
				MSGSTR(TYPE_UNK, "(unknown type)"))
...
va_dcl
{
    ...
    va_start(args);
    what = va_arg(args, char *);
    fprintf(stderr, "%s: TERM=%s:\n", what, term_name);
...
void checktermcap()
{
	...
	if (*tbuf == '.')
	    {
	    TRACE(MSGSTR(COMMENT,
		"termcap string '%c%c' commented out.\n"), tbuf[1], tbuf[2],0);
	    if (!capsearch (boolcodes, oboolcodes, tbuf+1) &&
		!capsearch (numcodes, onumcodes, tbuf+1) &&
		!capsearch (strcodes, ostrcodes, tbuf+1))
		warning("captoinfo", MSGSTR(COMMENT2,
		    "commented out code '%.2s' is unknown.\n"), tbuf+1);
	    continue;
	    }

	TRACE(MSGSTR(LOOKAT, "looking at termcap string '%.2s'.\n"), tbuf,0,0);
	...
	switch (tbuf[2])
	    ...
	    default:
		warning("captoinfo", MSGSTR(UNKTYPE,
		    "unknown type given for the termcap code '%.2s'.\n"), tbuf);
		type = tunknown;
	    }

	TRACE2(MSGSTR(TYPEOF, "type of '%.2s' is %s.\n"),tbuf,CAP_TYPE(type),0);

	/* look for the name in bools */
	if (capsearch (boolcodes, oboolcodes, tbuf))
	    {
	    if (type != tbool && type != tcancel)
		warning("captoinfo", MSGSTR(BADTYPE,
		    "wrong type given for the %s termcap code '%.2s'.\n"),
		    CAP_TYPE(tbool), tbuf);
	    continue;
	    }
	...
	warning("captoinfo", MSGSTR(INVALID,
	    "the termcap code '%.2s' is not a valid name.\n"), tbuf);
	}
}

/*
...
int filltables ()
{
	...
	{
	warning("captoinfo", MSGSTR(TGFAIL,
	    "tgetent failed with return code %d (%s).\n"), tret,
	    (tret == 0)  ? MSGSTR(TG_INV, "non-existent or invalid entry") :
	    (tret == -1) ? MSGSTR(TG_OPN, "cannot open $TERMCAP") :
			   MSGSTR(TG_UNK, "unknown reason"));
	return 0;
	}
	...
	TRACE2("boolcodes=%s, boolnames=%s, flag=%d.\n",
	    boolcodes[i], boolnames[i], boolval[uselevel][i]);
	}
	...
	TRACE2("numcodes=%s, numnames=%s, num=%d.\n",
	    numcodes[i], numnames[i], numval[uselevel][i]);
	}
	...
	TRACE2("strcodes=%s, strnames=%s, str=%s.\n",
	    strcodes[i], strnames[i], TPR(strval[uselevel][i]));

	/* remove zero length strings */
	if (strval[uselevel][i] && (strval[uselevel][i][0] == '\0') )
	    {
	    warning("captoinfo", MSGSTR(NULLREM,
		"cap %s (info %s) is NULL: REMOVED\n"),
		strcodes[i], strnames[i]);
	...
	TRACE2("oboolcodes=%s, flag=%d.\n",
	    oboolcodes[i], oboolval[uselevel][i], 0);
	}
	...
	TRACE2("onumcodes=%s, num=%d.\n", onumcodes[i], onumval[uselevel][i],0);
	}
	...
	TRACE2("ostrcodes=%s, ostr=%s.\n",
	    ostrcodes[i], TPR(ostrval[uselevel][i]),0);
	/* remove zero length strings */
	if (ostrval[uselevel][i] && (ostrval[uselevel][i][0] == '\0') )
	    {
	    warning("captoinfo", MSGSTR(NULLREM2,
		"cap %s (no terminfo name) is NULL: REMOVED\n"), ostrcodes[i]);
...
getlongname ()
{
    ...
    *l = '\0';
    ...
    if (b != &bp[0])
	{
	(void) fprintf(stderr, MSGSTR(OBSOL,
	    "captoinfo: obsolete 2 character name '%2.2s' removed.\n"), bp);
	(void) fprintf(stderr, MSGSTR(OBSOL2,
	    "\tsynonyms are: '%s'\n"), buflongname);
	}
}

...
register char *capname;
{
    ...
    TRACE2(MSGSTR(LOOKFOR, "looking for termcap value of %s.\n"), capname,0,0);

    /* Check the old termcap list. */
    for (i = 0; ostrcodes[i]; i++)
	if (strcmp (ostrcodes[i], capname) == 0)
	    {
	    TRACE2(MSGSTR(VALUE, "\tvalue is: %s.\n"),
		TPR(ostrval[uselevel][i]),0,0);
	    return ostrval[uselevel][i];
	    }

    TRACE2(MSGSTR(NOTFND,
	"termcap name '%s' not found in ostrcodes.\n"), capname,0,0);
	...
	if (strcmp (strcodes[i], capname) == 0)
	    {
	    TRACE2(MSGSTR(VALUE, "\tvalue is: %s.\n"),
		TPR(strval[uselevel][i]),0,0);
	    return strval [uselevel][i];
	    }

    warning("captoinfo", MSGSTR(NOTFND2,
	"termcap name '%s' not found.\n"), capname);
...
int max;
{
    ...
    (void) fprintf(trace, MSGSTR(SEARCH, "search looking for %s.\n"), infoname);
    (void) fprintf(trace, "base=%#x, bret=%#x, nel=%d.\n", names, bret, max);
    (void) fprintf(trace, MSGSTR(RETURN, "returning %d.\n"),
...
register char *infoname;
{
    ...
    TRACE2(MSGSTR(LOOKFOR2,"looking for terminfo value of %s.\n"),infoname,0,0);
    ...
    TRACE2(MSGSTR(NOTFND3, "terminfo name '%s' not found.\n"), infoname,0,0);
...
register int newvalue;
{
    ...
    TRACE2(MSGSTR(CHVAL, "changing value for %s to %d.\n"),infoname,newvalue,0);

    i = search (boolnames, boolcount, infoname);
    if (i != -1)
	{
	TRACE2(MSGSTR(VALWAS, "value was: %d.\n"), boolval[uselevel][i],0,0);

	boolval[uselevel][i] = newvalue;
	return;
	}

    warning("captoinfo", MSGSTR(NOTFND3,
...
register char *infoname, *newvalue;
{
    ...
    TRACE2(MSGSTR(CHVAL2, "changing value for %s to %s.\n"),
	infoname, TPR(newvalue),0);

    i = search (strnames, strcount, infoname);
    if (i != -1)
	{
	TRACE2(MSGSTR(VALWAS2,"value was: %s.\n"),TPR(strval[uselevel][i]),0,0);
...
handleko()
{
    ...
    TRACE2(MSGSTR(KOSTR, "working on termcap ko string.\n"),0,0,0);
	...
	TRACE2(MSGSTR(KEYNAME, "key termcap name is '%s'.\n"),TPR(capname),0,0);

	/* match it up into our list */
	found = 0;
	for (j = 0; !found && ko_map[j].keyedinfoname != NULL; j++)
	    {
	    TRACE2(MSGSTR(LOOKAT2, "looking at termcap name %s.\n"),
		ko_map[j].capname,0,0);
		...
		if ((capstr = getcapstr (capname)) != NULL)
			...
			if (strcmp(capstr, infostr) != 0)
			    {
			    warning("captoinfo", MSGSTR(FUNCKEY,
"a function key for '%s' was specified with the value\n'%s', but it already has the value '%s'.\n"),
				capname, TPR(capstr), TPR(infostr));
			    }
		    }
		found = 1;
		}
	...
	if (!found)
	    {
	    warning("captoinfo", MSGSTR(UNKCAP,
"the unknown termcap name '%s' was\nspecified in the 'ko' termcap capability.\n"),
		capname);
	    }
	}
}
...
handlema()
{
    ...
    TRACE2(MSGSTR(MASTR, "working on termcap ma string.\n"),0,0,0);
	...
	TRACE2(MSGSTR(KEYVAL, "key value is '%s'.\n"), TPR(cap),0,0);

	if (ostrval[uselevel][cap_ma][i] == '\0')
	    break;

	/* isolate the vi key name */
	vichar = xbuf[0] = ostrval[uselevel][cap_ma][i++];
	TRACE2(MSGSTR(KEYVI, "the vi key is '%s'.\n"), TPR(xbuf),0,0);

	/* match up the vi name in our list */
	found = 0;
	for (j = 0; !found && ma_map[j].keyedinfoname != NULL; j++)
	    {
	    xbuf[0] = ma_map[j].vichar;
	    TRACE2(MSGSTR(LOOKVI, "looking at vi character '%s'\n"),
		TPR(xbuf),0,0);
		...
		if ( infostr == NULL )
		    ...
		    {
		    xbuf[0] = vichar;
		    warning("captoinfo", MSGSTR(VICHAR,
"the vi character '%s' (info '%s') has the\nvalue '%s', but 'ma' gives '%s'.\n"),
			TPR(xbuf), ma_map[j].keyedinfoname, TPR(infostr),
			TPR(cap));
		    }
		found = 1;
		}
	...
	if (!found)
	    {
	    xbuf[0] = vichar;
	    (void) fprintf(stderr, MSGSTR(UNKVI,
"captoinfo: the unknown vi key '%s' was\nspecified in the 'ma' termcap capability.\n"),
		TPR(xbuf));
	    }
	}
}
...
void adddefaults ()
{
    ...
    TRACE2(MSGSTR(ASGNDEF, "assigning defaults.\n"),0,0,0);
	...
	warning("captoinfo", MSGSTR(WARNSGUG,
	    "Warning: termcap sg and ug had different values (%d<->%d).\n"),
	    sg, onumval[uselevel][cap_ug]);
...
void changecalculations ()
{
	...
	if (needscopying (strval [uselevel] [i]))
	    {
	    TRACE(MSGSTR(NEEDCOPY, "%s needs copying, was: %s.\n"),
		strnames [i], TPR(strval[uselevel][i]),0);
		...
		parmsaved = 1;
		warning("captoinfo", MSGSTR(INEFF,
"Warning: the string produced for '%s' may be inefficient.\nIt should be looked at by hand.\n"),
		    strnames[i]);
		}
	    else
		parmsaved = 0;
	    ...
	    TRACE(MSGSTR(BECOME, "and has become: %s.\n"),
		TPR(strval[uselevel][i]),0,0);
	    }
    nextstring = to;
}
...
captoinfo ()
{
    ...
    if (term_name == NULL)
	{
	(void) fprintf(stderr, MSGSTR(NULLTERM,
	    "captoinfo: Null term_name given.\n"));
	return;
	}

    TRACE(MSGSTR(CHNGCAP, "changing cap to info, TERM=%s.\n"), term_name,0,0);
	...
	{
	uselevel = 1;
	TRACE(MSGSTR(USEFOUND,
	    "use= found, %s uses %s.\n"), term_name, TLHtcname,0);
...
use_etc_termcap ()
{
    TRACE(MSGSTR(READFROM, "reading from %s\n"), "/etc/termcap",0,0);
...
initdirname ()
{
    ...
    (void) getwd (dirname);
#endif /* SYSV || USG */
    TRACE(MSGSTR(CURDIR, "current directory name=%s.\n"), dirname,0,0);
    environ = newenviron;
}
...
register char *capfile;
{
    ...
    TRACE(MSGSTR(SETENVI, "setting the environment for %s.\n"), TERMCAP,0,0);
}

setterm_name ()
{
    TRACE(MSGSTR(SETENVI2,
	"setting the environment for TERM=%s.\n"), term_name,0,0);
...
register char *line;
{
    ...
    TRACE(MSGSTR(EXTRNAME, "extracting name from '%s'.\n"), line,0,0);
	...
	for ( ; *lineptr ; lineptr++)
		...
		{
		*lineptr = '\0';
		TRACE(MSGSTR(RETURN2, "returning %s.\n"), line,0,0);
		return line;
		}
    TRACE(MSGSTR(RETURN2, "returning %s.\n"), "NULL",0,0);
    return NULL;
...
register char *filename;
{
	...
	(void) fprintf(stderr, MSGSTR(CANTOPEN, 
	    "captoinfo: cannot open %s for reading.\n"), filename);
	return;
	}
...
char **argv;
{
    ...
    catd = catopen(MF_CAPTOINFO, NL_CAT_LOCALE);
	...
	switch (c)
	    ...
	    case 'w':
                ...
                return_val = (int) strtol(optarg, &endptr, 0);

                        /*messages about invalid width */

                if(strlen(endptr))   /*string given as number */
                (void) fprintf(stderr, MSGSTR(INVALID_ARG1,
                    "Invalid argument '%s'.\n"), optarg);

                if(errno)            /*number overflow*/
                (void) fprintf(stderr, MSGSTR(INVALID_ARG2,
                    "Number out of range '%s'.\n"), optarg);

                if(strlen(endptr) || errno) {  /*either of the above errors*/
                (void) fprintf(stderr, MSGSTR(USAGE1,
                    "usage: captoinfo [-1Vv] [-w width] [filename ...]\n"));
                (void) fprintf(stderr, MSGSTR(USAGE2,
                    "\t-1\tsingle column output\n"));
                (void) fprintf(stderr, MSGSTR(USAGE3,
                    "\t-v\tverbose debugging output\n"));
                (void) fprintf(stderr, MSGSTR(USAGE4,
		...
		break;
	    case 'V':
		printf(MSGSTR(VERSION,
		    "@(#)curses:screen/captoinfo.c   %s\n"), "1.8");
		fflush(stdout);
		exit (0);
	    case '?':
		(void) fprintf(stderr, MSGSTR(USAGE1,
