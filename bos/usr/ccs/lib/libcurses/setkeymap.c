...
static char sccsid[] = "@(#)75  1.3  src/bos/usr/ccs/lib/libcurses/setkeymap.c, libcurses, bos411, 9428A410j 5/4/94 09:17:48";
/*
...
static FILE *outfile;
static int i;
static char *definenames[]=
		{    "KEY_BACKSPACE", "KEY_CATAB", "KEY_CLEAR", "KEY_CTAB",
		     "KEY_DC", "KEY_DL", "KEY_DOWN", "KEY_EIC", "KEY_EOL",
		     "KEY_EOS", "KEY_F(0)", "KEY_F(1)", "KEY_F(10)", "KEY_F(2)",
		     "KEY_F(3)", "KEY_F(4)", "KEY_F(5)", "KEY_F(6)", "KEY_F(7)",
		     "KEY_F(8)", "KEY_F(9)", "KEY_HOME", "KEY_IC", "KEY_IL",
		     "KEY_LEFT", "KEY_LL", "KEY_NPAGE", "KEY_PPAGE", "KEY_RIGHT",
		     "KEY_SF", "KEY_SR", "KEY_STAB", "KEY_UP", "KEY_A1", "KEY_A3", 
		     "KEY_B2", "KEY_C1", "KEY_C3", "KEY_COMMAND", "KEY_END",
		     "KEY_HELP", "KEY_ACTION", "KEY_BTAB", 
		     "KEY_BEG", "KEY_CANCEL", "KEY_CLOSE", "KEY_COPY", 
		     "KEY_CREATE", "KEY_ENTER", "KEY_EXIT", "KEY_FIND", 
		     "KEY_MARK", "KEY_MESSAGE", "KEY_MOVE", "KEY_NEXT", 
		     ...
		     "KEY_UNDO", "KEY_SBEG", "KEY_SCANCEL", "KEY_SCOMMAND", "KEY_SCOPY",
		     "KEY_SCREATE", "KEY_SDC", "KEY_SDL", "KEY_SELECT", "KEY_SEND",
		     "KEY_SEOL", "KEY_SEXIT", "KEY_SFIND", "KEY_SHELP", "KEY_SHOME",
		     "KEY_SIC", "KEY_SLEFT", "KEY_SMESSAGE", "KEY_SMOVE", "KEY_SNEXT",
		     "KEY_SOPTIONS", "KEY_SPREVIOUS", "KEY_SPRINT", "KEY_SREDO", 
		     "KEY_SREPLACE", "KEY_SRIGHT", "KEY_SRSUME", "KEY_SSAVE",
		     "KEY_SSUSPEND", "KEY_SUNDO"
		};
#endif
...
#ifdef DEBUG
fprintf(outfile, "a) %d. sends='%s'   keyval='%o'   define='%s'\n", i, 
	p->_sends, p->_keyval, definenames[i]);
#endif
    funckey[(txt)[0]] |= _KEY;
    p++;
}
...
,i++
	...
	if (*keyptr)
	{
	    ...
	    p->_keyval = *codeptr;
#ifdef DEBUG
fprintf(outfile, "b) %d. sends='%s'   keyval='%o'   define='%s'\n", i, 
	p->_sends, p->_keyval, definenames[i]);
#endif
...
#ifdef DEBUG
fprintf(outfile, "c)    sends='%s'   keyval='%o'   define='%s%d%c'\n", 
	p->_sends, p->_keyval, "KEY_F(",i++,')');
#endif
	funckey[*keyptr[0]] |= _KEY;
	p++;
    }
...
setkeymap()
{
    ...
    _keyfunc(&key_command, &key_command);
    _keyfunc(&key_end, &key_help);
    _keyfunc2(&key_f11, &key_f63);
    _keyfunc(&key_action, &key_action);
...
#ifdef	DEBUG
    if (outf)
	fprintf(outf, "key structure ending at %x\n", p);
#endif	/* DEBUG */
    return (OK);
}
