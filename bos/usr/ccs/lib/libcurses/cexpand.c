static char sccsid[] = "@(#)99  1.1  src/bos/usr/ccs/lib/libcurses/cexpand.c, libcurses, bos411, 9428A410j 9/3/93 15:06:52";
/*
		...
		if (*str == '<')
			...
			*pbufptr = '\0';
			...
			/* marked with /. Just remove it. */
			    ...
			    (void) fprintf (stderr, MSGSTR(PADREM,
				"infocmp: mandatory padding removed\n"));
			}
			...
			*retptr++ = '<';
		    ...
		    /* of the string. */
		    if (*str != '\0')
			(void) fprintf (stderr, MSGSTR(PADMOVE,
			    "infocmp: padding information moved to end\n"));
		    }
		else
		    *retptr++ = ch;
		break;
