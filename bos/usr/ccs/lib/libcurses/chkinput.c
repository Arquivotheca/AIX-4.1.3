...
static char sccsid[] = "@(#)00	1.2  src/bos/usr/ccs/lib/libcurses/chkinput.c, libcurses, bos41J, 9511A_all 3/9/95 14:12:56";
/*
...
_chkinput()
{
    ...
    gettimeofday(&e, NULL);
    secs  = e.tv_sec  - s.tv_sec;
    if (secs == 1)
	msecs = (e.tv_sec + uS_PER_SEC) - s.tv_sec;
    else if (secs == 0)
	msecs = e.tv_usec - s.tv_usec;
    /* else, more than a second has elapsed, don't bother calculating msecs. */

    /* To reduce the number of ioctl calls, check for keyboard input
     * only if it's been more than a half second since the last time
     * it was checked.
     */
    if(secs > 1 || msecs > half) {
