 ...
 */
#ifndef	lint
static char sccsid[] = "@(#)15	1.6  src/bos/usr/ccs/lib/libld/lddopen.c, libld, bos411, 9428A410j 4/16/91 05:16:30";
#endif	lint

/*
* lddopen - get LDFILE, header info for object file.
...
LDFILE *
lddopen(fd, mode, ldptr)
int	fd;
char	*mode;
LDFILE	*ldptr;
{
	extern FILE *fdopen TAKES((int,char *));
	...
	if (vldldptr(ldptr) == FAILURE)
	{
...
#else
#ifdef AIXV3AR
		if (fread((void *)buf, (size_t)(sizeof(char) * SAIAMAG), (size_t)1, ioptr) != 1)
			buf[0] = '\0';
		...
		if (strncmp(buf, ARMAG, SARMAG) == 0)
		{
			...
			if (fseek(ioptr, (long) (sizeof(char) * SARMAG), 0)
					== OKFSEEK
				&& fread((char *)&arbuf, sizeof(arbuf), 1,
					ioptr) == 1
		...
		if (strncmp(buf, AIAMAG, SAIAMAG) == 0)
		{
			...
			if (fseek(ioptr, 0L, 0) == OKFSEEK
				...
				&& (AR_OFF(ldptr) = atol(flbuf.fl_fstmoff))
					!= 0L
				&& fseek(ioptr, atol(flbuf.fl_fstmoff), 0)
					== OKFSEEK
				&& fread((char *)&arbuf, (size_t)sizeof(arbuf), (size_t)1,
					ioptr) == 1
				&& fseek(ioptr, ((atol(arbuf.ar_namlen)+01)
					& ~01), 1) == OKFSEEK)
			{
		...
		if (FSEEK(ldptr, (long int)0L, (int)BEGINNING) == OKFSEEK &&
			FREAD((char *)&(HEADER(ldptr)), (size_t)FILHSZ, (size_t)1, ldptr) == 1)
		{
			return(ldptr);
		}
	}
	else if (FSEEK(ldptr, (long int)0L, (int)BEGINNING) == OKFSEEK)
		return(ldptr);
