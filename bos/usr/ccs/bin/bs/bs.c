static char sccsid[] = "@(#)07  1.14  src/bos/usr/ccs/bin/bs/bs.c, cmdlang, bos411, 9428A410j 3/11/94 10:13:17";
/*
 * COMPONENT_NAME: (CMDLANG) Language Utilities
 *
 * FUNCTIONS: main, aname, cerror, cklabel, ckname, clear, clearstk, comp,
                cvbase, e1, e2, e3, e3a, e4, e5, e6, e7, e8, e9, error,
                execute, expr, fexec, fpe, getar, graph, grio, htable,
                htitem, intpow, intr, io, local, lookup, mkdouble, nextname,
                op, prtrace, prtsubs, push, putgr, putwd, rlist, salloc, setup,
                sigpipe, singstat, statement, substr, to_int, to_str, true
 *
 * ORIGINS: 27
 *
...
#ifdef KJI
#define LETTER (isalpha(*Lp) || NCisshift(*Lp) || isjkata(*Lp))
#define LETNUM (isalnum(*Lp) || NCisshift(*Lp) || isjkata(*Lp))
#else
...
#define STRTEMPS        50
#define J_STACK         5
#define MAX_LOCAL       10 /*D44428 - Max number of local params per function*/
...
char Fcode[] = {
        ...
        TABLE, ITEM, KEY, ISKEY, BSIZE1
};
char *Libnames[] = {
        "atan", "ceil", "cos", "exp", "abs", "floor", "log", "sin", "sqrt", 0
};
double (*Libcode[])(double x) = {
...
fpe(void)
{
        error(MSGSTR(FLTERR,"Floating exception"));                     /*MSG*/
}

sigpipe(void)
{
        error(Gio ? MSGSTR(BADGRAF,"Bad graph output")                  /*MSG*/
                  : MSGSTR(PIPECL,"Pipe closed"));                      /*MSG*/
}
...
char **argv;
{
        ...
        setlocale(LC_ALL, "");
#ifdef MSG
        catd = catopen(MF_BS,NL_CAT_LOCALE);
#endif
        info = localeconv();
        /*
         *  107098 - get the decimal point for the current locale
         */
...
again:
        ...
        if(signal(SIGINT, SIG_IGN) != SIG_IGN)
                (void)signal(SIGINT, (void(*)(int)) intr);
        (void)signal(SIGFPE, (void(*)(int)) fpe);
        (void)signal(SIGPIPE, (void(*)(int)) sigpipe);
        Lnames = 0;
        if(Sinput)
                Lnum = 0;
        else
...
statement()
{
        ...
        if(Lnum==1 && !strncmp(Line, "bs ", 3) &&
                !strncmp((Line+strlen(Line)-5), "exit", 4))
                return;
        SKIP;
        if(EOL || *Lp == '#')
                return;
        ...
        if(Ip > (Instr+INSTRUCTIONS-100))
                cerror(MSGSTR(NOCSPAC,"Out of code space"));            /*MSG*/
        op(LINE);
        push(Lnum);
        comp();
        if(!EOL)
                cerror(MSGSTR(INVEXPR,"Invalid expression"));           /*MSG*/
        Lnum += Cont, Cont = 0;
}
singstat()
{
...
char * s;
{
        ...
        if((v = fork()) == 0) { /* child */
                ...
                execl("/usr/bin/bsh", "bsh", "-c", s, 0);
                exit(0);
        } else if(v == -1) {
                cerror(MSGSTR(NOFORK,"Cannot fork"));                   /*MSG*/
        } else {
...
elseif:
        ...
        case ELIF:
                if(s[--Nest].tp != IF && s[Nest].tp != ELIF)
                        cerror(MSGSTR(ELNOIF,"Else: no if"));           /*MSG*/
                save1.ip = s[Nest].loc;
                op(JUMP);
...
fifi:
                if(s[--Nest].tp != IF && s[Nest].tp != ELSE &&
                        s[Nest].tp != ELIF)
                        cerror(MSGSTR(FINOIF,"Fi: no if"));             /*MSG*/
                *(s[Nest].loc) = (int)Ip;
        ...
        case OBASE:
                ...
                Lp = Atof;
                if(i != 10 && i!= 8 && i != 16)
                        cerror(MSGSTR(BASERR,                           /*MSG*/
                                "ibase/obase must be 8, 10, or 16"));   /*MSG*/
                if(Rescode[ns] == IBASE)
                        Ibase = i;
                else
                        Obase = i;
...
forerr:                         cerror(MSGSTR(FORERR,"For"));           /*MSG*/
                        op(JUMP); /* setup jump around incr */
                        save2.ip = Ip++;
                        save3.cp = Lp;
                        s[Nest].tp = FOR; /* loc for jump from `next' */
                        s[Nest++].loc = Ip;
                        Lp = save1.cp; /* re-parse left side of
                                        * 1st expr for incr */
                        e9(1);
                        Lp = save3.cp;
                        op(INCR);
...
next:
                if(s[--Nest].tp != NEXT)
                        cerror(MSGSTR(NNFOR,"Next: no for"));           /*MSG*/
                save1.ip = s[Nest].loc;
        ...
        case BREAK:
                ...
                if(i < 0)
brkerr:                 cerror(MSGSTR(NOFOR,"No for"));                 /*MSG*/
                break;
        case CONTINUE:
                for(i = Nest-1; i>=0; --i)
                        if(s[i].tp == FOR) {
        ...
        case FUNCDEF:
                if(!aname(tname))
funerr:                 cerror(MSGSTR(FUNERR,"Func def."));             /*MSG*/
                (void)cklabel(tname, Ip);
                ...
                /*D44428 - bs command allows only 9 local parameters per
                function, not 10 as specified in the AIX Commands Reference.
                Note: This is NOT the best place to check Lnames; Lnames could
                be greater than MAX_LOCAL during the string copies above.*/
                if(Lnames > MAX_LOCAL) goto funerr;
                *save1.ip = i;
                *(save1.ip+1) = Lnames;
                break;
        case NUF:
        ...
        case INCLUDE:
                Savelnum = Lnum;
                if(Sinput)
                        cerror(MSGSTR(NSTINC,"Nested `include's"));     /*MSG*/
                if(Input != stdin)
                        Sinput = Input;
                Ip = Sinstr;
        case COMPILE:
                ...
                if(expr(0)) {
                        ...
                        if(Input == NULL) {
                                ...
                                cerror(MSGSTR(NOPF,"Cannot open file"));/*MSG*/
                        }
                        *Sinstr = '\0';
                        if(Rescode[ns] == COMPILE)
                                clear();
                        else
                                if(Ip >= Sinstr)
                                        Ip = Instr;
                ...
                if(Rescode[ns] == INCLUDE)
                        cerror(MSGSTR(MISSFN,"Missing filename"));      /*MSG*/
...
expr(a)
{
        ...
        if(*Lp == '=') {
                ...
                if(Last_op == NAME || Last_op == LNAME) {
                        ...
                        Lp -= 2, cerror(MSGSTR(NAMREQ,"Name Required"));/*MSG*/
        }
        return saveip != Ip;
}
e1(a)
...
ret_false:
        ...
        if(*Lp == '"') {
                ...
                while(*++Lp != '"')
                        if(EOL)
                                cerror(MSGSTR(UBALQ,"Unbal. quote"));   /*MSG*/
                        else {
                                if(*Lp == '\\') {
                                        switch(*++Lp) {
                                        case '"': break;
        ...
        if(!aname(tname)) {
                if(a || incrflg)
e9err:
                        cerror(MSGSTR(EXPR,"Expression"));              /*MSG*/
                else
                        goto ret_false;
        }
        if(ckname(tname, Resnames) != -1)
                cerror(MSGSTR(RESNAME,"Reserved Name in Expression"));  /*MSG*/
        SKIP;
        if(*Lp == '(') {
                ++Lp;
                i = rlist(')');
                if((builtin = ckname(tname, Fnames)) != -1) {
                        ...
                        if((j>=0 && j!=i)
                        || (i < -(j+1)))
argerr:                         cerror(MSGSTR(ARGCNT,"Arg. count"));    /*MSG*/
                        op(BUILTIN);
                        push(Fcode[builtin]);
                        push(i);
        ...
        if(i) {
                ...
                push(i);
                if(local(tname) != -1)
                        cerror(MSGSTR(SUBLN,"Subscript on local name"));/*MSG*/
...
pskip:
        if(Lnum) {
                ...
                fprintf(stderr, MSGSTR(SRCLN,"Source line %d: "), Lnum);/*MSG*/
        }
        fprintf(stderr, "%s\n%s%s^\b|\n", s, Line, eline);
        if(Lnum)
...
register *s;
{
        ...
        for(i = 0; i < LABELS; ++i) {
                if(EQL(tname, Label[i].l_name)) {
                        if(s && Label[i].l_instr)
                                cerror(MSGSTR(DUPLBL,"Dup. label"));    /*MSG*/
                        if(s)
                                Label[i].l_instr = s;
                        return i;
                } else {
        ...
        cerror(MSGSTR(TOOMLBS,"Too many labels"));                      /*MSG*/
}
...
symerr:
                error(MSGSTR(TBLOVF,"Symbol table overflow"));          /*MSG*/
        if(!Np[Namegrps]) {
                Np[Namegrps] = (struct names *)malloc(
                                        (unsigned)sizeof(struct names)*NAMES);
                if(!(int)Np[Namegrps])
                        goto symerr;
        }
...
char delim;
{
        ...
        for(;;) {
                ...
                if(*Lp != delim)
                        cerror(MSGSTR(ERRDEL,"Wrong delimiter"));       /*MSG*/
        }
}
...
struct htab *htable(sz)
{
        ...
        error(MSGSTR(TOOBIG,"Table too big"));                          /*MSG*/
}
...
register char *key;
{
        ...
        error(MSGSTR(TBLOVFL,"Table overflow"));                        /*MSG*/
}
...
struct estack *estackp;
{
        ...
        if(Lnames || Nest)
                error(MSGSTR(QERR,"Open function, if, or for"));        /*MSG*/
        for(;;) {
                switch(opr = *instr++) {
                case 0: /* end-of-instr */
                        Last = *estack;
...
namerr:
                                ...
                                error(MSGSTR(NAMCON," name context"));  /*MSG*/
                        }
                        break;
                case GT:
                case LT:
                ...
                case NEQ:
                        if((estack--)->t==STRING && estack->t==STRING)
                                ...
                                r.i = NLstrcmp(estack->v.sp, (estack+1)->v.sp);
#else 
...
brk:                    break;
                ...
                case SUBSCR:
                        ...
                        if(r.np->t == TABLE) {
                                if(args != 1)
                                        error(MSGSTR(SNGLDIM,           /*MSG*/
                                        "Only single dim. tables"));    /*MSG*/
                                r.ht = htitem((struct htab *)r.np->v.htabl,
                                                to_str(estack));
                                r.esp = &(r.ht->ht_v);
                                goto subsc;
                        }
                        for(ct = 0; ct < args; ++ct) {
                                if(r.np->set == ALLOC)
                                     error(MSGSTR(SUBS,"Subscript"));   /*MSG*/
                                r.np->t = ARRAY;
                                x = mkdouble(estack+ct);
                                if(x < 0)
                                 error(MSGSTR(SUBRNG,"Subscr. range")); /*MSG*/
                                r.np = getar(r.np, x);
                        }
subsc:
                        if(opr == SUBSCR)
...
ckif:
                ...
                case LINE:
                        ...
                        if(estack > (Estack+E_STACK-15))
                                error(MSGSTR(STOV,"Stack over"));       /*MSG*/
                        Lnum = *instr++;
                        break;
                case FUNCCALL:
                        ep = ++estack;
                        ...
                        if(r.fp >= (Fstack+F_STACK-1))
                                --Fstackp, error(MSGSTR(RECUR,          /*MSG*/
                                                        "Recursion"));  /*MSG*/
                        ...
                        instr = (int *)Label[*instr].l_instr;
                        if(*instr++ != FUNCDEF) {
                           printf(MSGSTR(UNDEF,"Undefined function ")); /*MSG*/
                           prtrace(r.fp);
                           printf(MSGSTR(RETURNC,"Return:  "));         /*MSG*/
                           Lp = salloc(128, 0);
                        ...
                        if(r.fp->callargs > ct)
                                error(MSGSTR(TOOMRG,"Too many args"));  /*MSG*/
...
retn:
                        ...
                        *estack = Last;
                        ...
                        --Fstackp;
                        if(Fstackp < Fstack)
                                error(MSGSTR(RETURND,"Return"));        /*MSG*/
                        break;
                case FRETURN:
                        if(J != &Jmp[0]) {
                                sv_estack = estack;
                ...
                case BUILTIN:
                        ...
                        case ARG:
                                ...
                                if(Fstackp == Fstack) { /* main func */
                                        if(r.i >= Argc || r.i < 0)
argerr:                                         error(MSGSTR(ARG_ERROR,"Arg"));
                        ...
                        case FTYPE:
                                ...
                                /*D44404 - Change bitwise and operator (&) in
                                ternary conditional to equals (==) as several
                                st_mode macros have bits in common.  For
                                example: 0x4000 & 0x6000 would return true,
                        ...
                        case SUBSTR:
                                ...
                                break;
                        case MATCH:
                                estack->v.d = ematch(to_str(estack),
                                                to_str(estack+1));
                                estack->t = DOUBLE;
                                break;
                        case MSTRING:
...
#ifdef KJI
                        case SIZE:
                                estack->v.d = (double)NLcplen(to_str(estack));
                                estack->t = DOUBLE;
                                break;
                        case BSIZE1:
                        ...
                        case FORMAT:
#ifdef KJI
                                NLsprintf((tstr=salloc(64,0)), to_str(estack),
                                        *(estack+1));
#else
                        ...
                        case TABLE:
                                ...
                                x = mkdouble(estack+1);
                                if(r.np->t)
                                        error(MSGSTR(ALLNAM,            /*MSG*/
                                             "Table: allocated name")); /*MSG*/
...
nottab:
                ...
                case DUMP:
                        s1 = (char *)*instr++;
                        if(LastS)
                                printf(MSGSTR(STATELT,                  /*MSG*/
                                        "Statement: %d\n"), LastS);     /*MSG*/
                        if(Ftrace > Fstack) {
                                printf(MSGSTR(FUNCTST,                  /*MSG*/
                                        "Function stack:\n"));          /*MSG*/
                                ...
                                if(r.np->t == TABLE) {
                                        ...
                                        for(x=0; x<r.htbl->ht_size; ++x)
                                                if(*r.htbl->ht_item[x].ht_key)
                                                 printf("%s[%s]=%s\n", s1,
                                                     r.htbl->ht_item[x].ht_key,
                                                   to_str(&(
                                                    r.htbl->ht_item[x].ht_v)));
                                        continue;
                                }
                                if(r.np->set==ALLOC) {
                                        printf("%.6s=%s\n",
                        ...
                        break;
                default:
                        error(MSGSTR(COMPIL,"Compiler"));               /*MSG*/
                }
                Last_op = opr;
        }
}
...
register char *s;
{
        ...
        fprintf(stderr, MSGSTR(EXECERR,"%s - execution error"), s);     /*MSG*/
        if(Lnum)
           fprintf(stderr, MSGSTR(SOURCLN," in source line %d"), Lnum); /*MSG*/
        fprintf(stderr, "\n");
        intr();
}
prtsubs(ps, np)
...
register struct estack *estack;
{
        ...
        if(estack->t==INPUT || estack->t==OUTPUT)
                ...
                if(estack->set == ALLOC)
                        free(s);
                if(Atof != strend)
conv:                   error(MSGSTR(CONV,"Conversion"));               /*MSG*/
                estack->t = DOUBLE;
                return estack->v.d;
        }
}
...
register struct estack *estack;
{
        ...
        if(estack->t == DOUBLE) {
                if(Obase == 10)
                        ...
                        sprintf(st, (Obase==8? "%lo": "%lx"),
                                (long)estack->v.d);
                        return st;
                }
                s = st + strlen(st);
                /*
                 *  107098 - was comparing *s to '.' but should compare
                 *           to the decimal point for the current locale
                 */
                while(*--s != decimal)
                        if(*s == '0') {
                                *s = '\0';
...
register ct, t;
{
        ...
        error(MSGSTR(STRSPC,"Out of string space"));                    /*MSG*/
}
/*VARARGS2*/
...
accerr:                 error(MSGSTR(NOACSF,"Cannot access file")); /*MSG*/
                np->t = INPUT;
                break;
        case 'W': /* no CR */
                *func = 'w';
        ...
        case 'a':
                ...
                break;
        default:
                error(MSGSTR(LSTARG,                                    /*MSG*/
                        "Last arg to open() must be r, w, or a"));      /*MSG*/
        }
...
register struct estack *estack;
{
        ...
        fcn = to_int(estack);
        if(fcn >= NGRAPH || fcn < 0)
                error(MSGSTR(ILLGRAF,"Illegal graph function"));        /*MSG*/
        if(args != gr[fcn].nargs)
                error(MSGSTR(ARGGRAF,"graph: arg count"));              /*MSG*/
        if(fcn && Graphout==0)
                error(MSGSTR(NOGRAF,"Graphic output never specified")); /*MSG*/
        estack++;
        ...
        if(fcn == 0) {          /* pipe output through plot(I) */
                ...
                if(strlen(stp = to_str(estack)) >  10)
                        error(MSGSTR(DSTGRAF,"graph: Bad dest."));      /*MSG*/
                strcat(Plot, stp);
                grio();
                return;
        }
        if(gr[fcn].code)
                ...
                switch(fcn) {
                case 6:         /* Dot */
                        error(MSGSTR(NODOT,"Dot not implemented"));     /*MSG*/
                        return;
...
grio()
{
        ...
        if(Graphout == NULL)
                error(MSGSTR(NOPLOT,"No plot!"));                       /*MSG*/
        Gio = 0;
}
