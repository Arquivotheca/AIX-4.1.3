static char sccsid[] = "@(#)70	1.4  src/bos/usr/ccs/bin/structure/2.dfs.c, cmdprog, bos411, 9428A410j 3/9/94 13:20:40";
/*
 * COMPONENT_NAME: (CMDPROG) Programming Utilites
 *
 * FUNCTIONS: addloop, chreach, dfs, exchange, insloop, repsearch, search
 *
 * ORIGINS: 26 27
 *
...
chreach()		/* look for unreachable nodes */
	...
	for (v = 0; v < nodenum; ++v)
		if (status[v] == UNPROCESSED && NTYPE(v) != FMTVX
			...
			unreach = TRUE;
			if (debug)
				fprintf(stderr,MSGSTR(NODEUNREACH, "node %d unreachable\n"),v); /*MSG*/
			}
	if (unreach) {
#ifdef MSG
		char tempmsg[200];
		strcpy(tempmsg,MSGSTR(BEIGNORED, "will be ignored"));
		error(MSGSTR(UNREACH, ": unreachable statements - "),tempmsg,""); /*MSG*/ /*MSG*/
#else MSG
		error(MSGSTR(UNREACH, ": unreachable statements - "),MSGSTR(BEIGNORED, "will be ignored"),""); /*MSG*/ /*MSG*/
#endif MSG
	}
	}
