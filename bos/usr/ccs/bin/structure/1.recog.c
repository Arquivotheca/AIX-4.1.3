static char sccsid[] = "@(#)66	1.5  src/bos/usr/ccs/bin/structure/1.recog.c, cmdprog, bos411, 9428A410j 3/9/94 13:20:19";
/*
 * COMPONENT_NAME: (CMDPROG) Programming Utilites
 *
 * FUNCTIONS: accum, compcase, contin, distinct, innerdo, makeif, nesteddo,
	      recognize
 *
 * ORIGINS: 26 27
 *
...
int type, ifflag;				/* do whatever is needed for this statement */
	...
	arctype = (int *) challoc(sizeof(*arctype) * sp);  arclab = (long *) challoc(sizeof(*arclab) * sp);
	...
	switch(type)
		...
		case DOVX:
			if (arctype[0] != -2) 
				{
				error("", "", "");
				fprintf(stderr, MSGSTR(ILLEGDORG,
	...
	if (debug)
		{
		fprintf(debfd,MSGSTR(LINECOL, "line %d:  "), begline); /*MSG*/
