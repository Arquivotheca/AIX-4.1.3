static char sccsid[] = "@(#)10  1.6.1.3  src/bos/usr/ccs/bin/indent/io.c, cmdprog, bos411, 9428A410j 6/21/91 15:14:58";
/*
 * COMPONENT_NAME: (CMDPROG) Programming Utilites
 *
 * FUNCTIONS: compute_code_target, compute_label_target, count_spaces, diag,
 *            dump_line, fill_buffer, pad_output, set_option
 *
 * ORIGINS: 26; 27
 *
    ...
    if (s_code == e_code && s_lab == e_lab && s_com == e_com) {
        ...
        if (s_com != e_com)
        {
            if (troff) {
                ...
                if (*(prevmbc(e_com)) == '/' && *(prevmbc(prevmbc(e_com))) == '*')
                    e_com -= 2;
                e_com = rtrimmbc (s_com, e_com, " ");
                *e_com = 0;
                p = s_com;
                while (*p == ' ')
                    p++;
...
register FILE   *f = input;
    ...
    /*
     * assuming that "fgets" handles MBs which may have an embedded '\n'
     * read the next line into the input buffer;  first, read in a pending
     * portion of a truncated MB character.
     */
    buf_ptr = in_buffer;
    i = 0;
    if (indent_mb_cur_max > 1)
    {
        for (i = 0; win_buffer[i]; i++)
            in_buffer[i] = win_buffer[i];
        win_buffer[0] = 0;                      /* no MB left */
    }
    if (fgets (&in_buffer[i], sizeof(in_buffer)-i, f) == NULL)
    {
        strcpy (in_buffer, " \n");              /* newline the empty buffer */
        had_eof = true;                         /* we are done */
    }
    for (p = in_buffer; *p; p++);               /* position p to the end */

    if (indent_mb_cur_max > 1)                  /* record a possible truncated MB */
    {
        w_old = prevmbc (p);
        if (mbsinvalid (w_old))
        {
            for (i = 0; w_old[i]; i++)
                win_buffer[i] = w_old[i];
            win_buffer[i] = 0;                  /* close truncated copy */
            p = w_old;                          /* p = truncated end */
#ifdef MBCDEBUG
            printf ("fill_buffer: MB truncation <%s>\n", win_buffer);
#endif
        }
    ...
    char       *buffer;
{
    ...
    for (buf = buffer; *buf != '\0'; buf = nextmbc (buf)) {
        switch (*buf) {
...
char *msg;
{
    if (output == stdout) {
        ...
        if (level == 0)
                fprintf(stdout, MSGSTR(WARNING, "Warning@%d: "), line_no);
        else {
                /* GH 03/01/91 A17100 indent exited with code 0 even if error */
                exit_code = 1;
                fprintf(stdout, MSGSTR(ERROR, "Error@%d: "), line_no);
             }
        ...
        if (level == 0)
                fprintf(stderr, MSGSTR(WARNING, "Warning@%d: "), line_no);
        else {
                exit_code = 1;
                fprintf(stderr, MSGSTR(ERROR, "Error@%d: "), line_no);
             }
