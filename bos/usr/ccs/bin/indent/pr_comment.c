static char sccsid[] = "@(#)13  1.8  src/bos/usr/ccs/bin/indent/pr_comment.c, cmdprog, bos411, 9428A410j 11/1/93 11:40:42";
/*
 * COMPONENT_NAME: (CMDPROG) Programming Utilites
 *
 * FUNCTIONS: pr_comment
 *
 * ORIGINS: 26 27
 *
...
pr_comment()
{
    ...
    if (ps.box_com) {
        ...
        while (*buf_ptr == ' ' || *buf_ptr == '\t')
            buf_ptr++;
        /*
         * 110082 - advanced buf_ptr but did not check for end
         */
        if(buf_ptr >= buf_end)
            fill_buffer();
    }
        ...
        switch (*buf_ptr) {     /* this checks for various spcl cases */
            ...
            case '\n':
                if (had_eof) {  /* check for unexpected eof */
                    printf(MSGSTR(NOENDCOMMENT, "Unterminated comment\n")); /*MSG*/
                    *e_com = '\0';
                    dump_line();
                    return;
                }
                ...
                if (*(prevmbc(e_com)) == ' ' || *(prevmbc(e_com)) == '\t')
                    last_bl = e_com - 1;
                /*
            ...
            end_of_comment:
                    ...
                    if (*(prevmbc(e_com)) != ' ' && !ps.box_com) { /* insure blank before end */
                        *e_com++ = ' ';
                        ++now_col;
                    }
                    if (break_delim == 1 &&
                        ( single_line_bc || !one_liner )
                        && s_com[0] == '/'
                        && s_com[1] == '*' && s_com[2] == ' ') {
                        char       *t = e_com;
                        break_delim = 2;
            ...
            default:            /* we have a random char */
                ...
                if (*e_com == '\t')     /* keep track of column */
                    ...
                    now_col -= widthmbc(prevmbc(e_com));
                else
                    now_col += widthmbc(e_com);
                ...
                if (now_col > adj_max_col && !ps.box_com && unix_comment == 1 && e_com[-1] > ' ') {
                    ...
                    *last_bl = '\0';
                    last_bl = rtrimmbc (s_com, last_bl, NULL);
                    *last_bl = 0;
