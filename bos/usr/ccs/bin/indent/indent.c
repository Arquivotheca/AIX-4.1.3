static char sccsid[] = "@(#)08  1.9.1.8  src/bos/usr/ccs/bin/indent/indent.c, cmdprog, bos411, 9428A410j 3/16/94 08:41:45";
/*
 * COMPONENT_NAME: (CMDPROG) Programming Utilites
 *
 * FUNCTIONS: main, bakcopy, cksubstr
 *
 * ORIGINS: 26,27
 *
    ...
    char      **argv;
{
    ...
    extern int exit_code;       /* GH 03/01/91 A17100 */
    int         last_else = 0;  /* true iff last keyword was an else */
    int         magic_comma = false;
    int         plist_tail = false;
    int         proc_online = false;
    int         paren_cnt = 0;
    int         len_procname = 0;
    int         procname_len = 0;
    int         allign_param = false;

    char        ch;             /* local character */
    int         n;              /* local counter */
    ...
    indent_mb_cur_max = MB_CUR_MAX;             /* set local copy */
    catd = catopen(MF_INDENT, NL_CAT_LOCALE);
    ...
    win_buffer[0] = 0;                          /* no input buffer carry over */
    line_no = -1;               /* we haven't started the input files yet */
        ...
        if (argv[i][0] != '-') {/* no flag on parameter */
            if (input == 0) {   /* we must have the input file */
                ...
                if (input == 0) {       /* check for open error */
                    fprintf(stderr, MSGSTR(CANTOPEN, "indent: can't open %s\n"), argv[i]); /*MSG*/
                    exit(1);
                }
                continue;
                ...
                if (strcmp(in_name, out_name) == 0) {   /* attempt to overwrite
                                                         * the file */
                    fprintf(stderr, MSGSTR(CANTOWRITE, "indent: input and output files must be different\n")); /*MSG*/
                    exit(1);
                }
                output = fopen(out_name, "w");
                if (output == 0) {      /* check for create error */
                    fprintf(stderr, MSGSTR(CANTCREATE, "indent: can't create %s\n"), argv[i]); /*MSG*/
                    exit(1);
                }
                continue;
            }
            fprintf(stderr, MSGSTR(BADPARAM2, "indent: unknown parameter: %s\n"), argv[i]); /*MSG*/
            exit(1);
        } else
            set_option(argv[i]);
    ...
    if (input == 0) {
        printf(MSGSTR(USAGE, "Usage: indent file [ outfile ] [ options ]\n")); /*MSG*/
        exit(1);
    }
    if (output == 0)
        if (troff)
    ...
    line_no = 1;                /* we are about to look at the input files */
    ...
    parse(semicolon);
    {
        ...
        if (col > ps.ind_size && ps.ind_size != 0)
    ...
    while (1) {                 /* this is the main loop.  it will go
        ...
        type_code = lexi();     /* lexi reads one token.  The actual
                                 ...
                                 * of token */
        if (is_procname = ps.procname[0])
            procname_len = strlen(ps.procname);
                        ...
                        for (;;) {      /* loop until we get to the end of
                            ...
                            *sc_end = *buf_ptr++;
                            ...
                            if (sc_end >= &(save_com[sc_size])) {       /* check for temp buffer
                                                                         * overflow */
                                diag(1, MSGSTR(BUFFERFULL, "Internal buffer overflow - Move big comment from right after if, while, or whatever.")); /*MSG*/
                                fflush(output);
                                exit(1);
                            }
                        }
                    ...
                    if (force_nl) {     /* if we should insert a nl here,
                        ...
                        *sc_end++ = ' ';
                        if (verbose && !flushed_nl)     /* print error msg if
                                                         * the line was not
                                                         * already broken */
                            diag(0, MSGSTR(BROKENLINE, "Line broken")); /*MSG*/
                        flushed_nl = false;
                    }
...
check_type:
        if (type_code == 0) {   /* we got eof */
            ...
            if (ps.tos > 1)     /* check for balanced braces */
                diag(1, MSGSTR(EARLYEOF, "Stuff missing from end of file.")); /*MSG*/

            if (verbose) {
                printf(MSGSTR(OUTLINE, "There were %d output lines and %d comments\n"),
                       ps.out_lines, ps.out_coms);
                printf(MSGSTR(COMPERCODE, "(Lines with comments)/(Lines with code): %6.3f\n"),
        ...
        switch (type_code) {    /* now, decide what to do with the token */
            ...
            case lparen:        /* got a '(' or '[' */
                ...
                if (ps.last_token == ident) proc_online = true;
                break;
            ...
            case rparen:        /* got a ')' or ']' */
                ...
                if (--ps.p_l_follow < 0) {
                    ps.p_l_follow = 0;
                    if (ps.leave_comma || !new_func_proto
                                || !ps.in_parameter_declaration)
                        diag(0, MSGSTR(EXTRAPAREN, "Extra %c"), *token); /*MSG*/
                    else
                        ps.ind_level--;
                }
                paren_cnt--;
                if (!ps.leave_comma && new_func_proto
                        && *token == ')' && paren_cnt == 0)
                {
                    new_func_proto = ps.in_or_st = false;
                    ps.i_l_follow = procname_len = len_procname = 0;
                    plist_tail = ps.in_decl = ps.decl_on_line = false;
                }
            ...
            case unary_op:      /* this could be any unary operation */
                ...
                if (troff && !ps.dumped_decl_indent && ps.in_decl) {
                    ...
                    if (ps.in_decl && !ps.block_init) { /* if this is a unary op
                        ...
                        if (!ps.leave_comma && new_func_proto
                                        && (plist_tail || proc_online)
                                        && ps.in_parameter_declaration)
                            while ((e_code - s_code) <
                                        (dec_ind - i + len_procname + 1))
                                *e_code++ = ' ';        /* pad it */
                        else
                            while ((e_code - s_code) < (dec_ind - i))
                                *e_code++ = ' ';        /* pad it */
                    }
                    if (troff && (token[0] == '-') && (token[1] == '>') && !mbsinvalid(token))
                        res = "\\(->";
        ...
        comma_to_semi:
                ...
                scase = false;  /* these will only need resetting in a
                                 * error */
                squest = 0;
                if (ps.last_token == rparen && !magic_comma)
                ...
                if ((!sp_sw || hd_type != forstmt) && ps.p_l_follow > 0) {
                     ...
                     */
                    if (ps.leave_comma || !magic_comma)
                        diag(1, MSGSTR(UNBALPAREN, "Unbalanced parens"));/*MSG*/
                    ps.p_l_follow = 0;
            ...
            case decl:          /* we have a declaration type (int,
                        ...
                        && ps.in_parameter_declaration )
                {
                    if (!len_procname)
                        if (e_code > s_code)
                            len_procname = e_code - s_code - 1;
                        else
                            len_procname = procname_len;
                    if (allign_param)
                    {
                        for(i = len_procname; i >= 0; i--)
                            *e_code++ = ' ';
                        allign_param = false;
                    }
                }
                ...
                else if (!ps.leave_comma && new_func_proto)
                    ps.in_parameter_declaration = 1;
                if (ps.in_parameter_declaration && ps.indent_parameters && ps.dec_nest == 0) {
                    if (!ps.leave_comma && new_func_proto)
                        ps.ind_level = ps.i_l_follow = ps.p_l_follow = 0;
                    else
                ...
                if ( /* !ps.in_or_st && */ ps.dec_nest <= 0)
                    ps.just_saw_decl = 2;
                if (ps.tos > 1 || (!blanklines_after_procs && !blanklines_after_declarations) || ps.dec_nest > 0)
                    prefix_blankline_requested = 0;
            ...
            case lstring:                       /* L"foo" potential */
            case ident:                         /* got an identifier or constant */

                if (ps.last_token == lstring)   /* keep L"foo" together */
                    ps.want_blank = false;

                if (ps.in_decl)                 /* if we are in a declaration, we must indent identifier */
                {
                    if (ps.want_blank)
                        *e_code++ = ' ';
                    ps.want_blank = false;
                             ...
                             && ps.in_parameter_declaration )
                    {
                        if (!ps.block_init)
                            ...
                            {
                                if (!len_procname)
                                    len_procname = strlen(ps.procname);
                                while ((e_code - s_code) <
                                       dec_ind + len_procname + 1)
                                    *e_code++ = ' ';
                            }
            ...
            case comma:
                if (!ps.leave_comma && new_func_proto && paren_cnt == 1
                        && ps.in_parameter_declaration)
                {
                    plist_tail = magic_comma = allign_param = true;
                    goto comma_to_semi;
                }
            ...
            case preesc:        /* got the character '#' */
                ...
                *(s_com = e_com) = '\0';
                        ...
                        switch (ch) {
                            ...
                            case '/':
                                if (*buf_ptr == '*' && !in_comment && !quote) {
                                    in_comment = 1;
                                    *e_lab++ = *buf_ptr++; /* copy in the '*' */
                                    com_start = e_lab - 2;
                                }
                                break;
                            case '"': /* MF: Useless code?? */
                                if (quote == '"')
                                    quote = 0;
                                break;
                            case '\'': /* MF: Useless code?? */
                ...
                if (cksubstr(s_lab, "#if", 3) == 0)             /* */
                    if (ifdef_level < sizeof state_stack / sizeof state_stack[0]) {
                        match_state[ifdef_level].tos = -1;
                        state_stack[ifdef_level++] = ps;
                    } else
                        diag(1, MSGSTR(TOOMANYIFDEF, "#if stack overflow")); /*MSG*/
                else if (cksubstr(s_lab, "#elif", 5) == 0)      /* */
                    if (ifdef_level <= 0)                       /* */
                        diag(1, MSGSTR(ORPHANELIF, "Unmatched #elif")); /*MSG*/
                    else {                                      /* */
                        match_state[ifdef_level - 1] = ps;      /* */
                        ps = state_stack[ifdef_level - 1];      /* */
                    }                                           /* */
                else if (cksubstr(s_lab, "#else", 5) == 0)      /* */
                    if (ifdef_level <= 0)
                        diag(1, MSGSTR(ORPHANELSE, "Unmatched #else")); /*MSG*/
                    else {
                        match_state[ifdef_level - 1] = ps;
                        ps = state_stack[ifdef_level - 1];
                    }
                else if (cksubstr(s_lab, "#endif", 6) == 0)     /* */
                    if (ifdef_level <= 0)
                        diag(1, MSGSTR(ORPHANENDIF, "Unmatched #endif")); /*MSG*/
                    else {
                        ifdef_level--;
#ifdef notdef
                        ...
                        if (match_state[ifdef_level].tos >= 0
                            ...
                            diag(0, MSGSTR(BADSYNTAX, "Syntactically inconsistant #ifdef alternatives.")); /*MSG*/
#endif
                    }
                else if (cksubstr(s_lab, "#define", 7) == 0)
                    /*ps.i_l_follow = 0;*/ ;
...
bakcopy()
{
    ...
    if (bakchn < 0) {
        fprintf(stderr, MSGSTR(CANTCREATEBAK, "indent: can't create backup file \"%s\"\n"), bakfile); /*MSG*/
        exit(1);
    }
    while ((n = read(fileno(input), buff, sizeof buff)) > 0)
        if (write(bakchn, buff, n) != n) {
            fprintf(stderr, MSGSTR(NOWRITEBAK, "indent: error writing backup file \"%s\"\n"), /*MSG*/
                bakfile);
            exit(1);
        }
    if (n < 0) {
        fprintf(stderr, MSGSTR(NOREADINPUT, "indent: error reading input file \"%s\"\n"), in_name); /*MSG*/
        exit(1);
    }
    close(bakchn);
    fclose(input);
    ...
    if (input == NULL) {
        fprintf(stderr, MSGSTR(CANTREOPEN, "indent: can't re-open backup file\n")); /*MSG*/
        exit(1);
    }
...
/*
 *  check whether the current label is one of the following six:
 *      #if, #ifdef, #ifndef, #elif, #else, #endif.
 *  It is possible to have some white spaces (including comments)
 *  before # sign, or right after it.
*/

int  cksubstr(label_ptr, pattern_ptr, len)
char   *label_ptr;
char   *pattern_ptr;
int     len;
{
    ...
    /* skip spaces and comments right before # sign */
    while ( *s != '#' && strlen (s) > 3 &&
                        (*s != '?' || s[1] != '?' || s[2] != '=' ) )
    {
        /* skip leading spaces */
    ...
    /* skip spaces and comments right after # sign */
    while ( 1 )
    {
        /* skip spaces in between */
        for ( ; *s == ' ' || *s == '\t'; s++ );

        /* discard comments in between */
        if ( *s == '/' )
        {
            if (*++s != '*')
                return 1;
            ...
            return ( strncmp(s, &pattern_ptr[1], len - 1) );
        if (*s == '\0')
            return 1;
    }   /* while */
