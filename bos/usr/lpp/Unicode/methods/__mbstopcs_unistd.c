static char sccsid[] = "@(#)47	1.1  src/bos/usr/lpp/Unicode/methods/__mbstopcs_unistd.c, cfgnls, bos411, 9428A410j 1/21/94 10:14:30";
/*
 *   COMPONENT_NAME: CFGNLS
 *
 *   FUNCTIONS: __mbstopcs_unistd
 *
 *   ORIGINS: 27
 *
...
size_t __mbstopcs_unistd( _LC_charmap_objhdl_t handle, 
			  wchar_t *pwcs, size_t pwcs_len,
			  const char *s, 
			  size_t s_len, 
			  char stopchr, 
			  char **endptr, 
			  int *err )
{
	...
	**********/
	if ((cnt = __mbtopc_unistd(handle, &(pwcs[pwcs_cnt]), &(s[s_cnt]), 
	    (s_len - s_cnt), err)) == 0) {
	    *endptr = &(s[s_cnt]);
	    break;
	}
