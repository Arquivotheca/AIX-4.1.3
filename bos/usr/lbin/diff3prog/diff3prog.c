static char sccsid[] = "@(#)48  1.12  src/bos/usr/lbin/diff3prog/diff3prog.c, cmdfiles, bos411, 9428A410j 4/11/94 19:29:24";
/*
...
/*
 * @OSF_COPYRIGHT@
 * No OSF rcsid string found in usr/lbin/diff3prog/diff3prog.c
 */
/*
 *                                                                   
...
#include "diff3_msg.h"
nl_catd	catd;
#define MSGSTR(Num, Str) catgets(catd, MS_DIFF3, Num, Str)
...
char **argv;
{
	...
	catd = catopen(MF_DIFF3, NL_CAT_LOCALE);
	eflag = 0; oflag = 0;
	if(*argv[1]=='-') {
		switch(argv[1][1]) {
		case 'e':
		...
		case 'x':
			eflag = 1;
                ...
                case 'X':
                        ...
                        break;
		default:
			fprintf(stderr, MSGSTR(UNKFLG,"diff3: Unknown flag: '%c'\n"), argv[1][1]);
			fprintf(stderr, MSGSTR(USAGE,
				"Usage: diff3 [-e|-x|-E|-X|-3] File1 File2 File3\n"));
			exit(1);
		}
		argv++;
		argc--;
	...
	for(i=0;i<=2;i++)
		if((fp[i] = fopen(argv[i+3],"r")) == NULL) {
			fprintf(stderr, MSGSTR(CANTOPEN, "diff3: can't open %s\n"),
								argv[i+3]);
			exit(1);
		}
	merge(m,n);
