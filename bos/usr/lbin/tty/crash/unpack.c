#ifndef lint
static char sccsid[] = "@(#)97  1.1  src/bos/usr/lbin/tty/crash/unpack.c, cmdtty, bos411, 9428A410j 3/3/94 10:07:42";
#endif
/*
...
int dump_mem;    /* global - fd for /dev/kmem or dump file */
int readflag;    /* global - true if running system */
...
/* This is the tty_read_mem function that all the print routines
 * will be calling.  Just an interface to the read_memory function
 * above, but read_memory returns 0 on failure and 1 on success.
 * the return values for tty_read_mem are 0 for success and -1 for
 * failure.
 */
int tty_read_mem(ulong addr, void *buf, int size)
{
  if (read_memory(dump_mem, readflag, buf, addr, size))
	  return 0;
  else return -1;
}
