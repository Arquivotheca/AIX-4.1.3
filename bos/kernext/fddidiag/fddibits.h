/* @(#)75       1.1  src/bos/kernext/fddidiag/fddibits.h, diagddfddi, bos411, 9428A410j 11/1/93 10:59:49 */
/*
 ...
 */
#define FDDI_NULL 		(0x0fdd0000)	/* the initial state 
						 */
#define FDDI_DWNLDING 		(0x0fdd0dac)	/* Downloading
						 * microcode to adapter
						 */
#define FDDI_DWNLD		(0x0fdd0010)	/* microcode has been
						 * successfully loaded
						 * we will be in this
						 * after the config method
						 * has successfully finished.
						 */
#define FDDI_INIT		(0x0fdd0020)	/* The 1st CIO_START has
						 * been issued.  We are 
						 * attempting to bring up the
						 * adapter.
						 */
#define FDDI_OPEN		(0x0fdd0fdd)	/* all things have been done
						 * for the adapter to be fully
						 * operational.  We are open
						 * for business and taking 
						 * orders.
						 */

#define FDDI_LLC_DOWN		(0x0fdd0303)	/* LLC services have been
						 * disabled.  We are in a
						 * pseudo state of limbo 
						 */

#define FDDI_LIMBO		(0x0fdd0404)	/* We are in limbo. we are
						 * trying to recover from a
						 * significant error.
						 */

#define FDDI_CLOSING		(0x0fdd0505)	/* shutting down the device */
#define FDDI_DEAD		(0x0fdddead)	/* the adapter has 
						 * died, it is no 
						 * longer useable 
						 */
...
#define FDDI_MAX_RDTO		(256)	 	/* maximum rdto allowed */
...
#define FDDI_SMT_THRESH		(60)	/* The threshold of errors for smt */
					/*  events */
...
#define FDDI_SMT_ERR_HI_CCE	(0x0004)	/* configuration change event */
...
#define FDDI_SMT_ERR_LO_TME	(0x0010)	/* Trace Max exceeded */
...
#define FDDI_SMT_EVNT_HI_RGA	(0x0001)	/* Removed long group address */
/*
 * SMT Event Word (Low)
 */
#define FDDI_SMT_EVNT_LO_AGA	(0x8000)	/* Added long group address */
#define FDDI_SMT_EVNT_LO_RTT	(0x4000)	/* Restricted token terminated*/
#define FDDI_SMT_EVNT_LO_CW	(0x2000)	/* Connection withheld */
...
#define FDDI_SMT_EVNT_LO_MD	(0x0002)	/* MAC Disconnected due to */
						/*  remote cmd */
 ...
 * SMT Control Word
 */
#define	FDDI_SMT_CTL_PROM	(0x0020)	/* set the adapter into prom */
...
#define FDDI_HCR_NO_ADDR	(0x2C01) /* No addresses avail */
 ...
 * Shared Ram
 */
#define FDDI_RCV_SHARED_RAM		(0xf0)
...
#define FDDI_HCR_ABORT_RCV		(0x0400) /* Abort Receive */
...
#define FDDI_HSR_ERRORS	(FDDI_HSR_NSF	|\
			...
			FDDI_HSR_ACI	)
/*
 * Events errors are other errors handled in the offlevel routine.
 *	These errors are generated from PIO, ...
 *	They are OR'd into the high order 16 bits of the oflv_events.
 *	The low order 16 bits of the oflv_events are the HSR bits (see above).
 */
#define FDDI_EVENT_ERRORS	(0xFF0000)	/* other errors */
#define FDDI_EXCEPT_IO		(0x010000)	/* io exception reading HSR */
#define FDDI_DEAD_IO		(0x020000)	/* exception retry exceeded */
#define	FDDI_CMD_WDT_IO		(0x040000)	/* the cmd wdt popped */
#define	FDDI_CLOSE_WDT_IO	(0x080000)	/* the close wdt popped */
#define	FDDI_LIMBO_WDT_IO	(0x100000)	/* the limbo wdt popped */
#define	FDDI_DNLD_WDT_IO	(0x200000)	/* the dnld wdt popped */
#define	FDDI_TX_WDT_IO		(0x400000)	/* the tx wdt popped */

/*
 * Only interrupt on ERRORS, and CCI
 */
#define FDDI_HSR_BASE_INTS	(0x817B)
...
#define FDDI_NS1_RCV    (0x0800) /* Receive Command */
...
/* values for fddi_tx_t.flags */
#define FDDI_TX_EXT		(0x01)	/* extension is valid */
#define FDDI_TX_SMALLFRAME	(0x02)	/* this descriptor has a smallframe */

/* RECEIVE bits for control: adap.ctl  (Set by HOST) */
#define FDDI_RCV_CTL_BDV	(0x8000)	/* Buffer descriptor valid */
#define FDDI_RCV_CTL_IFR	(0x4000)	/* Interrupt Frame Receive */

/* RECEIVE bits for statue: adap.stat  (Set by NPOSW) */
#define FDDI_RCV_STAT_SV	(0x8000)	/* Status Valid */
#define FDDI_RCV_STAT_SOF	(0x4000)	/* Start of Frame */
#define FDDI_RCV_STAT_EOF	(0x2000)	/* End of Frame */
#define FDDI_RCV_STAT_BTS	(0x1000)	/* Buffer to Small */
#define FDDI_RCV_STAT_AB 	0x0800
#define FDDI_RCV_STAT_LF 	0x0400
#define FDDI_RCV_STAT_AF 	0x0200
#define FDDI_RCV_STAT_FCS	0x0100

#define FDDI_CLBYTES		(4096)		/* HW Lim (pg size */
#define FDDI_MAX_PACKET  	(4500)		/* (specs) */
#define FDDI_RI_SA		(0x80)
#define FDDI_GRP_ADDR		(0x80)
...
#define FDDI_FC_SMT		(0x40)	/* */
#define FDDI_FC_NSA		(0x47)	/* */
#define FDDI_FC_BEA		(0xC2)	/* */
...
#define FDDI_READ               (121)
#define FDDI_WRITE              (122)

/*
 * Component dump table stuff
 */
#define FDDI_CDT_ENTRIES	(32)	/* initial size of dump entries */
#define FDDI_DD_STR		"fddidd"

#define FDDI_CIO_TIMEOUT 	(CIO_TIMEOUT << 16)

#define FDDI_STEST_CNT	11
