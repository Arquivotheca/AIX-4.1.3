static char sccsid[] = "@(#)97  1.33  src/bos/kernext/dlc/e3l/e3lmod.c, sysxdlci, bos411, 9428A410j 6/2/94 10:21:06";

/**********************************************************************
 * COMPONENT_NAME: (SYSXDLCI) IEEE Ethernet Data Link Control
 *
 * FUNCTIONS: e3lmod.c
 *
 * ORIGINS: 27
...
/**********************************************************************/
/*  This module contains 802.3 Ethernet specific routines only.       */
/**********************************************************************/
  ...
  register struct port_dcl *p;
{
  ...
  p->d.send_data->lpdu_length = (p->m->m_next->m_len+3);
  if (p->m->m_next->m_len < (MIN_PACKET-UN_HDR_LENGTH)) { /* Defect 115940 */
    ...
    p->m->m_next->m_len = (MIN_PACKET-UN_HDR_LENGTH); /* End Defect 115940 */
  } 
  p->m->m_len = UN_HDR_LENGTH;
...
/* <<< end feature CDLI >>> */
  ...
  /********************************************************************/
  /* build and send the test response in the same buffer that the test*/
  /* command was received in, and bump test ras counts.               */
  /********************************************************************/
...
#ifdef   DEBUG
    if (p->debug)
      printf("e3lmod:using resolve\n");
#endif

    /******************************************************************/
    /* build the remainder of the DLC header for a test command       */
    /******************************************************************/
