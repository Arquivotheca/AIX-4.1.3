static char sccsid[] = "@(#)07  1.39.1.10  src/bos/kernext/dlc/trl/trlmod.c, sysxdlct, bos411, 9439A411b 9/26/94 18:07:46";
/**********************************************************************
...
/**********************************************************************/
/*  This module contains Token Ring specific routines only.           */
/**********************************************************************/
...
/* <<< feature CDLI >>> */
      if /* no routing is established */
	 (p->sta_ptr->ri_length < 3)
	/* reset the link station's routing length */
	p->sta_ptr->ri_length = 0;
      p->ri.ptr_ri = ((p->d.send_data->ri_field)+p->sta_ptr->ri_length);

      /****************************************************************/
      /* copy the prebuilt dlc header (1st 14 bytes) from the station */
      /* cb                                                           */
      /****************************************************************/

      bcopy(&(p->sta_ptr->phy_ctl1), &(p->d.send_data->phy_ctl_1),
	      TRL_ROUTING_OFFSET);
/* <<< end feature CDLI >>> */
...
/* <<< feature CDLI >>> */
      p->m->m_len = TRL_ROUTING_OFFSET +4 +p->sta_ptr->ri_length;
/* <<< end feature CDLI >>> */
    } 
...
/* <<< feature CDLI >>> */
      cmd_size = TRL_ROUTING_OFFSET +3 +p->sta_ptr->ri_length;
/* <<< end feature CDLI >>> */
...
/* LEHb defect XXX */
      ...
      p->sta_cmd_buf->phy_ctl_2 = p->sta_ptr->phy_ctl2;
/* LEHe */
...
/* <<< feature CDLI >>> */
      /****************************************************************/
      /* call the write send command generator, with station command  */
      /* buffer address.                                              */
      /****************************************************************/
...
/* LEHe */
      ...
      /****************************************************************/
/* <<< feature CDLI >>> */
      cmd_size = p->sta_ptr->ri_length +TRL_ROUTING_OFFSET +4;
/* <<< end feature CDLI >>> */
      ...
      p->d.send_data->phy_ctl_2 = p->sta_ptr->phy_ctl2;
...
/* <<< feature CDLI >>> */
      /****************************************************************/
      /* call the write send command generator, with station response */
      /* buffer address.                                              */
      /****************************************************************/
...
/* LEHb defect XXX */
      ...
      p->rnr_rsp_buf->phy_ctl_2 = p->sta_ptr->phy_ctl2;
/* LEHe */
...
/* LEHb defect XXX */
  ...
  p->disc_rsp_buf->phy_ctl_2 = p->sta_ptr->phy_ctl2;
/* LEHe */
...
/* <<< removed ack_slot >>> */
  ...
  p->m->m_len = p->sta_ptr->ri_length +TRL_ROUTING_OFFSET +3;

  /********************************************************************/
  /* write the disconnect response to the device handler, with        */
  /* the disconnect response buffer address.                          */
  /********************************************************************/
...
/* LEHb defect XXX */
/* deleted trl_buf_build() */
/* LEHe */

/* <<< feature CDLI >>> */
trl_setfa(p, opcode, in_len_func_addr, in_func_addr_ptr)
  register struct port_dcl *p;
  int      opcode;
  ulong_t  in_len_func_addr;
  uchar_t  *in_func_addr_ptr;
{
  uchar_t   tmp_func_addr[6];
  int      rc;
  int      i;
  uchar    tmpchar;

  if /* the functional address length is correct */
     ((in_len_func_addr == 4) ||
      (in_len_func_addr == 6))
    {
	 ...
	 (in_len_func_addr == 4)
	{
	  ...
	  bcopy(in_func_addr_ptr, &tmp_func_addr[2], 4);
	}
      else /* it's a 6-byte CDLI address */
	{
	  /* copy in all 6-bytes from the input address */
	  bcopy(in_func_addr_ptr, &tmp_func_addr[0], 6);
	}

      /* force the top 2-bytes to 0xC000 */
      tmp_func_addr[0] = 0xc0;
      tmp_func_addr[1] = 0x00;

      /* reset the group address bit (ie. it's a functional address) */
      tmp_func_addr[2] &= 0x7F;

      if /* deleting the functional address */
	 (opcode == DLC_DEL_FUNC_ADDR)
        {
	  if /* a functional address already exists for this sap */
	     (p->sap_ptr->sap_profile.len_func_addr_mask == 6)
	    {
	      /* issue NDD_DISABLE_ADDRESS to the device driver */
	      rc = (*p->nddp->ndd_ctl)(p->nddp, NDD_DISABLE_ADDRESS,
						       &tmp_func_addr, 6);
	      if (rc == 0)
		{
		  /******************************************/
		  /* AND in the inverted mask to the sap cb */
		  /******************************************/
		  for (i = 0; i < 6; i++)
		      p->sap_ptr->sap_profile.func_addr_mask[i] &=
						     ~tmp_func_addr[i];

		  /********************************************************/
		  /* check if the last functional addr bit has been reset */
		  /********************************************************/
		  tmpchar = 0;
		  for (i = 0; i < 6; i++)
		      tmpchar |= p->sap_ptr->sap_profile.func_addr_mask[i];

		  if /* there are no functional address bits remaining */
		     (tmpchar == 0)
		    {
		      /* set the sap cb functional address length to zero */
		      p->sap_ptr->sap_profile.len_func_addr_mask = 0;
		    }
		}
	      else /* ndd disable address failed */
		return (EIO);
	    }
	  else /* no functional address exists for this sap */
            return (EINVAL);
        } 
      else /* adding the functional address */
        {
	  /* issue NDD_ENABLE_ADDRESS to the device driver */
	  ...
	  if (rc == 0)
		...
		{
		  /**************************************************/
		  /* OR in the new functional address to the sap cb */
		  /**************************************************/
	      ...
	      else /* no functional address exists for this sap yet */
		{
		  /* copy the new functional address to the sap cb */
		  for (i = 0; i < 6; i++)
	              p->sap_ptr->sap_profile.func_addr_mask[i] =
						      tmp_func_addr[i];

		  /* set the sap cb functional address length to 6 */
		  p->sap_ptr->sap_profile.len_func_addr_mask = 6;
		}
	    }
  ...
  else /* input functional address length is invalid */
    return (EINVAL);
}                                      /* end trl_setfa               */
/* <<< end feature CDLI >>> */
...
/* <<< removed tladdr[6] >>> */
  ...
  /********************************************************************/
  /* save access control byte (masking off invalid bits)              */
  /********************************************************************/

  tphy1 = (p->rcv_data.phy_ctl_1 >> 5);

  /********************************************************************/
  /* update buffer offset value                                       */
  /********************************************************************/

/* <<< removed shift_size gorp >>> */
...
/* defect 162732 */
  ...
  p->d.data_ptr = (p->i.i_field_ptr - p->common_cb.ri_length
                                    - TRL_ROUTING_OFFSET -3);

  /* re-adjust the mbuf's data pointer and length */
  p->m->m_data = (caddr_t)p->d.data_ptr;
  p->m->m_len = p->lpdu_length + TRL_ROUTING_OFFSET + p->common_cb.ri_length;
...
/* LEHb defect XXX */
				       /* set access control to the
					     station's frame priority */
...
/* LEHe */
  ...
  /********************************************************************/
/* <<< feature CDLI >>> */
/* This is a rework of apar ix33108 defect 76763 */
/* The synchronous bit should be reset, ie. priority = 0x00 */
  p->d.send_buf_rsp->phy_ctl_1 = 0x00;
...
/* <<< this is a rework of token-ring apar IX34430 defect 82097 >>> */
  ...
  /********************************************************************/
  /* swap the remote and destination sap fields. temp_rsap =          */
  /* rcv_data.rsap;                                                   */
  /********************************************************************/

  temp_rsap = p->ri.ri_rcv->rsap;
  p->ri.ri_sbp->lsap = (p->ri.ri_sbp->rsap|RESP_ON);
  ...
  if                                   /* the received buffer         
     ...
     (((int)p->m->m_nextpkt & DLC_OFLO) == DLC_OFLO)
    {
      ...
      p->m->m_len = p->common_cb.ri_length +TRL_ROUTING_OFFSET +3;
/* <<< end feature CDLI >>> */
    } 
...
/* <<< feature CDLI >>> */
    /******************************************************************/
    /* set the frmr control byte-2 to received control byte-2         */
    /******************************************************************/
/* <<< end feature CDLI >>> */
...
/* <<< feature CDLI >>> */
  ...
  p->m->m_len = TRL_ROUTING_OFFSET +3 + p->sta_ptr->ri_length +
		sizeof(struct i_field_frmr);
/* <<< end feature CDLI >>> */
...
/* LEHe */
  ...
  /********************************************************************/
/* <<< feature CDLI >>> */
/* <<< removed "round up to the next full word" >>> */

  p->m->m_len = (if_size+TRL_ROUTING_OFFSET +3 +p->sta_ptr->ri_length);
/* <<< end feature CDLI >>> */
...
/* <<< feature CDLI >>> */
      p->d.send_data->phy_ctl_1 = 0x00;
/* <<< end feature CDLI >>> */
      p->d.send_data->phy_ctl_2 = 0x40;

/* LEHb defect XXX */
/* deleted phy_1 and phy_2 */
/* LEHe */
...
/* LEHe */
      ...
      /****************************************************************/
      /* call trlbuild to build find vector                           */
      /****************************************************************/
...
/* <<< feature CDLI >>> */
      p->m->m_len = p->common_cb.ri_length +vec_size
						+TRL_ROUTING_OFFSET +3;
...
/* LEHe */
  ...
  if                                   /* station using resolve       */
     ((p->sta_ptr->ls_profile.flags&DLC_SLS_ADDR) != 0)
    {
      ...
      bcopy(p->sta_ptr->ls_profile.raddr_name, p->d.send_data->raddr, 
         6);
...
/* LEHe */
  ...
  /********************************************************************/
  /* call trlbuild to build found vector                              */
  /********************************************************************/
  ...
  /********************************************************************/
/* <<< feature CDLI >>> */
  p->m->m_len = TRL_ROUTING_OFFSET +3 +vec_size +p->common_cb.ri_length;
/* <<< end feature CDLI >>> */
...
/* <<< end feature CDLI >>> */
  ...
  if                                   /* link trace activated        */
    ...
    lanrcvtr(p);
  tphy1 = p->rcv_data.phy_ctl_1;
  tphy2 = p->rcv_data.phy_ctl_2;
...
/* <<< feature CDLI >>> */
/* <<< removed shift_size >>> */
  p->m->m_data += p->common_cb.ri_length;

  /********************************************************************/
  /* update data pointer to include routing information offset        */
  /********************************************************************/

  p->d.data_ptr += p->common_cb.ri_length;
/* <<< end feature CDLI >>> */
...
/* <<< end defect 128100 >>> */
  ...
  p->d.send_buf_rsp->phy_ctl_2 = tphy2;
...
/* <<< feature CDLI >>> */
      p->m->m_len = TRL_ROUTING_OFFSET +3 +ret +p->common_cb.ri_length;
/* <<< end feature CDLI >>> */
    } 
...
/* <<< feature CDLI >>> */
      p->m->m_len = p->common_cb.ri_length +TRL_ROUTING_OFFSET +3 +3;
/* <<< end feature CDLI >>> */
