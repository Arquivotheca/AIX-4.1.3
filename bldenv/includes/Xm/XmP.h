/* @(#)83	1.10  src/gos/2d/MOTIF/lib/Xm/XmP.h, libxm, gos41J, 9508A 2/14/95 14:52:46 */
/*
...
*/ 
/*   $RCSfile: XmP.h,v $ $Revision: 1.163 $ $Date: 94/10/10 08:06:04 $ */
/*
...
/* NON_OSF_FIX: Added Display parameter, ref. 129126 */
extern XImage * _XmGetImageFromFile( 
                        Display *display,
                        char *filename) ;
extern XImage * _XmGetImageAndHotSpotFromFile( 
...
#ifdef BIDI
extern void _XmStringDirectionDefault();
#endif /* BIDI */
...
#ifdef _NO_PROTO
Boolean _XmStringIsDirection();
void _XmStringPropagateDirection();
#else /* _NO_PROTO */
Boolean _XmStringIsDirection(XmString string);
void _XmStringPropagateDirection(_XmString string, XmStringDirection direction);
#endif /* _NO_PROTO */

#define GetLayout(w)     (_XmGetLayoutDirection((Widget)(w)))
#define LayoutM(w)       (XmDirection) \
                         (((XmManagerWidget)(w))->manager.string_direction)
#define LayoutP(w)       (XmDirection) \
                         (XmIsLabel(w) ? \
                         ((XmLabelWidget)w)->label.string_direction : \
                          (XmIsManager((w)->core.parent) ? \
                            LayoutM((w)->core.parent) : \
                             GetLayout(w)))
#define LayoutG(w)     (XmDirection) \
                       (XmIsLabelGadget(w) ? \
                         ((XmLabelGadget)(w))->label.cache->string_direction :\
                         (XmIsManager(((XmGadget)(w))->object.parent) ? \
                          LayoutM(((XmGadget)(w))->object.parent) : \
                          GetLayout(w)))


#define LayoutIsRtoL(w)      (XmDirectionMatch(GetLayout(w), XmRIGHT_TO_LEFT))
#define LayoutIsRtoLM(w)     (XmDirectionMatch(LayoutM(w), XmRIGHT_TO_LEFT))
#define LayoutIsRtoLP(w)     (XmDirectionMatch(LayoutP(w), XmRIGHT_TO_LEFT))
#define LayoutIsRtoLG(w)     (XmDirectionMatch(LayoutG(w), XmRIGHT_TO_LEFT))


/* in Alpha, only XmLEFT_TO_RIGHT an XM_RIGHT_TO_LEFT are supported, but
  the full range is implemented as such:                                   */
