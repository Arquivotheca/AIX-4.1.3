#!/bin/sh
  SCCSID=" @(#)89        1.2  src/bldenv/pkgtools/rename/gen_inst.sh, pkgtools, bos412, GOLDA411a 2/1/93 12:21:18"
#
# COMPONENT_NAME: gen_inst.sh
#
# FUNCTIONS:
#	Parse and store command line parameters.
#	Generate ccss-formatted bosinst package.
#	Place bosinst package onto the ship server.
#	Add bosinst entry to the ship server index file.
#
# ORIGINS: 27
#
...
#
# Purpose:
#	Generate ccss-formatted bosinst package that will be
#	placed onto the ship server with an entry added to the
#	ship server index file.
#
# Command Format:
#	gen_inst -b bosinst-file-name [-h|-?]
#		where:
#		-b path & file-name of input bosinst binary.
...
syntax() {				# You get here if you err or ask.
    ...
    echo "$myName -b bosinst-file-name [-h|-?]"
    echo "    where"
    echo "	-b path & file-name of input bosinst binary file.\n"
...
WORK=$BLDTMP/gen_inst.$$		# Name of unique dir.
...
if [ $# -lt 1 ]; then			# Must have one parameter.
    echo "$myName: ERROR: Bosinst file-name must be in command line."
...
do					#   loop through this routine
	...
	"-b")				# -b is for input bosinst binary
	    bosInst=$2			#   path & file-name.
	    shift 2;;
...
#
 if [ ! -f "$bosInst"  -o  ! -r "$bosInst" ]
    then
    echo "$myName: ERROR: Unable to read bos inst file $bosInst"
    rc=7; Exit
 fi
 if [ ! -w "$SHIP_DIRNAME" ]; then
    echo "$myName: ERROR: $SHIP_DIRNAME is not writable."
...
#
#
 echo "======processing bos inst file $bosInst ======"
#
# Reblock the file to required blocksize.
#
 dd if=$bosInst of=$WORK/reblocked bs=$BLOCK_SIZE conv=sync
 rc=$?					# Did dd work?
 if [ "$rc" -ne 0 ]			# If not, tell user & abort.
     then
     echo "\n$myName: ERROR: dd error number $rc reblocking bos inst file.\n"
     rc=80; Exit
 fi
#
# Pack it.
#
 ccss_pack -2 $WORK/reblocked -c $SHIP_DIRNAME/bosinst.ptf
...
#
# Delete old bosinst index entry & append a new one.
#
#  The grep copies all lines that do not have 'bosinst' in 2nd subfield.
 grep -v "^[^:]*:bosinst:" $SHIP_DIRNAME/index >  $WORK/new_ship_index
#  The echo appends new bosinst entry to work index before copy back.
 echo "bos:bosinst:prereq:bos:v=03 r=02 m=0000"  >> $WORK/new_ship_index
 cp   $WORK/new_ship_index $SHIP_DIRNAME/index
#
# Log successful run of gen_inst.
#
...
#
 echo "\n$myName:  bosinst.ptf successfully placed on ship server.\n"
 rc=0; Exit
#
### End of gen_inst ####### End of gen_inst ####### End of gen_inst ###
